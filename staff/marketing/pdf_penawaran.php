...
$query_data1	= "SELECT * FROM `gx_penawaran` WHERE  `gx_penawaran`.`id_penawaran` = '$id_data';";
$sql_data1		= mysql_query($query_data1, $conn);
$row_data1		= mysql_fetch_array($sql_data1);

$hit_print		= $row_data1["print_pdf"] + 1;
$sql_update 	= "UPDATE `gx_penawaran` SET `print_pdf` = '$hit_print' WHERE `id_penawaran` = '$_GET[id]';";        
...
for ($i = 1; $i < 7; $i++)
{
    ...
    //$pdf->Cell(0, 30, 'TCPDF and FPDI');
    $pdf->Cell(130);
    $pdf->Cell(1,5, "Proposal No : $row_data1[kode_penawaran] / Copy ke $hit_print");
