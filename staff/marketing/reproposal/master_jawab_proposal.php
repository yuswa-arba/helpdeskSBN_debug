...
if($loggedin["id_level"] == "Kabag"){
	$sql_masterjawabproposal = mysql_query("SELECT * FROM `gx_jawab_proposal` WHERE `level` = '0' AND `id_cabang` = '$loggedin[cabang]' ORDER BY `id_jawab_proposal` DESC LIMIT $start,$perhalaman;",$conn);
	$sql_total_masterjawabproposal = mysql_num_rows(mysql_query("SELECT * FROM `gx_jawab_proposal` WHERE `level` = '0' AND `id_cabang` = '$loggedin[cabang]' ORDER BY `id_jawab_proposal` DESC;",$conn));
}else{
	$sql_masterjawabproposal = mysql_query("SELECT * FROM `gx_jawab_proposal` WHERE `level` = '0' AND `user_add` = '$loggedin[username]' ORDER BY `id_jawab_proposal` DESC LIMIT $start,$perhalaman;",$conn);
	$sql_total_masterjawabproposal = mysql_num_rows(mysql_query("SELECT * FROM `gx_jawab_proposal` WHERE `level` = '0' AND `user_add` = '$loggedin[username]' ORDER BY `id_jawab_proposal` DESC;",$conn));
}
...
while ($row_masterjawabproposal = mysql_fetch_array($sql_masterjawabproposal))
{
    ...
    $row_cabang		= mysql_fetch_array($sql_cabang);
	 $content .= '<tr>
		    ...
		    <td align="center">
				<a href="detail_jawab_proposal?id='.$row_masterjawabproposal['id_jawab_proposal'].'"><span class="label label-info">Detail</span></a>  <a href="form_jawab_proposal?id='.$row_masterjawabproposal['id_jawab_proposal'].'"><span class="label label-info">Edit</span></a>
		    </td>
		</tr>';
		$no++;
}
