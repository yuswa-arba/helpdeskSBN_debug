...
if($row_data1['monthly_fee1'] != ''){
	$monthly1 = 'Rp '. number_format($row_data1['monthly_fee1'], 0, '.', ',');
}else{
	$monthly1 = '';
}
if($row_data1['monthly_fee2'] != ''){
	$monthly2 = 'Rp '. number_format($row_data1['monthly_fee2'], 0, '.', ',');
}else{
	$monthly2 = '';
}
if($row_data1['monthly_fee3'] != ''){
	$monthly3 = 'Rp '. number_format($row_data1['monthly_fee3'], 0, '.', ',');
}else{
	$monthly3 = '';
}
if($row_data1['monthly_fee4'] != ''){
	$monthly4 = 'Rp '. number_format($row_data1['monthly_fee4'], 0, '.', ',');
}else{
	$monthly4 = '';
}
if($row_data1['monthly_fee5'] != ''){
	$monthly5 = 'Rp '. number_format($row_data1['monthly_fee5'], 0, '.', ',');
}else{
	$monthly5 = '';
}

$nama_paket1 = $row_data1['nama_paket1'];
$nama_paket2 = $row_data1['nama_paket2'];
$nama_paket3 = $row_data1['nama_paket3'];
$nama_paket4 = $row_data1['nama_paket4'];
$nama_paket5 = $row_data1['nama_paket5'];
...
for ($i = 1; $i < 7; $i++)
{
	...
	*/if($i == 3)
        ...
        $tbl = <<<EOF
<table border="0" cellpadding="5" cellspacing="5" align="center" style="width:65%;font-size:12px;">
 
    <tr nobr="false">
        <td style="background-color:#000099;color:#fff;">{$nama_paket1}</td>
        <td style="background-color:#99b3ff;color:#000;">{$monthly1}</td>
    </tr>
    <tr nobr="false">
        <td style="background-color:#000099;color:#fff;">{$nama_paket2}</td>
        <td style="background-color:#99b3ff;color:#000;">{$monthly2}</td>
    </tr>
    <tr nobr="true">
        <td style="background-color:#000099;color:#fff;">{$nama_paket3}</td>
        <td style="background-color:#99b3ff;color:#000;">{$monthly3}</td>
    </tr>
	<tr nobr="true">
        <td style="background-color:#000099;color:#fff;">{$nama_paket4}</td>
        <td style="background-color:#99b3ff;color:#000;">{$monthly4}</td>
    </tr>
	<tr nobr="true">
        <td style="background-color:#000099;color:#fff;">{$nama_paket5}</td>
