...
if(isset($_GET["f"])){
 if($_GET["f"] == "brosur"){
  ...
  while ($row_cabang = mysql_fetch_array($query_cabang)) {
	 ...
	 $tanggal    = date("d");
	 $kode_cabang = $row_cabang["kode_cabang"].'-'.$row_cabang["kode_proposal"].''.$tanggal.''.sprintf("%04d", $last_data);
	 $content .='<tr>
		     <td>'.$no.'</td>
		     <td>'.$row_cabang["kode_cabang"].'</td>
		     <td>'.$row_cabang["nama_cabang"].'</td>
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(kodeproposal, idcabang, namacabang){
		 window.opener.document.'.$return_form.'.kode_proposal.value=kodeproposal;
		 window.opener.document.'.$return_form.'.kode_cabang.value=idcabang;
