...
if(isset($_GET["f"])){
 if($_GET["f"] == "aktivasi"){
  ...
  while ($row_cabang = mysql_fetch_array($query_cabang)) {
	 $sql_spk_aktivasi_konversi = mysql_fetch_array(mysql_query("SELECT * FROM `gx_jawab_spk_aktivasi_konversi` ORDER BY `id_jawab_spk_aktivasi_konversi` DESC", $conn));
	 $last_data  = $sql_spk_aktivasi_konversi["id_jawab_spk_aktivasi_konversi"] + 1;
	 $tanggal    = date("d");
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(kode_jawab_spk_konversi,idcabang, namacabang){
		 window.opener.document.'.$return_form.'.kode_jawab_spk_aktivasi.value=kode_jawab_spk_konversi;
		 window.opener.document.'.$return_form.'.kode_cabang.value=idcabang;
		 window.opener.document.'.$return_form.'.nama_cabang.value=namacabang;
