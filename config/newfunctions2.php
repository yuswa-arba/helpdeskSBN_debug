...
function get_nama_cabang($id_cabang)
{
    ...
    $sql_cabang = mysql_query("SELECT * FROM `gx_cabang`
                         WHERE `id_cabang` = '" . $id_cabang . "' LIMIT 1;", $conn);

    $row_cabang = mysql_fetch_array($sql_cabang);

    return $row_cabang["nama_cabang"];
}
...
//login all group untuk menampilkan menu sesuai dengan group
function logged_inGlobal()
{
    global $conn;
    $sess_id = (isset($_COOKIE["GXSOFTWARE"]) != "") ? ($_COOKIE["GXSOFTWARE"]) : "";

    $sess_check = mysql_query("SELECT * FROM `gxSessions_admin`, `gxLogin_admin`
                              WHERE `gxSessions_admin`.`id_employee` = `gxLogin_admin`.`id_employee`
                              AND `gxSessions_admin`.`sess_id` = '" . $sess_id . "'
                              AND `gxSessions_admin`.`logged` = '0';", $conn);
    // If there is no session in the table where they are not logged in, show them as not logged in

    if (mysql_num_rows($sess_check)) { // Check if there is a row in the table.
        $s = mysql_fetch_array($sess_check); // Retrieve the data from the tables.
        $uinfo = mysql_query("SELECT `gxLogin_admin`.*, `gx_pegawai`.*
                             FROM `gxLogin_admin`, `gx_pegawai`
                             WHERE `gxLogin_admin`.`id_employee` = `gx_pegawai`.`kode_pegawai`  AND
                             `gxLogin_admin`.`id_employee` = '" . $s['id_employee'] . "';", $conn); // Retrieve the users table where the uid matches the uid in the sessions table
 ...
 * Start
 */
function enableLog($id_customer = "", $nama_customer = "", $id_admin = "", $activity = "", $script = "")
{
    ...
    mysql_query("INSERT INTO `gx_log_detail` (`id_logs`, `id_customer`, `nama_customer`, `id_admin`,`activity`, `script`, `time`, `ip`, `browser_detail`)
		VALUES (NULL, '" . mysql_real_escape_string($id_customer) . "', '" . mysql_real_escape_string($nama_customer) . "', '" . mysql_real_escape_string($id_admin) . "',
                '" . mysql_real_escape_string($activity) . "', '" . mysql_real_escape_string($script) . "', NOW(), '" . $_SERVER['REMOTE_ADDR'] . "',
