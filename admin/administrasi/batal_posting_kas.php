...
if($loggedin = logged_inAdmin())
{
	 if($loggedin["group"] == 'admin')
	 {
		  ...
		  if(isset($_POST["id"]))
		  {
			   ...
			   {
					$sql_bm = mysql_query("SELECT * FROM `gx_kas_masuk` WHERE `transaction_id`='".$id."' LIMIT 1;", $conn);
					...
					if(($id != ""))
					{
						 ...
						 $sql_update_batal = "UPDATE `gx_kas_masuk` SET `level`='1', `date_upd` = NOW(), `user_upd`= '".$loggedin["username"]."'
