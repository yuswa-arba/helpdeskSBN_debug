...
if(isset($_POST["update"]) OR isset($_POST["updatelock"]))
{
    ...
    $sql_update = "UPDATE `gx_returbeli_detail` SET `kode_returbeli`='".$kode_returbeli."',
	...
	$sql_update_lock = "UPDATE `gx_returbeli` SET `status`='1', `date_upd` = NOW(), `user_upd` = '".$loggedin["username"]."'
	WHERE `kode_returbeli`='".$kode_returbeli."';";
	...
	echo "<script language='JavaScript'>
	alert('Data telah diupdate.');
	window.location.href='master_returbeli';
	</script>";
    }
...
if(isset($_GET["c"]))
{
    ...
    $return_url 	= 'form_returbelidetail.php?c='.$kode_beli;
}
    ...
    $content = '
			...
			<section class="col-lg-12"> 
					...
					<div class="row">
					    ...
					    <div class="col-xs-2">
						<input type="text" readonly="" class="form-control" required="" name="kode_returbeli" value="'.(isset($_GET["c"]) ? $row_data["kode_returbeli"] : "").'" >
					    </div>
					    <div class="col-xs-2">
						<label>No Pembelian</label>
					    </div>
					    <div class="col-xs-2">
						<input type="text" class="form-control" readonly="" required="" name="kode_beli" id="kode_beli" value="'.(isset($_GET["c"]) ? $row_data["kode_beli"] : "").'"
					...
					<div class="row">
					    ...
					    <div class="col-xs-2">
						<input type="text" class="form-control" id="acc_hutang" name="acc_hutang" value="'.(isset($_GET["c"]) ? $row_data["acc_hutang"] : "").'">
					    </div>
					    <div class="col-xs-2">
...
if(isset($_GET["c"]))
{
    ...
    $query_setujubeli_item	= "SELECT * FROM `gx_returbeli_detail` WHERE `kode_returbeli` ='".$row_data["kode_returbeli"]."';";
    $sql_setujunbeli_item	= mysql_query($query_setujubeli_item, $conn);
    $id_detail_setujubeli  = isset($_GET['id']) ? (int)$_GET['id'] : '';
    $query_setujubeli_detail = "SELECT * FROM `gx_returbeli_detail` WHERE `kode_returbeli` ='".$row_data["kode_returbeli"]."' AND `id_returbeli_detail` = '".$id_detail_setujubeli."' LIMIT 0,1;";
	...
	<td><a href="form_returbelidetail?c='.$row_setujubeli_item["kode_returbeli"].'&id='.$row_setujubeli_item["id_returbeli_detail"].'"><span class="label label-info">Edit</span></a></td>
	</tr>';
	$no++;
	    ...
	    <td>
		    &nbsp;
					...
					<div class="row">
						...
						<input type="hidden" name="id" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["id_returbeli_detail"] : '').'" readonly="">
						<input type="hidden" name="kode_returbeli" value="'.(isset($_GET['c']) ? $row_data["kode_returbeli"] : "").'" readonly="">
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control" required="" name="qty" id="qty" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["qty"] : "").'">
					    </div>
                                        </div>
					</div>
