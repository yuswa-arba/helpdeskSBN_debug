...
//		$conn_soft = Config::getInstanceSoft();//koneksi RBS
        ...
        function topup_voip()
        {
            ...
            enableLog("", $loggedin["username"], $loggedin["id_employee"], "$sql_update_card"); // for create log in newfunction2.php
        ...
        if (isset($_POST["posting"])) {
            ...
            if ($id_customer != "" AND $transaction_id != "") {
                //detail_bankmasuk
                $query_bm = "SELECT SUM(`gx_bm_detail`.`nominal`) AS `nominal_total`, `gx_bm_detail`.*
                    FROM `gx_bank_masuk`, `gx_bm_detail`
                ...
                while ($row_bm = mysql_fetch_array($sql_bm)) {
                    ...
                    if ($nominal > 0) {
                        ...
                        $sql_update_data = "UPDATE `tbCustomer` SET `gx_saldo`='" . $UserPaymentBalance_update . "'
			                WHERE (`cKode`='" . $id_customer . "');";
                        mysql_query($sql_update_data, $conn) or die (mysql_error());

                        enableLog("", $loggedin["username"], $loggedin["id_employee"], $sql_update_data); // for create log in newfunction2.php
                        ...
                        if ($row_bm["no_acc"] == "492") {
                            ...
                            $RECHARGEID = "TV" . sprintf("%05d", $count);
                ...
                $sql_update_lock = "UPDATE `gx_bank_masuk` SET `reference`='" . $BillingTransactionIndex . "', `posting`='1', `status`='1', `date_upd` = NOW(), `user_upd`= '" . $loggedin["username"] . "'
