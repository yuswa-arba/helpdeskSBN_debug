...
if(isset($_GET["f"])){
 if($_GET["f"] == "aktivasi"){
  ...
  while ($row_cabang = mysql_fetch_array($query_cabang)) {
	 ...
	 $nama_acc = $sql_penjualan_barang['nama_acc'];
	 $nama_divisi = $sql_penjualan_barang['nama_divisi'];
	 $kode_divisi = $sql_penjualan_barang['kode_divisi'];
	 ...
	 $content .='<tr>
		     ...
		     <td>'.$row_cabang["user_add"].'</td>
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($kode_cabang).'\', \''.mysql_real_escape_string($row_cabang["id_cabang"]).'\', \''.mysql_real_escape_string($row_cabang["nama_cabang"]).'\', \''.mysql_real_escape_string($nama_acc).'\', \''.mysql_real_escape_string($kode_divisi).'\', \''.mysql_real_escape_string($nama_divisi).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(noaccpengeluaran,idcabang, namacabang, namaacc, kodedivisi, namadivisi){
		 window.opener.document.'.$return_form.'.kode_acc_permintaan.value=noaccpengeluaran;
		 window.opener.document.'.$return_form.'.kode_cabang.value=idcabang;
		 window.opener.document.'.$return_form.'.nama_cabang.value=namacabang;
		 window.opener.document.'.$return_form.'.nama_acc.value=namaacc;
