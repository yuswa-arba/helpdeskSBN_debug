...
if($loggedin["group"] == 'admin'){
     enableLog("", $loggedin["username"], $loggedin["username"], "Open List Master kas masuk");
    global $conn;
...
if(isset($_GET["id"]) & isset($_GET["action"]))
{
    ...
    header("location: master_kasmasuk.php");
	...
	<hr>
			   ...
			   <div class="box-header">
					...
					<div class="box-tools pull-right">
						 <a class="btn bg-olive btn-flat margin" href="'.URL_ADMIN.'administrasi/form_kasmasuk.php"><span>Create new</span></a>
					</div>
			   </div>
			   <div class="box-body ">
...
while ($row_kasir = mysql_fetch_array($sql_kasir))
{
	 $sql_total = mysql_query("SELECT SUM(`nominal`) AS total FROM gx_km_detail
							  WHERE gx_km_detail.id_kasmasuk = '".$row_kasir["id_kasmasuk"]."'
							  GROUP BY gx_km_detail.id_kasmasuk", $conn);
...
<script>
	 function hapus(id)
	 {
		  ...
		  if (r === true) {
			  window.location.href = "master_kasmasuk?id=" + id + "&action=hapus";
		  }
	 }
