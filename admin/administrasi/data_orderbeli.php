...
if(isset($_POST["save_search"])){
 ...
 $sql_data	= "SELECT * FROM `gx_order_beli` WHERE `level` = '0' ORDER BY `id_order_beli` DESC LIMIT 0,20;";
}
		$content .='<table class="table table-bordered table-striped" id="customer">
                <thead>
                  <tr>
                    <th>No.</th>
                    <th>Kode Order Pembelian</th>
		    <th>Tanggal</th>
		    <th>Keterangan</th>
		    <th>Action</th>
                  </tr>
...
$no = 1;
 ...
 while ($row_data = mysql_fetch_array($query_data)) {
    ...
    $query_item	= "SELECT SUM(`qty` * `harga`) AS `subtotal`, SUM(`qty`) AS `qtytotal` FROM `gx_setuju_pembelian_detail` WHERE `kode_setuju_pembelian` ='".$row_data["kode_setuju_pembelian"]."';";
    $sql_item	= mysql_query($query_item, $conn);
    $row_item	= mysql_fetch_array($sql_item);
    if($row_data["ppn"] == "yes"){
	$ppn	= (10/100 * $row_item["subtotal"]);
    }elseif($row_data["ppn"] == "no"){
	$ppn	= 0;
    }else{
	$ppn	= 0;
    }
	...
	$content .='<tr>
		    ...
		    <td>
                      <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_data["kode_order_beli"]).'\',';
...
<!-- DataTable -->
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(orderbeli, setujubeli, vqty, sub, vppn, vdisc, grand, supplier, nama_su, shippping, nama_sh){
		 window.opener.document.'.$return_form.'.kode_order_beli.value=orderbeli;
		 window.opener.document.'.$return_form.'.kode_setuju_beli.value=setujubeli;
