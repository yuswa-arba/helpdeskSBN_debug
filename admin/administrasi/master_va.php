...
$content = '
		    ...
		    <div class="row">
			    ...
			    <div class="box">
				<div class="box-body">
				    <a href="form_va.php" class="btn bg-maroon btn-flat margin">Add New</a>
				</div>
			    </div>
			</div>
		    </div>
...
//select data
	...
	$content .='<tr>
			<td>'.$no.'.</td>
			<td>'.$row_data["nama_va"].'</td>
			<td>'.$row_data["nama_rekening"].'</td>
			<td>'.$row_data["kode_bank"].'</td>
			<td><a href="detail_va.php?id='.$row_data["id_va"].'" onclick="return valideopenerform(\'detail_va.php?id='.$row_data["id_va"].'\',\'va\');">Details</a>  ||
			<a href="form_va.php?id='.$row_data["id_va"].'">edit</a></td>
		</tr>';
	$no++;
    }
