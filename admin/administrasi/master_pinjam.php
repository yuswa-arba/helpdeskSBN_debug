...
$content = '
		    ...
		    <div class="row">
			    ...
			    <div class="box">
				<div class="box-body">
				    <a href="form_pinjam.php" class="btn bg-maroon btn-flat margin">Create New</a>
				</div>
			    </div>
			</div>
		    </div>
...
//select data
    ...
    $sql_data		= mysql_query("SELECT * FROM `gx_pinjam_barang` WHERE `level` =  '0' ORDER BY `id_pinjam` DESC LIMIT $start, $perhalaman;", $conn);
    $sql_total_data	= mysql_num_rows(mysql_query("SELECT * FROM `gx_pinjam_barang` WHERE `level` =  '0';", $conn));
    $hal		= "?";
    $no = $start + 1;
	...
	$content .='<tr>
			...
			<td><a href="detail_pinjam?c='.$row_data["kode_pinjam"].'"><span class="label label-info">Detail</span></a>
			   <!-- <a href="form_pinjam?c='.$row_data["kode_pinjam"].'"><span class="label label-info">Edit</span></a>-->
			</td>
		</tr>';
	$no++;
    }
