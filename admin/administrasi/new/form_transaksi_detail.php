...
//save ke database gx_transaksi    
if(isset($_POST["save"]))
{

	$id_detail			= isset($_POST['id_detail']) ? mysql_real_escape_string(trim($_POST['id_detail'])) : '';
	$idtransaksi_detail	= isset($_POST['idtransaksi_detail']) ? mysql_real_escape_string(trim($_POST['idtransaksi_detail'])) : '';
	$idinvoice_detail	= isset($_POST['idinvoice_detail']) ? mysql_real_escape_string(trim($_POST['idinvoice_detail'])) : '';
	$noacc_detail		= isset($_POST['noacc_detail']) ? mysql_real_escape_string(trim($_POST['noacc_detail'])) : '';
	$keterangan_detail	= isset($_POST['keterangan_detail']) ? mysql_real_escape_string(trim($_POST['keterangan_detail'])) : '';
	$nominal_detail		= isset($_POST['nominal_detail']) ? mysql_real_escape_string(trim($_POST['nominal_detail'])) : '';
	$tgl_txn			= isset($_POST['tgl_txn']) ? mysql_real_escape_string(trim($_POST['tgl_txn'])) : '';
	$waktu				= isset($_POST['waktu']) ? mysql_real_escape_string(trim($_POST['waktu'])) : '';
	$lokasi				= isset($_POST['lokasi']) ? mysql_real_escape_string(trim($_POST['lokasi'])) : '';
	$berita1			= isset($_POST['berita1']) ? mysql_real_escape_string(trim($_POST['berita1'])) : '';
	$berita2			= isset($_POST['berita2']) ? mysql_real_escape_string(trim($_POST['berita2'])) : '';
	$date_add			= isset($_POST['date_add']) ? mysql_real_escape_string(trim($_POST['date_add'])) : '';
	$date_upd			= isset($_POST['date_upd']) ? mysql_real_escape_string(trim($_POST['date_upd'])) : '';
    ...
    if($kode_bm != ""){
    //insert into cc_subscription_service
    $sql_insert = "INSERT INTO `gx_transaksi_detail` (`id_transaksi_detail`, `id_transaksi`, `no_acc`,
...
elseif(isset($_POST["update"]))
{
    ...
    $sql_update = "UPDATE `gx_transaksi_detail` SET `id_transaksi`='".$kode_bm."', `no_acc`='".$acc_bm."', 
...
if(isset($_POST["posting"]))
{
	...
	{
	//detail_transaksi
	$query_bm	= "SELECT `gx_transaksi_detail`.* FROM `gx_transaksi`, `gx_transaksi_detail`
	WHERE `gx_transaksi`.`id_transaksi` = `gx_transaksi_detail`.`id_transaksi`
	AND `gx_transaksi`.`transaction_id` ='".$transaction_id."' AND  `gx_transaksi`.`level` ='0';";
    $sql_bm		= mysql_query($query_bm, $conn);
	
	//detail user
	$query_user	= "SELECT `iuserIndex`, `cUserID` FROM `tbCustomer` WHERE `cKode` ='".$id_customer."';";
	...
	while($row_bm = mysql_fetch_array($sql_bm))
	{
		...
		while($row_invoice = mysql_fetch_array($sql_invoice))
		{
			...
			if($row_invoice["desc"] == "INTERNET")
			{
				...
				{
					//posting ke database RBS sesuai transaksi_detail
					//echo "rbs + voip";
					//INPUT RBS
					$sql = "SET IDENTITY_INSERT [dbo].[BillingTransactions] ON
					INSERT INTO [dbo].[BillingTransactions] ([BillingTransactionIndex], [CreateDate], [UserType], [UserIndex], [TransType], [Reference], [Total])
...
if(isset($_GET["bm"]))
{
    ...
    $query_data 	= "SELECT `tbCustomer`.`cNama`, `gx_transaksi`.* FROM `gx_transaksi`, `tbCustomer`
			    WHERE `gx_transaksi`.`id_customer` = `tbCustomer`.`cKode`
				AND `gx_transaksi`.`transaction_id` = '".$kode_data."'
			    LIMIT 0,1;";
    $sql_data	= mysql_query($query_data, $conn);
    $row_data	= mysql_fetch_array($sql_data);
   
    
    $return_url = 'form_transaksi_detail.php?bm='.$kode_data;
...
if(isset($_GET["bm"]))
{
    $id_transaksi	= $row_data["id_transaksi"];
    $query_item	= "SELECT * FROM `gx_transaksi_detail` WHERE `id_transaksi` ='".$id_transaksi."';";
    $sql_item	= mysql_query($query_item, $conn);
    
    $id_transaksi_detail  = isset($_GET['id']) ? (int)$_GET['id'] : '';
    $query_detail = "SELECT * FROM `gx_transaksi_detail` WHERE `id_transaksi` ='".$id_transaksi."' AND `id_transaksi_detail` = '".$id_transaksi_detail."' LIMIT 0,1;";
	...
	<!--<td><a href="form_transaksi_detail?bm='.$kode_data.'&id='.$row_item["id_transaksi_detail"].'"><span class="label label-info">Edit</span></a>
	<a href="form_transaksi_detail?bm='.$kode_data.'&id='.$row_item["id_transaksi_detail"].'&act=del"><span class="label label-danger">Hapus</span></a>
	</td>-->
	</tr>';
	$no++;
...
$content .='<tr>
                        ...
                        <section class="col-lg-9"> 
                            <div class="box box-solid box-info">
                                ...
                                <!-- form start -->
                                <form action="" role="form" name="form_transaksi_item" id="form_transaksi_item" method="post" enctype="multipart/form-data">
					...
					<div class="row">
						...
						placeholder="Search Invoice" onclick="return valideopenerform(\'data_invoice.php?r=form_transaksi_item&f=bmd&c='.$row_data["id_customer"].'\',\'bmd\');">
						<input type="hidden" name="id" value="'.(isset($_GET['id']) ? $row_detail["id_transaksi_detail"] : '').'" readonly="">
						<input type="hidden" name="kode_bm" value="'.(isset($_GET['bm']) ? $row_data["id_transaksi"] : '').'" readonly="">
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" readonly="" class="form-control" required name="acc_bm" onclick="return valideopenerform(\'data_acc.php?r=form_transaksi_item&f=bm\',\'bm\');"
