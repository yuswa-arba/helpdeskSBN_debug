...
if ($loggedin = logged_inAdmin()) { // Check if they are logged in
    ...
    if ($loggedin["group"] == 'admin') { // jika yang login adalah admin
        ...
        enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data Bank"); // for create log in newfunction2.php
        ...
        while ($row_data = mysql_fetch_array($query_data)) {
            $content .= '
                <tr>
                    ...
                    <td>' . $row_data["nama"] . '</td>
                    <td>
                      <a href="" onclick="validepopupform2(\'' . mysql_real_escape_string($row_data["kode_bank"]) . '\',\'' . mysql_real_escape_string($row_data["nama"]) . '\',\'' . mysql_real_escape_string($row_data["no_acc"]) . '\',\'' . mysql_real_escape_string($row_data["nama_acc"]) . '\')">Select</a>
                    </td>
                </tr>';
        ...
        $plugins .= '
            <script type="text/javascript">
                function validepopupform2(kb, nb, acc, nama){
                    window.opener.document.' . $return_form . '.kode_bank.value=kb;
                    window.opener.document.' . $return_form . '.acc_bank.value=acc;
