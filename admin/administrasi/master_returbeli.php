...
$content = '
		    ...
		    <div class="row">
			    ...
			    <div class="box">
				<div class="box-body">
				    <a href="form_returbeli.php" class="btn bg-maroon btn-flat margin">Create New</a>
				</div>
			    </div>
			</div>
		    </div>
...
//select data
    ...
    $sql_data		= mysql_query("SELECT * FROM `gx_returbeli` WHERE `level` =  '0' ORDER BY `id_returbeli` DESC LIMIT $start, $perhalaman;", $conn);
    $sql_total_data	= mysql_num_rows(mysql_query("SELECT * FROM `gx_returbeli` WHERE `level` =  '0';", $conn));
    $hal		= "?";
    $no = $start + 1;
	...
	$content .='<tr>
			<td>'.$no.'.</td>
			<td>'.$row_data["kode_returbeli"].'</td>
			<td>'.$row_data["kode_beli"].'</td>
			<td>'.$row_data["tanggal"].'</td>
			<td>'.$row_data["nama_supplier"].'</td>
			<td>'.$row_data["nama_shipping"].'</td>
			<td><a href="detail_returbeli?c='.$row_data["kode_returbeli"].'"><span class="label label-info">Detail</span></a>
			   <!-- <a href="form_order_beli?c='.$row_data["kode_returbeli"].'"><span class="label label-info">Edit</span></a>-->
			</td>
		</tr>';
	$no++;
    }
