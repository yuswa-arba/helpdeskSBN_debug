...
if(isset($_POST["save"]))
{
	...
	$sql_insert = "INSERT INTO `gx_kas_kecil` (`id_kaskecil`, `id_cabang`, `transaction_id`,
	...
	header("location: form_kkecil_detail.php?kk=".$transaction_id);
	
	/*echo "<script language='JavaScript'>
	    alert('Data telah disimpan');
	    window.location.href='form_kecil_detail.php?keluar=".$transaction_id."';
	    </script>";*/
    }else{
	echo "<script language='JavaScript'>
...
elseif(isset($_POST["update"]))
{
    ...
    $id_kaskecil 	= isset($_POST['id_kaskecil']) ? mysql_real_escape_string(trim($_POST['id_kaskecil'])) : '';
    $transaction_id		= isset($_POST['transaction_id']) ? mysql_real_escape_string(trim($_POST['transaction_id'])) : '';
    $tgl_transaction	= isset($_POST['tgl_transaction']) ? mysql_real_escape_string(trim($_POST['tgl_transaction'])) : '';
    $acc_cash			= isset($_POST['acc_cash']) ? mysql_real_escape_string(trim($_POST['acc_cash'])) : '';
    $mu			= isset($_POST['mu']) ? mysql_real_escape_string(trim($_POST['mu'])) : '';
    ...
    if($transaction_id != "" AND $id_kaskecil != ""){
	
	$sql_update = "UPDATE `gx_kas_kecil` SET `transaction_id`='".$transaction_id."',
	`tgl_transaction`='".$tgl_transaction."', `acc_cash`='".$acc_cash."', `mu`='".$mu."', `rate`='".$rate."',
	`remarks`='".$remarks."', 
	`date_upd` = NOW(), `user_upd`= '".$loggedin["username"]."'
	WHERE (`id_kaskecil`='".$id_kaskecil."');";
	...
	echo "<script language='JavaScript'>
	    ...
	    window.location.href='master_kaskecil.php';
	    </script>";
...
if(isset($_GET["id"]))
{
    ...
    $query_data = "SELECT * FROM `gx_kas_kecil` WHERE `id_kaskecil`='".$id_data."' LIMIT 0,1;";
...
$sql_last_data 	= mysql_num_rows(mysql_query("SELECT `id_kaskecil` FROM `gx_kas_kecil` WHERE `tgl_transaction` LIKE '%".date("Y-m-d")."%';", $conn));
                ...
                <section class="content">
					...
					<div class="row">
					    ...
					    <div class="col-xs-3">
							...
							<input type="hidden" name="id_kaskecil" value="'.(isset($_GET['id']) ? $row_data["id_kaskecil"] : "").'">
						</div>
					    <div class="col-xs-3">
						<label>Tanggal</label>
					    </div>
