...
if(isset($_POST["save"]))
{
    ...
    $sn				= isset($_POST['barcode']) ? mysql_real_escape_string(trim($_POST['barcode'])) : '';
    ...
    if($kode_barang != ""){
		...
		for($i=1;$i<=$qty;$i++)
		{
			...
			$sql_insert = "INSERT INTO `gx_beli_detail` (`id_beli_detail`, `kode_beli`, `kode_barang`, `nama_barang`, `qty`, `sn`, `macaddress`, `harga`, `kode_gudang`,
										`nama_gudang`, `shipping_cost`, `hpp`, `kode_lemari`, `lemari`, `check_in`, `user_add`, `user_upd`, `date_add`, `date_upd`, `level`)
						VALUES ('', '".$kode_beli."', '".$kode_barang."', '".$nama_barang."', '1', '".$sn.substr((date("Ymd")), 1 ).sprintf("%05d", $i)."', '".$macaddress."', '".$harga."', '".$kode_gudang."',
							'".$row_gudang["nama_gudang"]."', '".$shipping_cost."', '".$hpp."', '".$kode_lemari."', '".$row_lemari["nama_lemari"]."', '".$check_in."', 
...
elseif(isset($_POST["update"]) OR isset($_POST["updatelock"]))
{
    ...
    $sql_update = "UPDATE `gx_beli_detail` SET `kode_barang`='".$kode_barang."', `nama_barang`='".$nama_barang."',
	`kode_beli`='".$kode_beli."', `macaddress`='".$macaddress."',
	    ...
	    <td>
		    &nbsp;
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" readonly="" onclick="return valideopenerform(\'data_barangbeli.php?r=form_setujubeli_item&id='.(isset($_GET['c']) ? $row_data["kode_setuju_pembelian"] : "").'\',\'barang\');"
						class="form-control" required="" name="kode_barang" id="kode_barang" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["kode_barang"] : "").'">
					    </div>
                                        </div>
					</div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control" required="" name="barcode" id="barcode" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["sn"] : "").'" '.(isset($_GET['id']) ? 'readonly=""' : '').'>
					    </div>
                                        </div>
					</div>
					<div class="form-group">
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control" name="macaddress" id="macaddress" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["macaddress"] : "").'">
					    </div>
                                        </div>
					</div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control"  name="shipping_cost" id="shipping_cost" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["shipping_cost"] : "").'">
					    </div>
                                        </div>
					</div>
					<div class="form-group">
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control"  name="hpp" id="hpp" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["hpp"] : "").'">
					    </div>
                                        </div>
					</div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
						<input type="text" class="form-control"  name="check_in" id="check_in" value="'.(isset($_GET['id']) ? $row_setujubeli_detail["check_in"] : "").'">
					    </div>
                                        </div>
					</div>
					<div class="form-group">
					...
					<button type="submit" value="Submit" '.(isset($_GET['id']) ? 'name="update"' : 'name="save"').' class="btn btn-primary">Update</button>
                                    </div>
                                </form>
                            </div><!-- /.box -->
                        </section>
