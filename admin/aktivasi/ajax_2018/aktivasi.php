...
if(isset($_REQUEST["userid"]))
{
	...
	$update_onu = mysql_query("UPDATE `software`.`gx_master_onu` SET `status_onu`='1' WHERE (`userid_onu`='".$userid."');", $conn);
	// cek ip_vlan apa sudah ada, vlan recycle
	if($data_vlan_mac["ip_vlan"] == "")
	{
		...
		mysql_query("UPDATE `gx_data_vlan` SET `ip_vlan`='".trim($ip_baru)."', `user_upd`='".$loggedin["username"]."', `date_upd`=NOW()
					WHERE (`userid_vlan`='".$userid."');", $conn);

		$ip_default_vlan = $data_ip_vm['ipaddress_ipvm'];
	}
	else
	{
		$ip_baru = $data_vlan_mac["ip_vlan"];
		$ip_default_vlan = $ip_baru;
	}
	...
	if ($api_vm->connect($ip_baru, $data_ip_vm['username_ipvm'], $data_ip_vm['password_ipvm']))
	{
		...
		*/
		...
		if(count($output_dial) >= 1)
		{
			...
			$api_vm->write('/ip/address/print');
			$READ1 = $api_vm->read(false);
			$ARRAY1 = $api_vm->parseResponse($READ1);
			
			$output_ip = '<div style="margin-top:10px;">';
			foreach($ARRAY1 as $value)
			{
				if($value['interface'] == 'pppoe-out1')
				{
					$output_ip .= '<h3>IP Dial : '.$value['address'].'</h3>';
				}
	...
	else
	{
		...
		$output_ip = "Tidak Bisa Login ke Mesin VM";
											...
											<div class="col-xs-10">
												<div class="box box-solid box-info">
													...
													<div class="box-body">
														...
														Tipe Koneksi : '.$data_customer["cKdPaket"].'<br>
														'.$output_ip.'<br><br><br>
