...
else
{
    ...
    $status_bongkar = '<h3 class="box-title danger">Bongkar Gagal.</h3>';
	$output_ip_vlan = "-";
}
...
#-----------------------------
$olt_customer = mysql_query("SELECT * FROM `software`.`gx_inet_olt_customer` WHERE `userid` = '".$userid."' LIMIT 0,1;", $conn);
$data_olt_customer = mysql_fetch_array($olt_customer);

$remove_vlan_helpdesk = mysql_query("UPDATE `gx_data_vlan` SET `userid_vlan`='', `mac_address`='', `flag_vlan` = 'nonaktif',
								`user_upd`='".$loggedin["username"]."', `date_upd`=NOW()
								WHERE (`vlan`='".$vlan."');", $conn);
$remove_vlan_helpdesk = 'ok';

$remove_olt_customer = mysql_query("UPDATE `software`.`gx_inet_olt_customer` SET `userid`='', `mac_address`='' WHERE (`id_olt_customer`='".$data_olt_customer["id_olt_customer"]."');", $conn);
$remove_olt_customer = 'ok';

$remove_onu = mysql_query("UPDATE `software`.`gx_master_onu` SET `mac_onu`='' WHERE (`userid_onu`='".$userid."');", $conn);
$remove_onu = 'ok';
#-----------------------------
#	5.Remove IP Pool
#-----------------------------
