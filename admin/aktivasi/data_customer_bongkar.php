  ...
  $return_form	= isset($_GET['r']) ? mysql_real_escape_string(strip_tags(trim($_GET['r']))) : "";
    $content ='
        ...
        while ($row_execution = mysql_fetch_array($query_execution)) {
			...
			$sql_vlan = mysql_query("SELECT * FROM `gx_data_vlan` WHERE `userid_vlan` = '".$row_execution["cUserID"]."' LIMIT 0,1;", $conn);
			$row_vlan = mysql_fetch_array($sql_vlan);
			
			$sql_ip		= mysql_query("SELECT * FROM `gx_master_ip` WHERE `userid_ip` = '".$row_execution["cUserID"]."' LIMIT 0,1;", $conn);
			$row_ip 	= mysql_fetch_array($sql_ip);
			
			$sql_ipvm	= mysql_query("SELECT * FROM `gx_master_ipvm` WHERE `userid_ip` = '".$row_execution["cUserID"]."' AND `level` = '0' LIMIT 0,1;", $conn);
			$row_ipvm 	= mysql_fetch_array($sql_ipvm);
		...
		$content .='<tr>
							...
							$content .= '\''.mysql_real_escape_string(long2ip($row_ip["ip_address"])).'\',';
							$content .= '\''.mysql_real_escape_string(long2ip($row_ipvm["ip_address"])).'\')">Select</a>'; 
                            $content .= '</td>
                            </tr>';
                $no++;
            }
...
<!-- DataTable -->
 ...
 $plugins .= '<script type=\'text/javascript\'>
		      function validepopupform2(uid,  nama, ids, p, pid, mac, ckode, v, ip, ipv){
				  ...
				  window.opener.document.'.$return_form.'.ip_vlan.value=ipv;
			      self.close();
		      }
	      </script>
	      ';
