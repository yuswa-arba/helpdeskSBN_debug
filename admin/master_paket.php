...
if($loggedin["group"] == 'admin'){
    ...
    if(isset($_POST["hapus"]))
    {
        ...
        {
            $query = "UPDATE `gx_paket` SET `level` = '1', `user_upd` = '".$loggedin["username"]."', `date_upd` = NOW()
    ...
    $content ='<section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        <li><a href="'.URL_ADMIN.'home"><i class="fa fa-dashboard"></i> Home</a></li>
                        <li><a href="'.URL_ADMIN.'master_paket">Master Paket</a></li>
                    </ol>
                </section>
				   ...
				   <div class="box-tools pull-right">
					...
					<div class="btn bg-olive btn-flat margin">
						...
						<th>#</th>
                                        ...
                                        <tbody>';
$sql_masterPaket = mysql_query("SELECT `gx_paket`.*, `gx_tipe`.`nama_type`
				  FROM `gx_paket`, `gx_tipe`
				  WHERE `gx_paket`.`id_type` = `gx_tipe`.`id_type` AND
				  `gx_paket`.`level` = '0';", $conn);
$no = 1;
while ($row_masterPaket = mysql_fetch_array($sql_masterPaket))
{
     ...
     $content .= '<tr>
		    ...
		    <td>'.$row_masterPaket["nama_type"].'</td>
		    <td>'.(($row_masterPaket["id_cabang"] == "0") ? "Semua Cabang" : $row_cabang["nama_cabang"]).'</td>
		    <td>'.periodePaket($row_masterPaket["periode_paket"]).'</td>
		    <td>'.Rupiah($row_masterPaket["harga_paket"]).'</td>
		    <td align="center">
