...
if ($loggedin = logged_inAdmin()) { // Check if they are logged in
    ...
    if ($loggedin["group"] == 'admin') { // jika yang login adalah admin
        ...
        if (isset($_POST['search'])) {
            ...
            $sql_menu = mysql_query("SELECT * FROM `gx_menu` WHERE `level` = '0'
                $sql_nama_menu
                $sql_group_menu
                $sql_alias
                $sql_nama_url
            ...
            $count_menu = mysql_num_rows(mysql_query("SELECT * FROM `gx_menu` WHERE `level` = '0'
                $sql_nama_menu
                $sql_group_menu
                $sql_alias
                $sql_nama_url
            ...
            $sql_menu = mysql_query("SELECT * FROM `gx_menu` WHERE `level` = '0' ORDER BY `id_menu` ASC LIMIT $start, $perhalaman;", $conn);
        ...
        $content = '
            ...
            <section class="content">
                <div class="row">
                    <div class="col-xs-12">
                        <div class="box">
                            <form action="list_menu.php" method="post" name="form_search">
        ...
        while ($row_menu = mysql_fetch_array($sql_menu)) {
            $content .= '
                <tr>
                    ...
                    <td>' . $row_menu["file"] . '</td>
                    <td align="center">
                    <a href="form_menu.php?id_menu=' . $row_menu['id_menu'] . '"><span class="label label-info">Edit</span></a> |
                    <a href="" onclick="return valideopenerform(\'detail_menu.php?id=' . $row_menu["id_menu"] . '\',\'cabang\');"><span class="label label-info">Details</span></a></td>
