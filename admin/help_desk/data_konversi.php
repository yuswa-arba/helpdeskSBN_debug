 ...
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data Konversi");
    ...
    $content ='
				...
				<div class="box-header">
                                    <h2 class="box-title">Data koversi</h2>
                                </div>
...
if(isset($_POST["save_search"])){
	...
	$sql_customer	= "SELECT * FROM `gx_helpdesk_konversi`
	WHERE `kode_customer` LIKE '".$customer_number."%'
	$sql_nama
	$sql_userid
	ORDER BY `id_konversi` DESC LIMIT 0,10;";
  ...
  while ($row_customer = mysql_fetch_array($query_customer)) {
    $sql_tbcustomer = mysql_fetch_array(mysql_query("SELECT * FROM `tbCustomer` WHERE `cKode` = '".$row_customer["kode_customer"]."' ORDER BY `idCustomer` DESC", $conn));
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["kode_customer"]).'\', \''.mysql_real_escape_string($row_customer["nama_customer"]).'\', \''.mysql_real_escape_string($row_customer["uid"]).'\', \''.mysql_real_escape_string($sql_tbcustomer["ctelp"]).'\', \''.mysql_real_escape_string($row_customer["kode_paket_baru"]).'\', \''.mysql_real_escape_string($row_customer["nama_paket_baru"]).'\', \''.mysql_real_escape_string($sql_tbcustomer["cAlamat1"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
