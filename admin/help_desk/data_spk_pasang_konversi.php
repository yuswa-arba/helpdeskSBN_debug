...
if($loggedin["group"] == 'admin'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data Konversi");
    global $conn;
    ...
    $content ='
				...
				<div class="box-header">
                                    <h2 class="box-title">Data SPK Pasang koversi</h2>
                                </div>
...
if(isset($_POST["save_search"])){
	...
	$sql_customer	= "SELECT * FROM `gx_spk_pasang_konversi`
	WHERE `kode_customer` LIKE '".$customer_number."%'
	$sql_nama
	$sql_userid
	ORDER BY `id_spk_pasang_konversi` DESC LIMIT 0,10;";
...
$no = 1;
if(isset($_GET["f"])){
 if($_GET["f"] == "jawab_spk_pasang_konversi"){
  while ($row_customer = mysql_fetch_array($query_customer)) {
	...
	$data_spk_pasang_konversi_marketing = mysql_fetch_array(mysql_query("SELECT `cNama` FROM `tbPegawai` WHERE `id_employee`='$row_customer[id_marketing]'", $conn));
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["kode_spk_pasang_konversi"]).'\', \''.mysql_real_escape_string($row_customer["kode_customer"]).'\', \''.mysql_real_escape_string($row_customer["nama_customer"]).'\', \''.mysql_real_escape_string($row_customer["kode_paket"]).'\', \''.mysql_real_escape_string($row_customer["nama_paket"]).'\', \''.mysql_real_escape_string($row_customer["uid"]).'\', \''.mysql_real_escape_string($row_customer["telp"]).'\', \''.mysql_real_escape_string($row_customer["no_linkbudget"]).'\', \''.mysql_real_escape_string($row_customer["alamat"]).'\', \''.mysql_real_escape_string($row_customer["id_teknisi"]).'\', \''.mysql_real_escape_string($row_customer["id_marketing"]).'\', \''.mysql_real_escape_string($data_spk_pasang_konversi_teknisi["cNama"]).'\', \''.mysql_real_escape_string($data_spk_pasang_konversi_marketing["cNama"]).'\', \''.mysql_real_escape_string($row_customer["pekerjaan"]).'\')">Select</a>
		     </td>																																																																																																																																		
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(ckode_spk_pasang,ckode_customer,cnama_customer,ckode_paket,cnama_paket,cuid,ctelp,cnolink_budget,calamat,cid_teknisi,cid_marketing,cnama_teknisi,cnama_marketing,cpekerjaan){
		 window.opener.document.'.$return_form.'.kode_spk_pasang.value=ckode_spk_pasang;
		 ...
		 window.opener.document.'.$return_form.'.kode_linkbudget.value=cnolink_budget;
		 ...
		 window.opener.document.'.$return_form.'.pekerjaan.value=cpekerjaan;
