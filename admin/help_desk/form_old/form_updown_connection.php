...
if(isset($_GET["id"]))
{
    ...
    $content ='<section class="content-header">
                    <h1>
                        Form Upgrade Downgrade Connection
	...
	for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
	    ...
	    && in_array($file_extension, $validextensions)) {
			...
			// If file moved to uploads folder.
				$sql_insert_file = "INSERT INTO `gx_helpdesk_upgrade_downgrade_connection_upload` (`id_upload`, `kode_upgrade_downgrade_connection`, `nama_file`, `lokasi_file` ,`user_add`, `date_add`, `level`)
						VALUES ('', '".$kode_upgrade_downgrade."', '".$img_name."', '".$lokasi."', '".$loggedin["username"]."', NOW(), '0');";
...
}elseif(isset($_POST["update"]))
{
    ...
    $sql_update = "UPDATE `gx_helpdesk_upgrade_downgrade_connection` SET `level` = '1',
	...
	$sql_insert_update = "INSERT INTO `gx_helpdesk_upgrade_downgrade_connection` (`id_upgrade_downgrade_connection`, `kode_cabang`, `nama_cabang`,
						  `tanggal`, `kode_upgrade_downgrade`, `kode_customer`, `nama_customer`, `uid`, `kode_paket_lama`,
						  `nama_paket_lama`, `kode_paket_baru`, `nama_paket_baru`, `group_rbs`, `kode_formulir`,
						  `user_add`, `user_upd`, `date_add`, `date_upd`,  `level`) 
					  VALUES ('', '".$kode_cabang."', '".$nama_cabang."',
