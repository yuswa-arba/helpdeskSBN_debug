...
if(isset($_GET["id"]))
{
	...
	$data_spk_aktivasi_konversi_teknisi = mysql_fetch_array(mysql_query("SELECT `cNama` FROM `tbPegawai` WHERE `id_employee`='$row_spk_aktivasi_konversi[id_teknisi]'", $conn));
	$data_spk_aktivasi_konversi_marketing = mysql_fetch_array(mysql_query("SELECT `cNama` FROM `tbPegawai` WHERE `id_employee`='$row_spk_aktivasi_konversi[id_marketing]'", $conn));
                                ...
                                <form role="form" method="POST" name="myForm" action="" enctype="multipart/form-data" >
                                    <div class="box-body">
										...
										<div class="row">
											...
											<div class="col-xs-3">
												<input type="text" class="form-control" readonly="" id="kode_customer" name="kode_customer" required="" value="'.(isset($_GET['id']) ? $row_spk_aktivasi_konversi["kode_customer"] : "").'" onclick="return valideopenerform(\'data_konversi.php?r=myForm&f=spk_pasang_konversi\',\'customer\');" >
											</div>
											<div class="col-xs-3">
										...
										<div class="row">
											...
											<div  class="col-xs-9">
												<textarea name="alamat" class="form-control" placeholder="Alamat" style="resize: none;"> '.(isset($_GET["id"]) ? $row_spk_aktivasi_konversi['alamat'] :"").' </textarea>
											</div>
										</div>
										</div>
										...
										<div class="row">
											...
											<div class="col-xs-3">
												...
												<input class="form-control" readonly="" name="nama_teknisi" placeholder="Nama Teknisi" type="text" value="'.(isset($_GET["id"]) ? $data_spk_aktivasi_konversi_teknisi['cNama'] :"") .'" onclick="return valideopenerform(\'data_teknisi.php?r=myForm\',\'teknisi\');">
											...
											<div class="col-xs-3">
												...
												<input class="form-control" readonly="" name="nama_marketing" placeholder="Nama Marketing" type="text" value="'.(isset($_GET["id"]) ? $data_spk_aktivasi_konversi_marketing['cNama'] :"") .'" onclick="return valideopenerform(\'data_marketing.php?r=myForm\',\'marketing\');">
											</div>
										</div>
										</div>
...
if(isset($_POST["save"]))
{
    ...
    $kode_spk_aktivasi_konversi 			= isset($_POST['kode_spk_aktivasi_konversi']) ? mysql_real_escape_string(trim($_POST['kode_spk_aktivasi_konversi'])) : '';
	...
	if($kode_spk_aktivasi_konversi != "" && $kode_spk_pasang != ""){
    $sql_insert = "INSERT INTO `gx_spk_aktivasi_konversi` (`id_spk_aktivasi_konversi`, `kode_cabang`, `nama_cabang`,
						  `tanggal`, `kode_spk_aktivasi_konversi`, `kode_spk_pasang`, `kode_jawab_spk_pasang`, `kode_customer`, `nama_customer`, `no_linkbudget`, `kode_paket`,
						  `nama_paket`, `uid`, `telp`, `alamat`, `id_teknisi`, `id_marketing`, `pekerjaan`, 
						  `user_add`, `user_upd`, `date_add`, `date_upd`,  `level`) 
					  VALUES ('', '".$kode_cabang."', '".$nama_cabang."',
						  '".$tanggal."', '".$kode_spk_aktivasi_konversi."', '".$kode_spk_pasang."', '".$kode_spk_pasang."', '".$kode_customer."', '".$nama_customer."', '".$no_linkbudget."', '".$kode_paket."',
						  '".$nama_paket."', '".$uid."', '".$telp."', '".$alamat."', '".$id_teknisi."', '".$id_marketing."', 'AKTIVASI KONVERSI',
	...
	echo "<script language='JavaScript'>
			...
			window.location.href='".URL_ADMIN."help_desk/master_spk_aktivasi_konversi.php';
			</script>";
...
}elseif(isset($_POST["update"]))
{
    ...
    $sql_update = "UPDATE `gx_spk_aktivasi_konversi` SET `level` = '1',
		    `date_upd` = NOW(), `user_upd`= '".$loggedin["username"]."'
		    WHERE `id_spk_aktivasi_konversi` = '$_GET[id]';";
	...
	$sql_insert_update	= "INSERT INTO `gx_spk_aktivasi_konversi` (`id_spk_aktivasi_konversi`, `kode_cabang`, `nama_cabang`,
						  `tanggal`, `kode_spk_aktivasi_konversi`, `kode_spk_pasang`, `kode_jawab_spk_pasang`, `kode_customer`, `nama_customer`, `no_linkbudget`, `kode_paket`,
						  `nama_paket`, `uid`, `telp`, `alamat`, `id_teknisi`, `id_marketing`, `pekerjaan`, 
						  `user_add`, `user_upd`, `date_add`, `date_upd`,  `level`) 
					  VALUES ('', '".$kode_cabang."', '".$nama_cabang."',
