...
if($loggedin["group"] == 'admin'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data SPK Konversi");
    global $conn;
...
if(isset($_POST["save_search"])){
	...
	$sql_nama	= ($name != "") ? "AND `gx_spk_pasang_konversi`.`nama_customer` LIKE '".$name."%'": "";
	$sql_userid	= ($user_id != "") ? "AND `gx_spk_pasang_konversi`.`uid` LIKE '%".$user_id."%'": "";

	$sql_customer	= "SELECT `gx_jawab_spk_pasang_konversi`.* FROM `gx_spk_pasang_konversi` , `gx_jawab_spk_pasang_konversi`
							WHERE `gx_spk_pasang_konversi`.`kode_customer` LIKE '".$customer_number."%'
							$sql_nama
							$sql_userid
							AND `gx_spk_pasang_konversi`.`kode_spk_pasang_konversi` = `gx_jawab_spk_pasang_konversi`.`kode_spk_pasang`
							AND `gx_spk_pasang_konversi`.`level` = `gx_jawab_spk_pasang_konversi`.`level`
							AND  `gx_jawab_spk_pasang_konversi`.`level` = '0'
							ORDER BY `gx_spk_pasang_konversi`.`id_spk_pasang_konversi` DESC LIMIT 0,10;";
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(ckode_spk_pasang,ckode_jawab_spk_pasang,ckode_customer,cnama_customer,ckode_paket,cnama_paket,cuid,ctelp,cnolink_budget,calamat,cid_teknisi,cid_marketing,cnama_teknisi,cnama_marketing){
		 ...
		 window.opener.document.'.$return_form.'.kode_jawab_spk_pasang.value=ckode_jawab_spk_pasang;
