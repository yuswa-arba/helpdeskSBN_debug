...
if($loggedin["group"] == 'admin'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data SPK Aktivasi Konversi");
    global $conn;
    ...
    $content ='
				...
				<div class="box-header">
                                    <h2 class="box-title">Data SPK Aktivasi Konversi</h2>
                                </div>
...
if(isset($_POST["save_search"])){
	...
	$sql_nama	= ($name != "") ? "AND `gx_jawab_spk_aktivasi_konversi`.`nama_customer` LIKE '".$name."%'": "";
	$sql_userid	= ($user_id != "") ? "AND `gx_jawab_spk_aktivasi_konversi`.`uid` LIKE '%".$user_id."%'": "";
	$sql_address	= ($address != "") ? "AND `gx_jawab_spk_aktivasi_konversi`.`alamat` LIKE '%".$address."%'": "";
	$sql_phone	= ($phone != "") ? "AND `gx_jawab_spk_aktivasi_konversi`.`telp` LIKE '%".$phone."%'" : "";
	
	$sql_customer	= "SELECT `gx_jawab_spk_aktivasi_konversi`.* FROM `gx_spk_aktivasi_konversi`, `gx_jawab_spk_aktivasi_konversi`
	WHERE `gx_jawab_spk_aktivasi_konversi`.`kode_customer` LIKE '".$customer_number."%'
	AND `gx_jawab_spk_aktivasi_konversi`.`level` = `gx_spk_aktivasi_konversi`.`level`
	AND	`gx_spk_aktivasi_konversi`.`level` = '0'
	$sql_nama
	$sql_userid
	$sql_address
	$sql_phone
	ORDER BY `id_spk_aktivasi_konversi` DESC LIMIT 0,10;";
  ...
  while ($row_customer = mysql_fetch_array($query_customer)) {
	...
	$data_spk_pasang_konversi_marketing = mysql_fetch_array(mysql_query("SELECT `cNama` FROM `tbPegawai` WHERE `id_employee`='$row_customer[id_marketing]'", $conn));
	 ...
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["kode_customer"]).'\', \''.mysql_real_escape_string($row_customer["nama_customer"]).'\', \''.mysql_real_escape_string($row_customer["no_linkbudget"]).'\', \''.mysql_real_escape_string($row_customer["kode_paket"]).'\', \''.mysql_real_escape_string($row_customer["nama_paket"]).'\', \''.mysql_real_escape_string($row_customer["uid"]).'\', \''.mysql_real_escape_string($row_customer["telp"]).'\', \''.mysql_real_escape_string($row_customer["alamat"]).'\', \''.mysql_real_escape_string($row_customer["id_teknisi"]).'\', \''.mysql_real_escape_string($row_customer["id_marketing"]).'\',  \''.mysql_real_escape_string($data_spk_pasang_konversi_teknisi["cNama"]).'\', \''.mysql_real_escape_string($data_spk_pasang_konversi_marketing["cNama"]).'\', \''.number_format($row_paket["monthly_fee"],2).'\',\''.number_format($tagihanperhari,2).'\',\''.$total_hari_pengurangan.'\',\''.number_format($total_tagihan,2).'\',\''.number_format($ppn,2).'\',\''.number_format($totalppn,2).'\', \''.number_format($row_paket["monthly_fee"],2).'\',\''.number_format($tagihanperhari_baru,2).'\',\''.$sum_aktivasi_sd_akhirbulan.'\',\''.number_format($total_tagihan_baru,2).'\',\''.number_format($ppn_baru,2).'\',\''.number_format($totalppn_baru,2).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(customernumber, cnamacust, cno_linkbudget, ckodepaket, cnamapaket, cuid, ctelp, calamat, cid_teknisi, cid_marketing, cnama_teknisi, cnama_marketing, monthly_func,daily_func,aktivasi_func,total_func,ppn,totalppn, monthly_func_baru,daily_func_baru,aktivasi_func_baru,total_func_baru,ppn_baru,totalppn_baru){
