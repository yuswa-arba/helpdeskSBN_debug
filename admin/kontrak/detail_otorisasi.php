...
//UPDATE
    ...
    $redirect_update_data = "detail_otorisasi";
...
//String Data View
    ...
    $form_name = "detail_otorisasi";
    $form_id = "detail_otorisasi";
    $check_input_id = "kode_otorisasi";
    
    //id web
    $title_header = 'Master Otorisasi';
    $submenu_header = 'detail_otorisasi';
...
if(isset($_GET["c"]))
{
    ...
    $sql = "SELECT DISTINCT `kode_otorisasi` FROM `gx_otorisasi` WHERE `kode_otorisasi` ='".$kode."'";
	...
	$content .= '<tr>
	<td>'.$no.'.</td>
	<td>'.($sql_d_1["kode_pengedit"] != '' ? $sql_d_1["kode_pengedit"] : '-').'</td>
	<td>'.$sql_d_2["kode_pengedit"].'</td>
	<td>'.$sql_d_2["nama_pengedit"].'</td>
	<td>'.($sql_d_2['checklist'] == '1' ? 'Sudah' : 'Belum').'
	</td>
