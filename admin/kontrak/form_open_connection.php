...
//INSERT 
    ...
    $url_redirect_insert = "master_off_connection";
...
//UPDATE
    ...
    $redirect_update_data = "master_off_connection";
...
if(isset($_POST["save"]))
{
    ...
    /*
      SELECT `id`, `kode_cabang`, `nama_cabang`, `tanggal`, `kode_reaktivasi`, `kode_customer`, `nama_customer`, `user_id`, `no_inactive`, `status_inactive`, `remarks`, `kode_cso`, `request`, `foto_email`, `no_formulir`, `status`, `date_add`, `date_upd`, `level`, `user_add`, `user_upd` FROM `gx_off_connection` WHERE 1
     */
    ...
    $kode_off_connection				= isset($_POST['kode_off_connection']) ? mysql_real_escape_string(trim($_POST['kode_off_connection'])) : '';
  ...
  //  VALUES (NULL,'$kode_batal_posting','$nama_batal','$kode_batal','$alasan_batal','$data_asli','$data_baru',NOW(),NOW(),'0','".$loggedin['username']."','".$loggedin['username']."')";
    ...
    */
    ...
    $sql_insert = "INSERT INTO `gx_off_connection` (`id`, `kode_cabang`, `nama_cabang`, `tanggal`, `kode_off_connection`, `kode_customer`, `nama_customer`, `user_id`, `no_inactive`, `status_inactive`, `remarks`, `kode_cso`, `request`, `foto_email`, `no_formulir`, `status`, `date_add`, `date_upd`, `level`, `user_add`, `user_upd`)
    VALUES (NULL, '".$kode_cabang."', '".$nama_cabang."', NOW(), '".$kode_off_connection."', '".$kode_customer."', '".$nama_customer."', '".$user_id."', '".$no_inactive."', '".$status_inactive."', '".$remarks."', '".$kode_cso."', '".$request."', '".$foto_email."', '".$no_formulir."', '0', NOW(), NOW(), '0', '".$loggedin['username']."', '".$loggedin['username']."')";
    //echo $sql_insert."<br>";
	    ...
	    && in_array($file_extension, $validextensions)) {
			...
			// If file moved to uploads folder.
				...
				*/
				...
				$sql_insert_file = "INSERT INTO `gx_off_connection_image_email`(`id`, `tanggal`, `kode_off_connection`, `foto_email`, `status`, `date_add`, `date_upd`, `level`, `user_add`, `user_upd`)
				VALUES (NULL,NOW(),'".$kode_off_connection."','".$img_name."','0',NOW(),NOW(),'0','".$loggedin['username']."','".$loggedin['username']."')";
...
}elseif(isset($_POST["update"]))
{
    ...
    $nama_cabang					= isset($_POST['nama_cabang']) ? mysql_real_escape_string(trim($_POST['nama_cabang'])) : '';
    $tanggal						= isset($_POST['tanggal']) ? mysql_real_escape_string(trim($_POST['tanggal'])) : '';
    $kode_off_connection				= isset($_POST['kode_off_connecion']) ? mysql_real_escape_string(trim($_POST['kode_off_connection'])) : '';
    $kode_customer					= isset($_POST['kode_customer']) ? mysql_real_escape_string(trim($_POST['kode_customer'])) : '';
    $nama_customer					= isset($_POST['nama_customer']) ? mysql_real_escape_string(trim($_POST['nama_customer'])) : '';
    $user_id						= isset($_POST['user_id']) ? mysql_real_escape_string(trim($_POST['user_id'])) : '';
    $no_inactive					= isset($_POST['no_inactive']) ? mysql_real_escape_string(trim($_POST['no_inactive'])) : '';
	...
	$data_array_select 	= mysql_fetch_array(mysql_query("SELECT `id` FROM `gx_off_connection` WHERE `kode_off_connection`='".$c."' ORDER BY `id` DESC LIMIT 0,1", $conn));
	...
	*/
	...
	$sql_update = "UPDATE `gx_off_connection` SET `date_upd`=NOW(),`level`='1',`user_upd`='".$loggedin['username']."' WHERE  `kode_off_connection`='".$c."'";
	$sql_update_file = "UPDATE `gx_off_connection_image_email` SET `date_upd`=NOW(),`level`='1',`user_upd`='".$loggedin['username']."' WHERE `kode_off_connection`='".$c."'";
				
	
	
	$sql_insert = "INSERT INTO `gx_off_connection` (`id`, `kode_cabang`, `nama_cabang`, `tanggal`, `kode_off_connection`, `kode_customer`, `nama_customer`, `user_id`, `no_inactive`, `status_inactive`, `remarks`, `kode_cso`, `request`, `foto_email`, `no_formulir`, `status`, `date_add`, `date_upd`, `level`, `user_add`, `user_upd`)
	VALUES (NULL, '".$kode_cabang."', '".$nama_cabang."', NOW(), '".$c."', '".$kode_customer."', '".$nama_customer."', '".$user_id."', '".$no_inactive."', '".$status_inactive."', '".$remarks."', '".$kode_cso."', '".$request."', '".$foto_email."', '".$no_formulir."', '0', NOW(), NOW(), '0', '".$loggedin['username']."', '".$loggedin['username']."')";
	//echo $sql_insert."<br>";
    ...
    $content = '<section class="content-header">
                ...
                <section class="content">
				    ...
				    $content.=' <div class="col-xs-2">
					    ...
					    <div class="col-xs-4">
						<input type="text" readonly="" class="form-control" required="" name="nama_cabang" value="'.(isset($_GET['c']) ? $row["nama_cabang"] : '').'" onclick="return valideopenerform(\'data_cabang.php?r=form_off_connection&f=data_cabang\',\'cabang\');">
					...
					<div class="row">
					    <div class="col-xs-2">
						<label>Kode Off Connection</label>
					    </div>
					    <div class="col-xs-4">
						<input readonly="" type="text" class="form-control" required="" name="kode_off_connection" value="'.(isset($_GET['c']) ? $row["kode_off_connection"] : "OCN-".rand(0000000, 9999999)).'">
					    </div>
					    ...
					    <div class="col-xs-4">
						<input readonly="" type="text" class="form-control" required="" name="kode_customer" value="'.(isset($_GET['c']) ? $row["kode_customer"] : '').'" onclick="return valideopenerform(\'data_customer.php?r=form_off_connection&f=data_customer_off_connection\',\'customer\');">
					...
					<div class="row">
					    ...
					    <div class="col-xs-4">
						<input type="text"  class="form-control" name="request" value="'.(isset($_GET['c']) ? $row["request"] : '').'" onclick="return valideopenerform(\'data_cso.php?r=form_off_connection&f=data_cso\',\'CSO\');" readonly="">
						...
						<div class="col-xs-4">
							...
							';
								if(isset($_GET["id"])){
									$sql_data		= mysql_query("SELECT * FROM `gx_off_connection_image_email` WHERE `level` =  '0' AND `kode_off_connection` = '".$row_data["kode_off_connection"]."'  ORDER BY `id` DESC;", $conn);
					...
					<div class="row">
					    ...
					    <div class="col-xs-4">
						<input type="text" readonly="" class="form-control" name="no_formulir" value="'.(isset($_GET['c']) ? $row["no_formulir"] : '').'" onclick="return valideopenerform(\'data_formulir.php?r=form_off_connection&f=data_formulir\',\'formulir\');">
					    </div>
