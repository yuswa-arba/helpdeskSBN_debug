...
//INSERT 
    ...
    $url_redirect_insert = "detail_otorisasi";
...
if(isset($_GET["c"]))
{
    ...
    $sql_pengeluaran_ = "SELECT * FROM `gx_otorisasi_detail` WHERE `kode_otorisasi` ='".$kode."' LIMIT 0,1;";
    ...
    {
	$query_check_list = mysql_query("SELECT * FROM `gx_pengeluaran_barang_detail` WHERE `kode_barang`='$row_pengeluaran[kode_barang]' AND `kode_pengeluaran`='$row_pengeluaran[kode_pengeluaran]'", $conn);
	$num_rows_check_list = mysql_num_rows($query_check_list);
	$data_check_list = mysql_fetch_array($query_check_list);
	$data_level = $data_check_list['level'];
	...
	<td>'.$row_pengeluaran["quantity"].'</td>
	<td>'.$row_pengeluaran["nama_lemari"].'</td>
	<td>'.($num_rows_check_list > 0 ? ($data_level == '0' ? 'Sudah' : 'Dibatalkan') : 'Belum').'
	</td>
	
	</tr>';
	$no++;
	$total_qty = $total_qty + $row_pengeluaran["quantity"];
    }
	$lemari = '';
}else{
    $lemari = '';
