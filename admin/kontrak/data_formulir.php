  ...
  $return_form	= isset($_GET['r']) ? mysql_real_escape_string(strip_tags(trim($_GET['r']))) : "";
    $content ='<section class="content-header">
        ...
        if(isset($_POST["save_search"])){
                ...
                $sql_kode	= ($kode_formulir != "") ? "AND `kode_formulir` LIKE '".$kode_formulir."%'": "";
                $sql_nama	= ($nama_formulir != "") ? "AND `nama_formulir` LIKE '%".$nama_formulir."%'": "";
                
                $sql_execution	= "SELECT * FROM `gx_formulir`
                WHERE `level` = '0'
                $sql_kode
                $sql_nama
		...
		$sql_masterformulir_detail = mysql_query("SELECT * FROM `gx_formulir_detail` WHERE `level` = '0' AND `id_formulir` = '$row_execution[id_formulir]' ORDER BY `date_add` DESC LIMIT 0,1;",$conn);
		$row_masterformulir_detail = mysql_fetch_array($sql_masterformulir_detail);
                
		$content .='<tr>
                            <td>'.$row_execution["kode_formulir"].'</td>
                            <td>'.$row_execution["nama_formulir"].'</td>
                            <td>'.$row_masterformulir_detail["lokasi_file"].'</td>
                            <td>';
                            $content .= '<a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_execution["kode_formulir"]).'\')">Select</a>'; 
                            $content .= '</td>
                            </tr>';
                $no++;
            }
