...
//SQL 
$table_main = "gx_perpanjangan_kontrak";
$table_detail = "";
...
//INSERT 
    ...
    $url_redirect_insert = "master_perpanjangan_kontrak";
...
//UPDATE
    ...
    $redirect_update_data = "master_perpanjangan_kontrak";
...
//String Data View
    //Judul Form
    $judul_form = "Perpanjangan Kontrak";
    $header_form = "Data Perpanjangan Kontrak";
    $index_field_sql = "id";
    $unix_field_sql = "kode_perpanjangan_kontrak";
    $url_form_detail = "detail_perpanjangan_kontrak";
    $url_form_edit = "form_perpanjangan_kontrak";
    
    $form_name = "form_perpanjangan_kontrak";
    $form_id = "";
    
    //id web
    $title_header = 'Master Perpanjangan Kontrak';
    $submenu_header = 'master_perpanjangan_kontrak';
...
if(isset($_POST["save"]))
{
    ...
    $kode_perpanjangan_kontrak		= isset($_POST['kode_perpanjangan_kontrak']) ? mysql_real_escape_string(trim($_POST['kode_perpanjangan_kontrak'])) : '';
    ...
    $y_pk = $kode_perpanjangan_kontrak != '' ? $exp_periode_kontrak_mulai[0] : 0;
    $m_pk = $kode_perpanjangan_kontrak != '' ? $exp_periode_kontrak_mulai[1] : 0;
    $d_pk = $kode_perpanjangan_kontrak != '' ? $exp_periode_kontrak_mulai[2] : 0;
    ...
    if($kode_perpanjangan_kontrak != ""){
    //insert into cc_subscription_service
    //'".$loggedin["username"]."', '".$loggedin["username"]."', NOW(), NOW(), '0'
    $sql_insert = "INSERT INTO `gx_perpanjangan_kontrak`(`id`, `kode_perpanjangan_kontrak`, `kode_kontrak`, `kode_cabang`, `nama_cabang`, `kode_customer`, `nama_customer`, `lama_kontrak`, `periode_kontrak_mulai`, `periode_kontrak_akhir`, `tanggal`, `date_add`, `date_upd`, `level`, `user_add`, `user_upd`)
    VALUES (NULL,'$kode_perpanjangan_kontrak','$kode_kontrak','$kode_cabang','$nama_cabang','$kode_customer','$nama_customer','$lama_kontrak','$periode_kontrak_mulai','$periode_kontrak_akhir','$tanggal', NOW(),NOW(),'0','".$loggedin["username"]."','".$loggedin["username"]."')";
...
}elseif(isset($_POST["update"]))
{
    ...
    $id 				= isset($_GET['c']) ? mysql_real_escape_string(trim($_GET['c'])) : '';
	...
	$sql_update = "UPDATE `gx_perpanjangan_kontrak` SET `user_upd`='".$loggedin['username']."',`date_upd`=NOW(),`level`='1' WHERE `kode_perpanjangan_kontrak`='".$c."'";
...
if(isset($_GET["c"]))
{
    ...
    $query 	= "SELECT * FROM `".$table_main."` WHERE `".$unix_field_sql."`='".$c."' AND `level`='0' ORDER BY `id` DESC LIMIT 0,1"; 
    ...
    $content = '<section class="content-header">
                ...
                <section class="content">
					...
					$content.=' <div class="col-xs-2">
					    <label>No. Perpanjangan Kontrak</label>
					    </div>
					    <div class="col-xs-3">
						<input type="text" readonly="" class="form-control" required="" name="kode_perpanjangan_kontrak" value="'.(isset($_GET['c']) ? $row["kode_perpanjangan_kontrak"] : "PK-".rand(0000000, 9999999) ).'">
					    </div>';
					...
					<div class="row">					    	
					    ...
					    <div class="col-xs-3">
						<input type="text" readonly="" class="form-control" required="" name="kode_kontrak" value="'.(isset($_GET['c']) ? $row["kode_kontrak"] : "" ).'" onclick="return valideopenerform(\'data_kontrak.php?r=form_perpanjangan_kontrak&f=data_kontrak\',\'kontrak\');">
					    </div>					    
					    <div class="col-xs-2">
						<label>Cabang</label>
					    </div>
					    <div class="col-xs-3">
						<input type="text" class="form-control" readonly="" required="" name="nama_cabang" id="nama_cabang" value="'.(isset($_GET['c']) ? $row["nama_cabang"] : '').'" onclick="return valideopenerform(\'data_cabang.php?r=form_perpanjangan_kontrak&f=data_cabang\',\'cabang\');">
					...
					<div class="row">
					    ...
					    <div class="col-xs-3">
						<input type="text" class="form-control" readonly="" required="" name="kode_customer" id="kode_customer" value="'.(isset($_GET['c']) ? $row["kode_customer"] : '').'" onclick="return valideopenerform(\'data_customer.php?r=form_kontrak&f=customer\',\'customer\');">
					    </div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-3">
						<input type="text" class="form-control hasDatepicker" id="datepicker" required="" name="periode_kontrak_mulai" value="'.(isset($_GET['c']) ? $row["periode_kontrak_mulai"] : '').'">
					    </div>
						...
						$content .= '<!--<input type="text" class="form-control"  required="" name="periode_kontrak_akhir" value="'.(isset($_GET['c']) ? $row["periode_kontrak_akhir"] : '').'">-->
						'.(isset($_GET['c']) ? $row['periode_kontrak_akhir'] : '').'
						<input type="hidden" class="form-control"  required="" name="periode_kontrak_akhir" value="">
						';
