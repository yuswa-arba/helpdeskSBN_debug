 ...
 if($_GET["f"] == "aktivasi"){
  ...
  while ($row_customer = mysql_fetch_array($query_customer)) {
	 $content .='<tr>
		     ...
		     <td>'.$row_customer["cAlamat1"].'</td>
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["cKode"]).'\',\''.mysql_real_escape_string($row_customer["cNama"]).'\',\''.mysql_real_escape_string($row_customer["cBulanan"]).'\',\''.mysql_real_escape_string($row_customer["kontrak_awal"]).'\',\''.mysql_real_escape_string($row_customer["kontrak_akhir"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
  ...
  $plugins .= '
 <script type="text/javascript">
	 function validepopupform2(custNumber, cnama, cbulanan, kontrak_awal, kontrak_akhir){
		 
		 window.opener.document.'.$return_form.'.kode_customer.value=custNumber;
		 window.opener.document.'.$return_form.'.nama_customer.value=cnama;
		 window.opener.document.'.$return_form.'.lama_kontrak.value=cbulanan;
		 window.opener.document.'.$return_form.'.periode_kontrak_mulai.value=kontrak_awal;
		 window.opener.document.'.$return_form.'.periode_kontrak_akhir.value=kontrak_akhir;
		 self.close();
	 }
 </script>';
 }
