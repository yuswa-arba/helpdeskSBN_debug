...
$no = 1;
    ...
    while ($row_survey = mysql_fetch_array($query_survey)) {
     $data_marketing = mysql_fetch_array(mysql_query("SELECT * FROM `tbPegawai` WHERE `tbPegawai`.`id_employee` = '$row_survey[marketing]';", $conn));
	$content .='<tr>
                    <td>'.$no.'</td>
		    ...
		    <td>'.$row_survey["no_telp"].'</td>
                    <td>
                      <a href="" onclick="validepopupform2(\''.$row_survey["no_spk_survey"].'\',\''.$row_survey["nama_cust"].'\',\''.$row_survey["nama_perusahaan"].'\',\''.$row_survey["alamat"].'\',\''.$row_survey["kota"].'\',\''.$row_survey["no_telp"].'\',\''.$row_survey["no_hp_1"].'\',\''.$row_survey["email"].'\',\''.$row_survey["longitude"].'\',\''.$row_survey["latitude"].'\',\''.$row_survey["marketing"].'\',\''.$data_marketing["cNama"].'\')">Select</a>
                    </td>
                  </tr>';
	$no++;
    }
...
<script type="text/javascript">
	...
	function validepopupform2(cnosurvey, cnama, cperusahaan, calamat, ckota, cnotelp, chp1, cemail, clongitude, clatitude, cmarketing, cnamamarketing){
