...
if ($loggedin = logged_inAdmin()) { // Check if they are logged in
    ...
    if ($loggedin["group"] == 'admin') { // jika yang login adalah admin
        ...
        /** =========================================================================
         * akan mengeksekusi perintah save jika tombol yang diklik memiliki nama save
         * ======================================================================== */
        if (isset($_POST["save"])) {
            ...
            $propinsi = isset($_POST['propinsi']) ? mysql_real_escape_string(strip_tags(trim($_POST['propinsi']))) : '';
            //data internet
            ...
            //data_login
            $promosi_nama = isset($_POST['nama_bank']) ? mysql_real_escape_string(strip_tags(trim($_POST['nama_bank']))) : '';
            ...
            $id_vlan = isset($_POST['id_vlan']) ? mysql_real_escape_string(strip_tags(trim($_POST['id_vlan']))) : '';

            $query_cek_userid = "SELECT * FROM `tbCustomer` WHERE `cUserID` = '" . $cUserID . "';";
            $sql_userid = mysql_query($query_cek_userid, $conn);
            $row_userid = mysql_fetch_array($sql_userid);
            $total_userid = mysql_num_rows($sql_userid);

            if ($total_userid >= 1) {
                echo "<script language='JavaScript'>
                        alert('userid sudah ada.');
                      </script>";
            } else {
                if ($cKode != "" AND $cNama != "") {
                    ...
                    //DATA VLAN
                    if ($id_cabang == '7') {
                        $last_data_vlan = mysql_query("SELECT * FROM `gx_data_vlan_bpn` WHERE `level` = '0' AND (`userid_vlan` ='' OR `userid_vlan` IS NULL ) ORDER BY `vlan` ASC LIMIT 0,1;", $conn);
                        $row_data_vlan = mysql_fetch_array($last_data_vlan);
                    } elseif ($id_cabang == '9') {
                        $last_data_vlan = mysql_query("SELECT * FROM `gx_data_vlan_mlg` WHERE `level` = '0' AND (`userid_vlan` ='' OR `userid_vlan` IS NULL ) ORDER BY `vlan` ASC LIMIT 0,1;", $conn);
                        $row_data_vlan = mysql_fetch_array($last_data_vlan);
                    } elseif ($id_cabang == '18') {
                        $last_data_vlan = mysql_query("SELECT * FROM `gx_data_vlan_smd` WHERE `level` = '0' AND (`userid_vlan` ='' OR `userid_vlan` IS NULL ) ORDER BY `vlan` ASC LIMIT 0,1;", $conn);
                        $row_data_vlan = mysql_fetch_array($last_data_vlan);
                    } else {
                        $last_data_vlan = mysql_query("SELECT * FROM `gx_data_vlan` WHERE `VLAN` > '2745' AND `level` = '0' AND (`userid_vlan` ='' OR `userid_vlan` IS NULL ) ORDER BY `vlan` ASC LIMIT 0,1;", $conn);
                        $row_data_vlan = mysql_fetch_array($last_data_vlan);
                    }

                    $update_vlan = mysql_query("UPDATE `gx_data_vlan` SET `userid_vlan`='" . $cUserID . "', `mac_address`='" . $cMacAdd . "',
								   `flag_vlan`='aktif', `user_upd`='" . $loggedin["username"] . "',  `date_upd`=NOW()
								   WHERE (`id_vlan`='" . $row_data_vlan['id_vlan'] . "');", $conn);
                    ...
                    $sql_insert_customer = "INSERT INTO `tbCustomer` (`idCustomer`, `cKode`, `cNama`,
                                            ...
                                            `NASAttribute`, `ddns`, `tipe_koneksi`, `propinsi`,
                                            `nama_bank`,
                                            ...
                                            '" . $cKdPaket . "', '" . $cIncludePPN . "', '" . $cExcludePPN . "', '" . $no_npwp . "', '" . $nama_npwp . "',
                                            '" . $alamat_npwp . "', '" . $cNoRekVirtual . "', '" . $cSales . "', '" . $cPaket . "',
                                            '" . date("Y-m-d H:i:s", strtotime($dTglMulai)) . "', '" . date("Y-m-d H:i:s", strtotime($dTglBerhenti)) . "', '" . $id_acc_justifikasi . "', '" . $cNonAktiv . "',
                                            '" . $ip_ap . "', '" . $ip_wl . "', '" . $cLati . "', '" . $cLongi . "',
                                            '" . $ip_bts . "', '" . $cMacAdd . "', '" . $bts . "', '" . $acc_type_rbs . "',
        ...
        /** ===========================================================================
         * akan mengeksekusi perintah save jika tombol yang diklik memiliki nama update
         * ========================================================================== */
        elseif (isset($_POST["update"])) {
            ...
            if ($idCustomer != "" AND $cKode != "") {
                ...
                $sql_update_customer = "UPDATE `tbCustomer` SET `cNama` = '" . $cNama . "',
                                        ...
                                        `nama_bank` = '" . $promosi_nama . "',
                                        `alamat_npwp` = '" . $alamat_npwp . "', `cNoRekVirtual` = '" . $cNoRekVirtual . "', `cSales` = '" . $cSales . "', `cPaket` = '" . $cPaket . "',
                                        `dTglMulai` = '" . date("Y-m-d H:i:s", strtotime($dTglMulai)) . "', `dTglBerhenti` = '" . date("Y-m-d H:i:s", strtotime($dTglBerhenti)) . "', `id_acc_justifikasi` = '" . $id_acc_justifikasi . "',
                                        `cNonAktiv` = '" . $cNonAktiv . "', `propinsi` = '" . $propinsi . "', 
                ...
                enableLog("", $loggedin["username"], $loggedin["id_employee"], $sql_update_customer); // for create log

                $sql_rbs_user = "UPDATE [dbo].[Users] SET [UserID]='" . $cUserID . "', [Password]='" . $cPassword . "',
                                [GroupName]='" . $cGroupRBS . "', [AccountIndex]='" . $acc_type_rbs . "'
                                WHERE ([UserIndex]='" . $iuserIndex . "');";
                $query_rbs_user = $conn_soft->prepare($sql_rbs_user);
                $query_rbs_user->execute();

                if ($username != "" AND $password != "" AND $cKode != "") {
                    //update data login
                    $sql_update_login = "UPDATE `gxLogin` SET  `username`='" . $username . "',
                                        `password`= MD5('" . $password . "'), `password_date`=NOW()
                                        WHERE `customer_number`='" . $cKode . "';";
                    mysql_query($sql_update_login, $conn) or die (mysql_error());

                    enableLog("", $loggedin["username"], $loggedin["id_employee"], $sql_update_login); // for create log
                }

                if ($cKode != "" AND $id_dutapromosi != "") {
                    //update data promosi
                    $sql_update_promosi = "UPDATE `gx_duta_promosi` SET `kode_customer`='" . $cKode . "', `nama`='" . $promosi_nama . "',
                    ...
                    enableLog("", $loggedin["username"], $loggedin["id_employee"], $sql_update_promosi); // for create log
        ...
        /** ========================================================================
         * menjalankan query jika terdapat id pada url / id tidak sama dengan kosong
         * ======================================================================= */
        if (isset($_GET["id"])) {
            $cKode = isset($_GET['id']) ? strip_tags(trim($_GET['id'])) : '';
            $query_customer = "SELECT * FROM `tbCustomer` WHERE `cKode` LIKE '%" . $cKode . "%' AND `id_cabang` = '" . $loggedin['cabang'] . "' AND `level` = '0' LIMIT 0,1;";
            $sql_customer = mysql_query($query_customer, $conn);
            $row_customer = mysql_fetch_array($sql_customer);

            $query_vlan = "SELECT * FROM `gx_data_vlan` WHERE `userid_vlan`='" . $row_customer['cUserID'] . "' AND `level` = '0' LIMIT 0,1;";
            $sql_vlan = mysql_query($query_vlan, $conn);
                                        ...
                                        <div class="tab-pane active" id="tab_1">
                                            <div class="form-group">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-3">
                                                        <input type="text" class="form-control" required="" id="cKode" name="cKode" value="' . (isset($_GET['id']) ? $row_customer["cKode"] : '') . '" >
                                            ...
                                            <div class="form-group">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-3">
                                                        <select class="form-control" name="tipe_koneksi" id="tipe_koneksi">
                                                            <option value="wireless" ' . ((isset($_GET["id"]) && ($row_customer["tipe_koneksi"] == "wireless")) ? 'selected="selected"' : "") . '>Wifi</option>
                                                            <option value="fo" ' . ((isset($_GET["id"]) && ($row_customer["tipe_koneksi"] == "fo")) ? 'selected="selected"' : "") . '>FO</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-3">
                                                        <select name="propinsi" class="propinsi form-control">
                                                            <option value="">--Pilih Provinsi--</option>';

        $data_kota = mysql_query("SELECT DISTINCT(`propinsi`) FROM `gx_master_kota`
                                 WHERE `kabupaten` = 'Malang'
                                 OR `kabupaten` = 'Balikpapan'
                                 OR `propinsi` = 'Bali'
                                 OR `kabupaten` = 'Samarinda'
                                 OR `kabupaten` = 'Manado'
                                 ORDER BY `propinsi` ASC;", $conn);

        while ($row_kota = mysql_fetch_array($data_kota)) {
            if (isset($_GET['id'])) {
                $content .= '<option value="' . $row_kota["propinsi"] . '" ' . (($row_kota["propinsi"] == $row_customer['propinsi']) ? 'selected=""' : "") . '>' . $row_kota["propinsi"] . '</option>';
            } else {
                $content .= '<option value="' . $row_kota["propinsi"] . '">' . $row_kota["propinsi"] . '</option>';
            }
        }
        ...
        $content .= '
                                                        ...
                                                        <div class="col-xs-3">
                                                        <input type="hidden" class="form-control " name="" value="' . (isset($_GET['id']) ? $row_customer["cKota"] : "") . '">
                                                            <select name="cKota" class="cKota form-control">
                                                                <option value="">--Pilih Kota--</option>';

        $data_kota = mysql_query("SELECT DISTINCT(`kabupaten`) FROM `gx_master_kota` ORDER BY `kabupaten` ASC;", $conn);
        while ($row_kota = mysql_fetch_array($data_kota)) {
            if (isset($_GET['id'])) {
                $content .= '<option value="' . $row_kota["kabupaten"] . '" ' . (($row_customer["cKota"] == $row_kota["kabupaten"]) ? 'selected=""' : "") . '>' . $row_kota["kabupaten"] . '</option>';
            } else {
                $content .= '<option value="' . $row_kota["kabupaten"] . '">' . $row_kota["kabupaten"] . '</option>';
            }
        }
        ...
        $content .= '
                                                ...
                                                <div class="form-group">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <select name="cDaerah" class="cDaerah form-control">
                                                                <option value="">Pilih Kecamatan</option>
                                                                ' . (isset($_GET['id']) ? '<option selected="selected" value="' . $row_customer["cDaerah"] . '">' . $row_customer["cDaerah"] . '</option>' : '') . '
                                                            </select>
                                                            <input type="hidden" class="form-control"  name="" value="' . (isset($_GET['id']) ? $row_customer["cDaerah"] : "") . '">
                                                        </div>
                                                        <div class="col-xs-3">
                                                            <label>Kelurahan</label>
                                                        </div>
                                                        <div class="col-xs-3">
                                                            <select name="cArea" class="cArea form-control">
                                                                <option value="">Pilih Kelurahan</option>
                                                                ' . (isset($_GET['id']) ? '<option selected="selected" value="' . $row_customer["cArea"] . '">' . $row_customer["cArea"] . '</option>' : '') . '
                                                            </select>
                                                            <input type="hidden" class="form-control"  name="" value="' . (isset($_GET['id']) ? $row_customer["cArea"] : "") . '">
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <input type="text" class="form-control" required="" maxlength="15" id="cUserID" name="cUserID" value="' . (isset($_GET['id']) ? $row_customer["cUserID"] : "") . '">
                                                        </div>
                                                        <div class="col-xs-3">
                                                            <div id="status_userid"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <input type="text" class="form-control" required="" minlength="4" maxlength="20" name="cPassword" id="cPassword" value="' . (isset($_GET['id']) ? $row_customer["cPassword"] : "") . '">
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <input type="text" class="form-control" name="nterm" value="' . (isset($_GET['id']) ? $row_customer["nterm"] : "3") . '">
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <input type="text" class="form-control" name="nama_bank" value="' . (isset($_GET['id']) ? $row_customer["nama_bank"] : "") . '"
                                                            onclick="return valideopenerform(\'data_bank.php?r=customer&f=nama_bank\',\'nama_bank\');">
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-xs-3">
                                                            <select class="form-control required"  name="cNonAktiv" id="cNonAktiv">
                                                                ...
                                                                <option value="2" ' . ((isset($_GET["id"]) && $row_customer["cNonAktiv"] == "2") ? 'selected=""' : 'selected=""') . '>Proses</option>
        ...
        for ($no_lokasi = 1; $no_lokasi <= 5; $no_lokasi++) {
            $content .= '
						<div class="form-group">
                            <div class="row">
                                <div class="col-xs-4">
                                    <label>Lokasi ' . $no_lokasi . '</label>
                                </div>
                                <div class="col-xs-4">
                                    <input type="file" name="lokasi[]" id="lokasi[]" />
                                </div>
                                <div class="col-xs-4">
                                                ...
                                                <div class="tab-pane" id="tab_login">
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-8">
                                                                <input type="text" class="form-control" required="" id="username" name="username" value="' . (isset($_GET['id']) ? $row_login["username"] : "") . '">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-8">
                                                                <input type="text" class="form-control" ' . (isset($_GET['id']) ? "" : 'required=""') . ' name="pass" id="pass" value="">
                                                            </div>
                                                        </div>
                                                     </div>
                                                     <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-8">
                                                                <input type="text" class="form-control" ' . (isset($_GET['id']) ? "" : 'required=""') . ' id="repass" name="repass" value="">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div><!-- /.tab-pane -->
                                                ...
                                                <div class="tab-pane" id="tab_inet">
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control"  name="ip_ap" id="ip_ap" value="' . (isset($_GET['id']) ? $row_customer["ip_ap"] : "") . '">
                                                            </div>
                                                            <div class="col-xs-2">
                                                                <label>IP WL</label>
                                                            </div>
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control"  name="ip_wl" id="ip_wl" value="' . (isset($_GET['id']) ? $row_customer["ip_wl"] : "") . '">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control"  name="bts" id="bts" value="' . (isset($_GET['id']) ? $row_customer["bts"] : "") . '">
                                                            </div>
                                                            <div class="col-xs-2">
                                                                <label>IP BTS</label>
                                                            </div>
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control"  name="ip_bts" id="ip_bts" value="' . (isset($_GET['id']) ? $row_customer["ip_bts"] : "") . '">
                                                            </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control" readonly name="acc_type_rbs" value="' . (isset($_GET['id']) ? $row_customer["acc_type_rbs"] : "") . '">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control" readonly="" name="user_ip" id="user_ip"  value="' . (isset($_GET['id']) ? $row_customer["user_ip"] : "") . '">
                                                            </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control"  readonly="" name="tipe_ip" id="tipe_ip" value="">
                                                            </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control" readonly name="cGroupRBS" value="' . (isset($_GET['id']) ? $row_customer["cGroupRBS"] : "") . '">
                                                                <input type="hidden" class="form-control" readonly name="service_name" value="' . (isset($_GET['id']) ? $row_customer["service_name"] : "") . '">
                                                            </div>
                                                            <div class="col-xs-2">
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                <input type="text" class="form-control" readonly name="vlan" value="' . (isset($_GET['id']) ? $row_vlan["vlan"] : '') . '">
                                                                <input type="hidden" class="form-control" readonly name="id_vlan" value="' . (isset($_GET['id']) ? $row_vlan["id_vlan"] : '') . '">
                                                            </div>
                                                    ...
                                                    <div class="form-group">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-xs-3">
                                                                ...
                                                                onclick="return valideopenerform(\'data_blokir.php?r=customer&f=custipblokir\',\'custipblokir\');">
                                                            </div>
                                                        </div>
                                                    </div>
        ...
        $plugins = '<script type="text/javascript">
                    ...
                    $(document).ready(function()
                    {
                        ...
                        $("#cUserID").keyup(function () 
                        {
                            ...
                            ({
                                type: "GET",
                                url: "select/cek_userid.php",
                                data: dataString,
                                cache: false,
                                success: function(html)
                                {
                                   $("#status_userid").html(html);
                                } 
                             });
                        });
                     ...
                     if(id == "wireless")
                     {
                        $(\'#ip_ap\').attr(\'readonly\', true);
                        $(\'#ip_bts\').attr(\'readonly\', true);
                        $(\'#ip_wl\').attr(\'readonly\', true);
                        $(\'#bts\').attr(\'readonly\', true);
                        
                     } else if(id == "fo"){
                        $(\'#ip_ap\').attr(\'readonly\', false);
                        $(\'#ip_bts\').attr(\'readonly\', false);
                        $(\'#ip_wl\').attr(\'readonly\', false);
                        $(\'#bts\').attr(\'readonly\', false);
                     }
                    
                      });
                      
                     $(".cKota").change(function()
                     {
                      var id=$(this).val();
                      var dataString = \'id=\'+ id;
                      ...
                      ({
                       type: "GET",
                       url: "select/fetch_kecamatan.php",
                       data: dataString,
                       cache: false,
                       success: function(html)
                       {
                     ...
                     $(".cDaerah").change(function()
                     {
                      ...
                      ({
                       type: "GET",
                       url: "select/fetch_kelurahan.php",
                       data: dataString,
                       cache: false,
                       success: function(html)
                       {
