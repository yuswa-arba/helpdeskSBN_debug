...
if($loggedin["group"] == 'admin'){
     enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Form Compose Email ");
    global $conn;
...
$sql_email_setting   	= mysql_fetch_array(mysql_query("select `reply_from` FROM `gx_email_setting` WHERE `level` = '0' LIMIT 0,1", $conn));
...
if($send == "Send"){
         ...
         $from		= $sql_email_setting["reply_from"];
...
try {
   ...
   $mail->Send();
    ...
    $sql = "INSERT INTO `gx_sent_mail`(`id_sentmail`,`id_email`,`from`, `to`, `subject`, `message`, `date_add`, `date_upd`, `level`)
                     VALUES('','$id_email','$from','$email_to','$subject','$message',NOW(),NOW(),'0')";
