    ...
    if ($loggedin["group"] == 'admin') {
        ...
        if ($type == "troubleticket") {
            ...
            while ($r_spktech = mysql_fetch_array($sql_spktech)) {
                $sql_teknisi = mysql_query("SELECT  `gx_pegawai`.`nama`
                    FROM `gx_pegawai` 
                    WHERE `gx_pegawai`.`id_employee` = '" . $r_spktech["id_teknisi"] . "'
                    AND `gx_pegawai`.`level` = '0' LIMIT 0,1;", $conn);
                ...
                $content .= '
                    <tr>
                        ...
                        <td>' . (($r_spktech["id_teknisi"] == "0") ? "-" : $row_teknisi["nama"]) . '</td>
                        ...
                        <td align="center"><a href="detail_spk.php?id_spk=' . $r_spktech["id_spk"] . '" onclick="return valideopenerform(\'detail_spk.php?id_spk=' . $r_spktech["id_spk"] . '\',\'spk\');">View</a> ||
                        <a href="jawab_spk.php?id_spk=' . $r_spktech["id_spk"] . '">Jawab SPK</a> || 
                        <a href="pdf4_.php?id_spk=' . $r_spktech["id_spk"] . '" target="_blank">PDF</a></td>
            ...
            $sql_spkmkt = mysql_query("SELECT  `gx_helpdesk_spk` . * ,  `gx_pegawai`.`nama`, `gx_helpdesk_complaint`.`name` ,  `gx_helpdesk_complaint`.`note_nclient` ,  `gx_helpdesk_complaint`.`address` ,  `gx_helpdesk_complaint`.`email` ,  `gx_helpdesk_complaint`.`phone` 
			    ...
			    AND  `gx_helpdesk_spk`.`id_marketing` =  `gx_pegawai`.`id_employee`
            ...
            while ($r_complaint = mysql_fetch_array($sql_complaint)) {
                ...
                $content .= '
                    <tr>
                        <td>' . $no . '.</td>
                        <td>
                        <a href="detail.php?id=' . $r_complaint["id_complaint"] . '" onclick="return valideopenerform(\'detail.php?id=' . $r_complaint["id_complaint"] . '\',\'detail' . $r_complaint["id_complaint"] . '\');">' . $r_complaint["cust_number"] . '</a>
                        </td>
