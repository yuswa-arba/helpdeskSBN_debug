...
if ($loggedin = logged_inAdmin()) { // Check if they are logged in
    ...
    if ($loggedin["group"] == 'admin') { // jika yang login adalah admin
        ...
        enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Detail complaint"); // for create log in newfunction2.php

        global $conn;

        if (isset($_GET['complaint_id'])) {
            $id_mailbox = isset($_GET['complaint_id']) ? mysql_real_escape_string(strip_tags(trim($_GET['complaint_id']))) : ""; // menambil nilai/string yang ada pada url

            $sql_detail = mysql_query("SELECT * FROM `gx_email` WHERE `ID` = '$id_mailbox' LIMIT 0,1;", $conn);
            $row_detail = mysql_fetch_array($sql_detail);
            $user_detail = $row_detail["EmailFrom"];
            $user_email = $row_detail["EmailFrom"];

            $sql_user = mysql_query("SELECT * FROM `tbCustomer` WHERE `cKode` = '" . $row_detail["customer_number"] . "' AND `cNonAktiv` = '0' AND `level` = '0' LIMIT 0,1;", $conn);
            $row_user = mysql_fetch_array($sql_user);
            $user_id = $row_user["cUserID"];
            $user_index = $row_user["iuserIndex"];
        } elseif (isset($_GET['id'])) {
            $id_complaint = isset($_GET['id']) ? mysql_real_escape_string(strip_tags(trim($_GET['id']))) : ""; // menambil nilai/string yang ada pada url

            $sql_detail = mysql_query("SELECT * FROM `gx_helpdesk_complaint` WHERE `id_complaint` = '" . $id_complaint . "' LIMIT 0,1;", $conn);
            $row_detail = mysql_fetch_array($sql_detail);
		        ...
		        <div class="row">
			        <div class="col-xs-12">
			            <ul class="nav nav-tabs" role="tablist" id="myTab">
                            ...
                            <li role="presentation"><a href="#status" aria-controls="status" role="tab" data-toggle="tab">History Status</a></li>
			            ...
			            <div class="tab-content">
                                    ...
                                    <div class="box-body">
                                        <table style="margin-bottom: 0;width:60%">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>Status</td>
                                                    <td>' . $row_user["cStatus"] . '</td>
                                                </tr>
                                                <tr>
                                                    <td>Tanggal Kontrak</td>
                                                    <td>' . date("d-m-Y", strtotime($row_user["dTglMulai"])) . ' s/d ' . date("d-m-Y", strtotime($row_user["dTglBerhenti"])) . '</td>
                                                </tr>
                                                <tr>
                                                    <td>Paket RBS</td>
                                                    <td>' . $row_user["cKdPaket"] . '</td>
                                                </tr>
                                                <tr>
                                                    <td>Keterangan</td>
                                                    <td>' . nl2br($row_user["cKet"]) . '</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                            ...
                            <div role="tabpanel" class="tab-pane" id="status">
                                <div class="box">
                                    <div class="box-header">
                                        <h2 class="box-title">History Status</h2>
                                    </div>
        ...
        if ($row_user["cKode"] != "") {
            $sql_history_complaint = "SELECT * FROM `gx_helpdesk_complaint`
			    WHERE `cust_number` = '" . $row_user["cKode"] . "' ORDER BY `date_add` DESC;";
        ...
        if ($row_user["cKode"] != "") {
            $sql_history_email = "SELECT * FROM `gx_email` WHERE `EmailFrom` LIKE '%" . $user_email . "%' ORDER BY `date_add` DESC; ";
        ...
        $sql_billing_transaksi = $conn_soft->prepare("SELECT *
			...
			FROM [dbo].[Users], [dbo].[BillingTransactions]
