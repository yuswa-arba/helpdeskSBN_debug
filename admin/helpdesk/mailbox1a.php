...
$q = isset($_GET["type"]) ? mysql_real_escape_string($_GET["type"]) : "";
	...
	$sql_email 	= mysql_query("SELECT * FROM `gx_email` WHERE `EmailFrom` != '' AND `id_kategori` = '".$kategori."' ORDER BY `DateE` DESC",$conn);
	
    }else{
	$sql_email	= mysql_query("SELECT * FROM `gx_email` WHERE `EmailFrom` != '' ORDER BY `DateE` DESC",$conn);
    }
