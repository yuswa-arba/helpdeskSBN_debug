...
if($loggedin["group"] == 'admin'){
 ...
 //echo date("Y-m-d H:i:s");
    $id_mailbox	= isset($_GET['id_complaint']) ? mysql_real_escape_string(strip_tags(trim($_GET['id_complaint']))) : "";
    ...
    $sql_customer = mysql_query("SELECT * FROM `gx_helpdesk_complaint`
	...
	<div id="form_problem" '.(($email["id_kategori"] == "2" || $email["id_kategori"] == "3") ? '' : 'style="display:none;"').' >
		...
		<div class="table-container table-form">
			...
			<td width="37.5%">
			  ...
			  <input class="form-control required" name="user_id" id="user_id" placeholder="User ID" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["user_id"] :"").'" >
			</td>
		      </tr>
		      <tr>
			<td>
			...
			<td>
			  <input class="form-control" name="customer_number" id="customer_number" placeholder="Customer Number" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["cust_number"] :"").'" >
			</td>
		      </tr>
		      <tr>
			<td>
			...
			<td>
			  <input class="form-control" name="name" placeholder="Name" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["name"] :"").'" >
			</td>
		      </tr>
			...
			<td>
			  <textarea name="address" rows="3" cols="70" class="" style="resize: none;">'.(($email["customer_number"]!= "") ? $dcustomer["address"] :"").'</textarea>
			</td>
		      </tr>
		      <tr>
			<td>
			...
			<td>
			  <input class="form-control" name="phone" placeholder="Phone" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["phone"] :"").'" >
			</td>
		      </tr>
		      <tr>
			<td>
			...
			<td>
			  <input class="form-control" name="email" placeholder="Email" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["email"] :"").'" >
			</td>
		      </tr>
		      <!--<tr>
			<td>
			...
			<tr>
			    ...
			    <td>
				<input class="form-control" name="media" placeholder="Email" type="text" value="'.(($email["customer_number"]!= "") ? $dcustomer["media"] :"").'" readonly="">
			    </td>
		      </tr>
			...
			<td>
			  <textarea class="required" name="problem" rows="3" cols="70" style="resize: none;">'.$dcustomer["problem"].' </textarea>
			</td>
		      </tr>
		      <tr style="vertical-align: middle;">
			<td style="vertical-align: middle;">
			...
			<td>
			  <input class="required" type="radio" id="spk" name="spk" value="spk" style="float:left;"> SPK
			  <input class="required" type="radio" id="nonspk" name="spk" value="nonspk" style="float:left;"> Non SPK
			</td>
		      </tr>
...
$submit = isset($_POST["update"]) ? $_POST["update"] : "";
	...
	if($submit == "Save"){
		...
		$insert_complaint    	= "UPDATE `gx_helpdesk_complaint` SET `user_id` = '$user_id', `cust_number` = '$cust_number',
						      `name` = '$name', `email` = '$email', `address` = '$address', `phone` = '$phone',
						      `media` = '$media', `note_` = '$note_', `type_client` = 'client', `problem` = '$problem',
						      `spk` = '$spk', `solusi` = '$solusi', `status` = '$status', `action` = '$action',
						      `which_side` = '$which_side', `complaint_type` = '$complaint_type',`updated_by` = '$loggedin[username]',
						      `date_upd` = NOW(), `level` = '0' WHERE `id_email`='".$ID."'";
