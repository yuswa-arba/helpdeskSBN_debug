                ...
                <section class="content">
				    ...
				    <table id="brosur" class="table table-bordered table-striped">
						...
						<th>#</th>
                                        ...
                                        <tbody>';
$sql_masterreproposal = mysql_query("SELECT * FROM `gx_reproposal` WHERE `level` = '0' ORDER BY `id_reproposal` DESC LIMIT $start,$perhalaman;",$conn);
$sql_total_masterreproposal = mysql_num_rows(mysql_query("SELECT * FROM `gx_reproposal` WHERE `level` = '0' ORDER BY `id_reproposal` DESC;",$conn));
$hal		    = "?";
$no = 1;
while ($row_masterreproposal = mysql_fetch_array($sql_masterreproposal))
{
	 $content .= '<tr>
		    ...
		    <td align="center">
				<a href="form_reproposal?id='.$row_masterreproposal['id_reproposal'].'"><span class="label label-info">Edit</span></a>
		    </td>
		</tr>';
		$no++;
}
