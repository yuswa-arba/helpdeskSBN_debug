...
if(isset($_POST["search"])){
	...
	if($kategori == "1")
	{
		...
		While($row_olt = mysql_fetch_array($sql_server_ras))
		{
			...
			$fh = fopen('/var/www/sbn/beta/admin/data/olt/list_inactive'.$noa.'.exp', 'w');
			fwrite($fh, $script);
			fclose($fh);
			chmod("/var/www/sbn/beta/admin/data/olt/list_inactive$noa.exp",0755);
			...
			foreach($lines as $key)
			{
				...
				if (stripos($first_string[0], 'EPON') !== false)
				{
					...
					if(trim($LastDeregReason) == "wire-down" || trim($LastDeregReason) == "wire down")
					{
	...
	fwrite($fh, $script);
	fclose($fh);
	chmod("/var/www/sbn/beta/admin/data/olt/list_inactive.exp",0755);
