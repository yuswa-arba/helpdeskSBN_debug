...
if($loggedin["group"] == 'admin'){
    ...
    if(isset($_POST["hapus"]))
    {
        ...
        {
            $query = "UPDATE `gx_inet_command` SET `level` = '1', `user_upd` = '".$loggedin["username"]."', `date_upd` = NOW()
                    WHERE `id_cmd` = '".$value."';";
            $sql_update = mysql_query($query, $conn) or die (mysql_error());
            //log
            enableLog("",$loggedin["username"], $loggedin["id_employee"],$query);
        }
        header('location: '.URL_ADMIN.'data/list_command.php');
    }
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="box box-primary">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">List Profile Command</h3>
                                    <!--<div class="box-tools pull-right">
										<a class="btn bg-olive btn-flat margin" href="'.URL_ADMIN.'data/list_command.php">Profile CMD RAS Server</a>
										<a class="btn bg-olive btn-flat margin" href="'.URL_ADMIN.'data/list_command_olt.php">Profile CMD OLT Server</a>
										<a class="btn bg-olive btn-flat margin" href="'.URL_ADMIN.'data/list_command_switch.php">Profile CMD Switch Server</a>
                                    </div>-->
                                </div>
                                <div class="box-body table-responsive">
                                    <div>
                                        <a href="'.URL_ADMIN.'data/form_command">New command</a>
                                    </div><br>
...
if(isset($_POST["search"])){
    ...
    $sql_command = mysql_query("SELECT * FROM `gx_inet_command`
...
while ($row_command = mysql_fetch_array($sql_command))
{
    $content .= '<tr>
		    ...
		    <td align="center"><a href="form_command?id='.$row_command["id_cmd"].'">Update</a></td>
