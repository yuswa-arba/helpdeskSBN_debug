...
if(isset($_POST["step1"]))
{
	...
	if($userid != "")
	{
		...
		mysql_query("INSERT INTO `db_sbn`.`gx_log_aktivasi` (`id_log_aktivasi`, `userid_aktivasi`, `vlan_aktivasi`, `idolt_aktivasi`,
					`pon_aktivasi`, `ponid_aktivasi`, `macaddress_aktivasi`, `log`, `step`, `user_add`, `from_ip`)
					VALUES ('', '".$userid."', '".$vlan."', '".$id_olt."', '".$pon."', '".$pon_id."', '".$mac_address."',
                    NOW(), 'STEP 1 CONFIG ONU TEST DAN CEK POWER ONU', '".$loggedin["username"]."', '".$_SERVER["REMOTE_ADDR"]."');", $conn);
...
elseif(isset($_POST["cekpower"]))
{
	...
	if($userid != "")
	{
		...
		mysql_query("INSERT INTO `db_sbn`.`gx_log_aktivasi` (`id_log_aktivasi`, `userid_aktivasi`, `vlan_aktivasi`,
					...
					VALUES ('', '".$userid."', '".$vlan."', '".$id_olt."', '".$pon."', '".$pon_id."', '".$mac_address."',
                    NOW(), 'CEK POWER ONUTEST', '".$loggedin["username"]."', '".$_SERVER["REMOTE_ADDR"]."');", $conn);
...
if(isset($_POST["config_onu"]))
{
	...
	if($userid != "")
	{
		...
		mysql_query("INSERT INTO `db_sbn`.`gx_log_aktivasi` (`id_log_aktivasi`, `userid_aktivasi`, `vlan_aktivasi`,
					`idolt_aktivasi`, `pon_aktivasi`, `ponid_aktivasi`, `macaddress_aktivasi`, `log`, `step`, `user_add`, `from_ip`)
					VALUES ('', '".$userid."', '".$vlan."', '".$id_olt."', '".$pon."', '".$pon_id."', '".$mac_address."',
                    NOW(), 'STEP 2 (CONFIG ONU - CEK DHCP LEASE)', '".$loggedin["username"]."', '".$_SERVER["REMOTE_ADDR"]."');", $conn);
...
$replace_uplink = array_merge($uplink1, $uplink2, $uplink3, $uplink4, $uplink5, $uplink6);
			...
			$flan4 = fopen('/usr/script/olt/create_vlan_olt_new.exp', 'w');
			fwrite($flan4, $script_create_vlan_olt);
			fclose($flan4);
			chmod("/usr/script/olt/create_vlan_olt_new.exp",0755);
...
';
		...
		chmod("/usr/script/olt/remove_vlan_olt2.exp",0755);
			...
			$output_create = shell_exec('expect /usr/script/olt/create_vlan_olt_new.exp');
...
';
		...
		chmod("/usr/script/switch/remove_vlan.exp",0755);
			...
			//tes 18okto
			$output_create_switch = shell_exec('expect /usr/script/switch/create_vlan_new.exp');
			//sleep(30);
			
			//START SCRIPT SWTICH 2
			if($row_switch_olt["idparent_server"] != 0)
			{
				$result_switch2 = mysql_query("SELECT * FROM `gx_inet_listswitch` where `id_server` = '".$row_switch_olt["idparent_server"]."' LIMIT 0,1;", $conn);
									...
									<div class="form-group">
										<div class="row">
											...
											<div class="col-xs-8">
                                                ...
                                                '.((count($ARRAY_DHCP) < 1) ? '' : '<button type="submit" name="aktifasi" class="btn btn-danger">Aktifasi (Step 3)</button>').'
											</div>
...
elseif(isset($_POST["cek_dhcp"]))
{
	...
	if($userid != "")
	{
		...
		mysql_query("INSERT INTO `db_sbn`.`gx_log_aktivasi` (`id_log_aktivasi`, `userid_aktivasi`, `vlan_aktivasi`,
					`idolt_aktivasi`, `pon_aktivasi`, `ponid_aktivasi`, `macaddress_aktivasi`, `log`, `step`, `user_add`, `from_ip`)
					VALUES ('', '".$userid."', '".$vlan."', '".$id_olt."', '".$pon."', '".$pon_id."', '".$mac_address."',
                    NOW(), 'STEP 2 (CEK DHCP LEASE)', '".$loggedin["username"]."', '".$_SERVER["REMOTE_ADDR"]."');", $conn);
...
if(isset($_POST["aktifasi"]))
{
	...
	if($userid != "")
	{
		...
		mysql_query("INSERT INTO `db_sbn`.`gx_log_aktivasi3` (`id_log_aktivasi`, `userid_aktivasi`, `vlan_aktivasi`,
					`idolt_aktivasi`, `pon_aktivasi`, `ponid_aktivasi`, `macaddress_aktivasi`, `log`, `step`, `user_add`, `from_ip`)
					VALUES ('', '".$userid."', '".$vlan."', '".$id_olt."', '".$pon."', '".$pon_id."', '".$mac_address."',
                    NOW(), 'STEP 3 (AKTIVASI)', '".$loggedin["username"]."', '".$_SERVER["REMOTE_ADDR"]."');", $conn);
...
if ($api_vm->connect($ip_baru, $data_ip_vm['username_ipvm'], $data_ip_vm['password_ipvm']))
{
    ...
    */
    ...
    $status_aktivasi = '<h3 class="box-title success">Proses Aktifasi Selesai.</h3>';
