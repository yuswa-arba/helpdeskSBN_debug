  ...
  $return_form	= isset($_GET['r']) ? mysql_real_escape_string(strip_tags(trim($_GET['r']))) : "";
    $content ='
				...
				<div class="box-header">
                                    <h2 class="box-title">Data SPK Aktivasi</h2>
                                </div>
        ...
        <form action="" method="post" name="form_search" id="form_search">
		 <div class="form-group">
			<div class="row">
				...
				<div class="col-xs-3">
					<input class="form-control" name="kode" placeholder="Kode Aktivasi" type="text" value="">
				</div>
        ...
        if(isset($_POST["save_search"])){
                ...
                $sql_kode	= ($kode_data != "") ? "AND `kode_spkaktivasi` LIKE '".$kode_data."%'": "";
                $sql_nama	= ($nama_data != "") ? "AND `nama_customer` LIKE '%".$nama_data."%'": "";
				$sql_user	= ($user_data != "") ? "AND `user_id` LIKE '%".$user_data."%'": "";
                
                $sql_execution	= "SELECT * FROM `gx_spk_aktivasi`
                WHERE `level` = '0'
                $sql_kode
                $sql_nama
				$sql_user
        ...
        while ($row_execution = mysql_fetch_array($query_execution)) {
		 $sql_olt = mysql_query("SELECT * FROM `v_olt_customer` WHERE `userid` = '".$row_execution["user_id"]."' LIMIT 0,1;", $conn);
			$row_olt = mysql_fetch_array($sql_olt);
			
			$sql_vlan = mysql_query("SELECT * FROM `gx_data_vlan` WHERE `userid_vlan` = '".$row_execution["user_id"]."' LIMIT 0,1;", $conn);
			$row_vlan = mysql_fetch_array($sql_vlan);
			
			$sql_customer = mysql_query("SELECT * FROM `tbCustomer` WHERE `cUserID` = '".$row_execution["user_id"]."' AND  `level` = '0' LIMIT 0,1;", $conn);
			$row_customer = mysql_fetch_array($sql_customer);
			
			$sql_ip		= mysql_query("SELECT * FROM `gx_master_ip` WHERE `userid_ip` = '".$row_execution["user_id"]."' LIMIT 0,1;", $conn);
			$row_ip 	= mysql_fetch_array($sql_ip);
            
		$content .='<tr>
                            <td>'.$no.'</td>
							<td>'.$row_execution["kode_spkaktivasi"].'</td>
							<td>'.$row_execution["id_customer"].'</td>
                            <td>'.$row_execution["nama_customer"].'</td>
							<td>'.$row_execution["user_id"].'</td>
                            <td>';
                            $content .= '<a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_execution["kode_spkaktivasi"]).'\',';
							$content .= '\''.mysql_real_escape_string($row_execution["id_spkaktivasi"]).'\',';
							$content .= '\''.mysql_real_escape_string($row_execution["id_teknisi"]).'\',';
							$content .= '\''.mysql_real_escape_string($row_customer["cUserID"]).'\',';
							...
							$content .= '\''.mysql_real_escape_string($row_customer["cMacAdd"]).'\',';
							$content .= '\''.mysql_real_escape_string($row_customer["cKode"]).'\',';
...
<!-- DataTable -->
 ...
 $plugins .= '<script type=\'text/javascript\'>
		      function validepopupform2(uid,  idspk, idt, userid,  nama, ids, p, pid, mac, ckode, v, ip){
			      window.opener.document.'.$return_form.'.id_spkaktivasi.value=idspk;
				  window.opener.document.'.$return_form.'.kode_spkaktivasi.value=uid;
				  window.opener.document.'.$return_form.'.id_teknisi.value=idt;
				  window.opener.document.'.$return_form.'.kode_customer.value=ckode;
				  window.opener.document.'.$return_form.'.userid.value=userid;
