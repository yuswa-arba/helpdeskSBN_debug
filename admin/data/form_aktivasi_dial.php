                                    ...
                                    <div class="box-body">
                                        ...
                                        <div class="form-group">
											<div class="row">
												...
												<div class="col-xs-3">
													<input type="text" readonly="" placeholder="ID" class="form-control" name="id" maxlength="2" value="'.(isset($_POST["id"]) ? $_POST['id'] : "").'">
												</div>
											</div>
                                        </div>
										<div class="form-group">
...
//cut koneksi
try {
    $client_new = new RouterOS\Client($ipaddress_ipvm, $username_ipvm, $password_ipvm);
} catch (Exception $e) {
...
$data_status_dhcp = $client_new->sendSync(new RouterOS\Request($data_dhcp_lease_vm['script']));
...
$client_new->sendSync(new RouterOS\Request('/ip address set .id=*6 address='.$ip_baru.'/24'));

shell_exec('expect /usr/script/esx/power_off_vm.exp');
shell_exec('expect /usr/script/esx/power_vm.exp');
sleep(30);
						...
						<hr>
									...
									<div class="row">
										<div class="col-xs-10">											
											<div class="box box-solid box-info collapsed-box">
												<div class="box-header">
													<h3 class="box-title">IP VLAN Baru</h3>
													<div class="box-tools pull-right">
														<button class="btn btn-warning btn-sm" data-widget="collapse"><i class="fa fa-plus"></i></button>
													</div>
												</div>
												<div class="box-body">
													<pre>'.($output_ganti_ip).'</pre>
												</div>
											</div>
										</div>
									</div>
...
if(isset($_POST["aktifasi"]))
{
	...
	if($userid != "")
	{
		...
		$result_servicename = mysql_query("SELECT * FROM `v_master_ip` where `ip_address` = '".$row_customer['user_ip']."' LIMIT 0,1;", $conn);
...
$ipvm_user = mysql_query("SELECT * FROM `gx_master_ipvm` WHERE `keterangan_ip` = 'aktif' AND `userid_ip` = '".$userid."' LIMIT 0,1;", $conn);
$data_ipvm_user = mysql_fetch_array($ipvm_user);
...
$replaceVM = array(
	...
	'[ip_address]' => long2ip($data_ipvm_user['ip_address']),
	'[password]' => $row_customer['cPassword'],
	'[data_ras]' => $row_servicename['service_name']
);
...
$ipaddress_ipvm = long2ip($data_ipvm_user['ip_address']);
$username_ipvm  = $data_ipvm['username_ipvm'];
$password_ipvm  = $data_ipvm['password_ipvm'];
...
//config hostname vm
$client_new->sendSync(new RouterOS\Request($script_hostname));
$data_hostname_vm = $client_new->sendSync(new RouterOS\Request('/system identity print'));

//confing pppoe, enable pppoe
$client_new->sendSync(new RouterOS\Request($script_pppoe));
$client_new->sendSync(new RouterOS\Request($script_pppoe_enable));
$data_pppoe_vm = $client_new->sendSync(new RouterOS\Request('/interface pppoe-client print'));

//dial pppoe
$data_pppoe_dial = $client_new->sendSync(new RouterOS\Request($script_dial_vm));
...
$output_ganti_ip = $client->sendSync(new RouterOS\Request('/ip address set 1 address='.$ip_baru.'/24'));
$output_ganti_ip = $client->sendSync(new RouterOS\Request('/ip address print'));
