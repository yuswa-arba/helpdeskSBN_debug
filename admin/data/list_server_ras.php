...
if($loggedin["group"] == 'admin'){
    ...
    enableLog("",$loggedin["username"], $loggedin["id_employee"],"Open Menu List server RAS");
    ...
    if(isset($_POST["hapus"]))
    {
        ...
        {
            $query = "UPDATE `software`.`gx_inet_listServer` SET `level` = '1', `user_upd` = '".$loggedin["username"]."', `date_upd` = NOW()
        ...
        header('location: '.URL_ADMIN.'data/list_server_ras.php');
    }
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="box box-primary">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">List Server RAS</h3>
                                ...
                                <div class="box-body table-responsive">
                                    <div>
                                        <a href="'.URL_ADMIN.'data/form_server_ras">New Server</a>
                                    </div><br>
...
if(isset($_POST["search"])){
    ...
    $sql_server_ras = mysql_query("SELECT * FROM `software`.`gx_inet_listServer`, `software`.`gx_inet_listgroup`
				  WHERE `software`.`gx_inet_listServer`.`group_server` = `software`.`gx_inet_listgroup`.`id_inet_listgroup`
				  AND `level` = '0'
                                  $sql_search
    ...
    $sql_server_ras = mysql_query("SELECT * FROM `software`.`gx_inet_listServer`, `software`.`gx_inet_listgroup`
				 WHERE `software`.`gx_inet_listServer`.`group_server` = `software`.`gx_inet_listgroup`.`id_inet_listgroup`
				  AND `level` = '0' ORDER BY `nama_server` ASC;", $conn);
}
...
while ($row_server_ras = mysql_fetch_array($sql_server_ras))
{
    $content .= '<tr>
			...
			<td>'.($row_server_ras["group_server"]).'</td>
		    <td>'.$row_server_ras["desc_server"].'</td>
		    <td>'.$row_server_ras["date_upd"].' by '.$row_server_ras["user_upd"].'</td>
		    <td align="center"><a href="form_server_ras?id='.$row_server_ras["id_server"].'">Update</a></td>
