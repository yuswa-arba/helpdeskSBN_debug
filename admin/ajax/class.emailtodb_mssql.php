...
/**
  ...
  * Last Update: 31 Oktober 2014
  * Desc: Tambahan customer number, userid, kategori (intern, prospek, other)
  * Keterangan kategori, 1 : intern(klien), 2: prospek, 3: other
  * 
*/
...
class EMAIL_TO_DB {
	...
	var $IMAP_login;
	var $IMAP_pass;
	var $FILE_OWNER = 'dwi';		// owner of downloaded file attachments
	var $link;
	var $error = array();
	var $status;
	...
	var $filename_array;
	var $file_path;
        var $attachments_path = '/var/www/software/email_attach/';
	...
	function email_headers(){
		...
		function email_get(){
			...
			$header=imap_headerinfo($this->link, $this->msgid);
			$from = $header->from;
			$udate= $header->udate;
			$to   = $header->to;
			$size = $header->Size;
			...
			//if ($header->Unseen == "U" || $header->Recent == "N") {
				...
				if (count($s->parts)>0){
					...
					foreach($this->partsarray as $part){					
						if($part['text']['type'] == 'HTML'){
							...
							$email['BODY_HTML'] = $part['text']['string'];
						}elseif($part['text']['type'] == 'PLAIN'){
							$email['BODY'] = $part['text']['string'];
						}
					}
					...
					function spam_detect(){
						...
						$execute = mysql_query("SELECT ID, IDEmail, EmailFrom, EmailFromP, EmailTo,
					...
					function check_blacklist($email){
                                            global $conn;
						#spam - emails 
						$execute = mysql_query("SELECT Email FROM gx_email_list
                                                                       WHERE Email='".addslashes($email)."' AND Type='B'", $conn);
						$row = mysql_fetch_array($execute);
						$e_mail = $row['Email'];
						if($e_mail == $email){
							return 1;
					...
					function check_words($string){
						...
						$execute = mysql_query("SELECT Word FROM gx_email_words", $conn);
					...
					function db_add_message($email){
						...
						if(isset($email['BODY_HTML'])){
							...
							$temp_html_body = "";
                                                ...
                                                //check email dari customer atau bukan
						$conn_soft = Config::getInstanceSoft();
						$sql_user_soft = $conn_soft->prepare("SELECT TOP 1 [cKode], [cNama], [cEmail], [cUserID]
										   FROM [DRBSISP].[dbo].[tbCustomer]
										   WHERE ([tbCustomer].[cEmail] LIKE '%".addslashes(strip_tags($email['FROM_EMAIL']))."%');");
						
						$sql_user_soft->execute();
						$row_user_soft = $sql_user_soft->fetch();
						$customer_number = $row_user_soft["cKode"];
						$user_id = $row_user_soft["cUserID"];
						$kategori = '';
                                                
                                                $query_total_today  = "SELECT count(*) AS `total` FROM `gx_email` WHERE `date_add` LIKE '%".date("Y-m-d")."%';";
                                                $sql_total_today    = mysql_query($query_total_today, $conn);
                                                $row_total_today    = mysql_fetch_array($sql_total_today);
                                                
						//$execute = mysql_query("INSERT INTO gx_email (IDEmail, EmailFrom, EmailFromP, EmailTo, DateE, DateDb, Subject, MsgSize) VALUES
						$execute = mysql_query("INSERT INTO `gx_email` (`IDEmail`, `EmailFrom`, `EmailFromP`,
                                                                       `EmailTo`, `DateE`, `DateDb`, `Subject`, `Message`,
                                                                       `Message_html`, `MsgSize`, `date_add`, `user_add`,
                                                                       `customer_number`, `userid`, `id_kategori`, `id_complaint` )VALUES
						('".$message_id."',
						'".$email['FROM_EMAIL']."',
						...
						'".$email["SIZE"]."',
                                                ...
                                                '".$user_id."',
                                                '".$kategori."',
                                                'GXC".date("dmy").$row_total_today["total"]."')", $conn);

					$execute = mysql_query("select LAST_INSERT_ID() as UID", $conn);
					$row = mysql_fetch_array($execute);
					$this->newid = $row["UID"];
				...
				function db_add_attach($file_orig, $filename){
					...
					$execute = mysql_query("INSERT INTO `gx_email_attach` (`IDEmail`, `FileNameOrg`, `Filename`) VALUES
					('".$this->newid."',
					'".addslashes($file_orig)."',
					'".addslashes($filename)."')", $conn);
			...
			function db_update_message($msg, $type= 'PLAIN'){
				...
				if($type == 'PLAIN') $execute = mysql_query("UPDATE gx_email SET Message='".addslashes($msg)."'
                                                                            WHERE ID= '".$this->newid."'", $conn);

				if($type == 'HTML')  $execute = mysql_query("UPDATE gx_email SET Message_html='".addslashes($msg)."'
                                                                            WHERE ID= '".$this->newid."'", $conn);
			...
			function add_db_log($email, $info){
				...
				$execute = mysql_query("INSERT INTO gx_email_log (IDemail, Email, Info, FSize, Date_start, Status) VALUES
				('".$this->newid."',
				'".$email['FROM_EMAIL']."',
		...
		function update_folder($id, $folder){
			...
			$execute = mysql_query("UPDATE gx_email SET Type = '".addslashes($folder)."'
                                               WHERE ID = '".$id."'", $conn);
		...
		function update_db_log($info, $id){
			...
			$execute = mysql_query("UPDATE gx_email_log  SET Status = '1', Info='".addslashes(strip_tags($info))."',
                                               Date_finish = '".date("Y-m-d H:i:s")."' WHERE IDlog = '".$id."'", $conn);
		...
		function db_read_log(){
			...
			$execute = mysql_query("SELECT IDlog, IDemail, Email, Info, FSize, Date_start, Date_finish, Status
                                               FROM gx_email_log ORDER BY Date_finish DESC LIMIT 100", $conn);
			while($row = mysql_fetch_array($execute)){
				$ID = $row['IDlog'];
		...
		function db_read_emails(){
			...
			$execute = mysql_query("SELECT ID, IDEmail, EmailFrom, EmailFromP, EmailTo,
                                               DateE, DateDb, Subject, Message, Message_html, MsgSize
                                               FROM gx_email
                                               ORDER BY ID DESC LIMIT 25;", $conn);
		...
		function dir_name() {
			...
			if (is_dir($this->attachments_path)) {
				...
				echo "making path<br>";
				mkdir($this->attachments_path, 0777);
				return $this->attachments_path . '/';
			}
		...
		function db_num_rows($tbl){
			...
			$query = mysql_query("SELECT * FROM ".$tbl, $conn); 
		...
		function getNumNewMessages(){
			...
			*/
			...
			function db_update(){
				...
				//	get number of rows in gx_email table and start a huge for loop from the number of rows +1 until num_messages

				$total_messages = $this->num_message();

				if($total_messages >= 1) {	//if there are emails in the mailbox

					//$this->msgid_start = $this->db_num_rows("gx_email");
					//edit by dwi
					$this->msgid_start = 1;
