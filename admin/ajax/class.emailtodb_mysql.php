					...
					function db_add_message($email){
						...
						if(isset($email['BODY_HTML'])){
							...
							$temp_html_body = "";
                                                ...
                                                $query_customer = "SELECT * FROM `tbCustomer`
                                                WHERE `cEmail` LIKE '%".addslashes(strip_tags($email['FROM_EMAIL']))."%' LIMIT 0,1;";
                                                $sql_customer = mysql_query($query_customer, $conn);
                                                
                                                if(mysql_num_rows($sql_customer))
                                                {
                                                    $row_customer = mysql_fetch_array($sql_customer);
                                                    $customer_number = $row_customer["cKode"];
