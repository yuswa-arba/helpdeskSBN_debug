...
else
{
	...
	$sql_data = mysql_query("SELECT * FROM `tbCustomer` WHERE `level` = '0' ORDER BY `date_add` DESC;", $conn);	
...
while($row_data = mysql_fetch_array($sql_data))
{
	...
	$sql_rbs	= "SELECT TOP 1 dbo.Users.UserIndex, dbo.Users.UserID, dbo.Users.UserIP, dbo.AccountTypes.AccountName, dbo.Users.GroupName, dbo.Users.UserPaymentBalance,
  ...
  $sql_tanggal
  AND dbo.Users.UserIndex = '".$row_data["iuserIndex"]."';";
	...
	if($row_rbs["UserIndex"] !="")
	{
		...
		<td>'.$row_data["cGroupRBS"].'</td>
		<td>'.$row_data["acc_type_rbs"].'</td>
		<td>'.$row_data["user_ip"].'</td>
