...
$query_all_user = mysql_query($sql_all_user, $conn_voip);
		while ($row_user = mysql_fetch_array($query_all_user)){
		    ...
		    $sql_billing_indosat = mysql_query("SELECT * FROM `gx_voip_billing`
						       WHERE `id_voip` = '".$row_user["useralias"]."'
						       AND `tagihan_bulan` = '".date("Y-m")."'
						       LIMIT 0,1;", $conn);
		    if ($sql_billing_indosat)
		    {
			$billing_indosat = mysql_num_rows($sql_billing_indosat);
			if ($billing_indosat == 1)
			{
			    $row_billing_indosat = mysql_fetch_array($sql_billing_indosat);
			    $total_billing_indosat = Rupiah(Bulatdua($row_billing_indosat["total_billing_indosat"]));
			    $grand_total_indosat += $row_billing_indosat["total_billing_indosat"];
			}else{
			    $total_billing_indosat = 0;
			}
			...
			<tr> 
			    <td>'.$no.'</td>
			    <td>'.$row_customer["customer_number"].'</td>
			    <td>'.$row_user["firstname"].' '.$row_user["lastname"].'</td>
			    <td>'.$row_user["useralias"].'</td>
			    <td class="text-right">'.(Bulatdua($total)).'</td>
			    <td class="text-right">'.$total_billing_indosat.'</td>
			    <td>
			    <a href="form_billing.php?id='.$row_user["useralias"].'&m='.date("Y-m").'">Update</a> | 
			    <a href="detail_billing.php?id='.$card_id.'&m='.date("Y-m").'">detail</a></td>
			</tr>
			';
		    $grand_total += $total;
		    $grand_total_indosat += $total_billing_indosat;
		    $no++;
		}
		//<td class="text-right">'.Rupiah(Bulatdua($grand_total_indosat)).'</td>
                $content .='
		...
		<tfooter>
			...
			<td class="text-right">'.(Bulatdua($grand_total)).'</td>
			<td class="text-right">'.(Bulatdua($grand_total_indosat)).'</td>
			<td>&nbsp;</td>
		    </tr>
		</tfooter>
		</table>
