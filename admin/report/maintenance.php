...
if(isset($_GET["id"])){
	$id_employee	= isset($_GET["id"]) ? strip_tags(trim($_GET["id"])) : "";
    $date 			= isset($_GET["d"]) ? strip_tags(trim($_GET["d"])) : "";
	
    
	//$message_date	= ($date == "") ? " Pada Hari ini" : " Pada tanggal $date";
	
	$sql_teknisi = ($id_employee == "") ? "" : "AND `id_teknisi` = '$id_employee'";
	//$message_teknisi = ($teknisi == "") ? "" : " Dengan teknisi $teknisi";
	
	$data_pegawai = mysql_query("SELECT * FROM `gx_pegawai` WHERE `id_employee` = '".$id_employee."' AND `level` = '0' LIMIT 0,1;");
    $row_pegawai = mysql_fetch_array($data_pegawai);

    $queryList = "SELECT * FROM `gx_maintenance` WHERE `tanggal` LIKE '%$date%' $sql_teknisi
    AND `level` = '0' ORDER BY `id_monitoring` ASC;";
	//echo $queryList;
    $result = mysql_query($queryList) or die ("Error on Query List.data: ".mysql_error());
    $total = mysql_num_rows($result);
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">List Maintenance '.$row_pegawai["nama"].' Pada '.$date.'</h3>
								...
								<!--<form action="" method="post" name="form_search">
										...
										<div class="row">
											...
											<div class="col-xs-3">
												<input type="text" class="form-control" readonly="" name="id_employee" id="date" value="'.(isset($_GET["id"]) ? (int)$_GET["id"] : "").'">
											</div>
