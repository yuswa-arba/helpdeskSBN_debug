...
if(isset($_POST["save_search"])){
 ...
 $sql_rab	= "SELECT * FROM `gx_rab_control` WHERE `level` = '0' AND `status` = '1' ORDER BY `id_rab_control` DESC LIMIT 0,10;";
}
 ...
 if($_GET["f"] == "persetujuanrab"){
  while ($row_rab = mysql_fetch_array($query_rab)) {
	 ...
	 $content .='<tr>
		     ...
		     <td>'.$row_rab["kode_rab_control"].'</td>
		     <td>'.$row_rab["tanggal"].'</td>
		     <td>'.$row_rab["user_add"].'</td>
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_rab["kode_rab_control"]).'\', \''.mysql_real_escape_string($row_rab["keterangan"]).'\', \''.mysql_real_escape_string($row_rab["periode"]).'\', \''.mysql_real_escape_string($row_rab["probability_index"]).'\', \''.mysql_real_escape_string($row_rab["target_user_baru"]).'\', \''.mysql_real_escape_string($row_rab["user_add"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(kode_rabcontrol, ketterangan, perriode, probabilityindex, target_userbaru, pembuatrab){
		 window.opener.document.'.$return_form.'.kode_rab_control.value=kode_rabcontrol;
