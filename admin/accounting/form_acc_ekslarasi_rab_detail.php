...
if($loggedin = logged_inAdmin()){ // Check if they are logged in
 ...
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Form ACC Ekslarasi Rencana Anggaran Belanja Detail");
 
if(isset($_GET["c"]))
{
    $kode_data		= isset($_GET['c']) ? $_GET['c'] : '';
    $query_data		= "SELECT * FROM `gx_rab_acc_ekslarasi` WHERE `kode_rab_acc_ekslarasi`='$kode_data' LIMIT 0,1;";
	//echo $query_data;
    $sql_data		= mysql_query($query_data, $conn);
    $row_data		= mysql_fetch_array($sql_data);
	
	$return_url = 'form_acc_ekslarasi_rab_detail.php?c='.$row_data["kode_rab_acc_ekslarasi"];
                                ...
                                <form role="form" method="POST" name="myForm" action="" enctype="multipart/form-data" >
                                    <div class="box-body">
										...
										<div class="row">
											...
											<div class="col-xs-2">
												<input type="text" readonly="" class="form-control" id="no_rab_acc_ekslarasi" name="no_rab_acc_ekslarasi" required="" value="'.(isset($_GET['c']) ? $row_data["kode_rab_acc_ekslarasi"] : "").'">
											</div>
											<div class="col-xs-2">
...
if(isset($_GET["c"]))
{
    ...
    $query_rab_item	 = "SELECT * FROM `gx_rab_acc_ekslarasi_detail` WHERE `kode_rab_acc_ekslarasi` ='".$kode_rab."' AND `level` = '0';";
    $sql_rab_item	 = mysql_query($query_rab_item, $conn);
    $id_detail_rab 	 = isset($_GET['id']) ? (int)$_GET['id'] : '';
    $query_rab_detail = "SELECT * FROM `gx_rab_acc_ekslarasi_detail` WHERE `kode_rab_acc_ekslarasi` ='".$kode_rab."' AND `level` = '0' AND `id_rab_acc_ekslarasi_detail` = '".$id_detail_rab."' LIMIT 0,1;";
    //echo $query_rab_item;
	...
	<td>'.$row_rab_item["pendapatan_acc"].'</td>
	<td>'.$row_rab_item["biaya_acc"].'</td>
	<td>'.$row_rab_item["remarks"].'</td>
	<td><a href="form_acc_ekslarasi_rab_detail?c='.$row_rab_item["kode_rab_acc_ekslarasi"].'&id='.$row_rab_item["id_rab_acc_ekslarasi_detail"].'"><span class="label label-info">Edit</span></a></td>
	</tr>
	';
	$no++;
	...
	$total_pendapatan_persetujuan = $total_pendapatan_persetujuan + $row_rab_item["pendapatan_ekslarasi"];
	$total_biaya_persetujuan = $total_biaya_persetujuan + $row_rab_item["biaya_ekslarasi"];
	$total_pendapatan_ekslarasi = $total_pendapatan_ekslarasi + $row_rab_item["pendapatan_acc"];
	$total_biaya_ekslarasi = $total_biaya_ekslarasi + $row_rab_item["biaya_acc"];
    }
}else{
...
$content .='							    <tr>
					...
					<div class="row">
						...
						<input type="hidden" name="id" value="'.(isset($_GET['id']) ? $row_rab_detail["id_rab_acc_ekslarasi_detail"] : '').'" readonly="">
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
							<input readonly=""  type="text" class="form-control" name="pendapatan_ekslarasi" id="pendapatan_ekslarasi" value="'.(isset($_GET['id']) ? $row_rab_detail["pendapatan_ekslarasi"] : "").'">
					    </div>
                    </div>
					</div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-8">
							<input readonly="" type="text" class="form-control" name="biaya_ekslarasi" id="biaya_ekslarasi" value="'.(isset($_GET['id']) ? $row_rab_detail["biaya_ekslarasi"] : "").'">
					    </div>
                    </div>
					</div>
					...
					<div class="row">
					    <div class="col-xs-4">
							<label>Pendapatan ACC Ekslarasi</label>
					    </div>
					    <div class="col-xs-8">
							<input type="text" class="form-control" name="pendapatan_acc" id="pendapatan_acc" value="'.(isset($_GET['id']) ? $row_rab_detail["pendapatan_acc"] : "").'">
					    </div>
                    </div>
					</div>
					...
					<div class="row">
					    <div class="col-xs-4">
							<label>Biaya ACC Ekslarasi</label>
					    </div>
					    <div class="col-xs-8">
							<input type="text" class="form-control" name="biaya_acc" id="biaya_acc" value="'.(isset($_GET['id']) ? $row_rab_detail["biaya_acc"] : "").'">
					    </div>
                    </div>
					</div>
...
if(isset($_POST["save"]))
{
	...
	$pendapatan_acc			= isset($_POST['pendapatan_acc']) ? mysql_real_escape_string(trim($_POST['pendapatan_acc'])) : '';
    $biaya_acc		 		= isset($_POST['biaya_acc']) ? mysql_real_escape_string(trim($_POST['biaya_acc'])) : '';
	$remarks 				= isset($_POST['remarks']) ? mysql_real_escape_string(trim($_POST['remarks'])) : '';
	if($kode_rab != "" && $nama_account != "" && $no_perkiraan != ""){
	$sql_insert = "INSERT INTO `gx_rab_acc_ekslarasi_detail` (`id_rab_acc_ekslarasi_detail`, `kode_rab_acc_ekslarasi`, `no_perkiraan`,
					  ...
					  VALUES ('', '".$kode_rab."', '".$no_perkiraan."',
						  '".$nama_account."', '".$pendapatan_sekarang."', '".$biaya_sekarang."', '".$pendapatan_acc."', '".$biaya_ekslarasi."', '".$pendapatan_ekslarasi."', '".$biaya_acc."', '".$remarks."',
	...
	echo "<script language='JavaScript'>
			...
			window.location.href='".URL_ADMIN."accounting/form_acc_ekslarasi_rab_detail.php?c=$kode_rab';
			</script>";
	}else{
	echo "<script language='JavaScript'>
		alert('Maaf Data tidak bisa diinsert ke dalam Database, Ada kesalahan!');
...
}elseif(isset($_POST["update"]))
{
    ...
    $sql_update = "UPDATE `gx_rab_acc_ekslarasi_detail` SET `level` = '1',
		    `date_upd` = NOW(), `user_upd`= '".$loggedin["username"]."'
		    WHERE `id_rab_acc_ekslarasi_detail` = '$_GET[id]';";
	...
	$sql_insert_update	=  "INSERT INTO `gx_rab_acc_ekslarasi_detail` (`id_rab_acc_ekslarasi_detail`, `kode_rab_acc_ekslarasi`, `no_perkiraan`,
						  `nama_account`, `pendapatan_sekarang`, `biaya_sekarang`, `pendapatan_ekslarasi`, `biaya_ekslarasi`, `pendapatan_acc`, `biaya_acc`, `remarks`,
						  `user_add`, `user_upd`, `date_add`, `date_upd`,  `level`) 
					  VALUES ('', '".$kode_rab."', '".$no_perkiraan."',
						  '".$nama_account."', '".$pendapatan_sekarang."', '".$biaya_sekarang."', '".$pendapatan_ekslarasi."', '".$biaya_ekslarasi."', '".$pendapatan_acc."', '".$biaya_acc."', '".$remarks."',
...
}elseif(isset($_POST["savelock"]))
{
    ...
    $laba_rugi_seetelah_ekslarasi		= isset($_POST['laba_rugi_seetelah_ekslarasi']) ? mysql_real_escape_string(trim($_POST['laba_rugi_seetelah_ekslarasi'])) : '';
	...
	$sql_update_lock = "UPDATE `gx_rab_acc_ekslarasi` SET `status`='1', `total_biaya`='".$total_biaya."', `biaya_operasional`='".$biaya_operasional."', `pembelian`='".$pembelian."', `total_pendapatan`='".$total_pendapatan."', `laba_rugi`='".$laba_rugi."',
						...
						`date_upd` = NOW(), `user_upd` = '".$loggedin["username"]."'
						WHERE `kode_rab_acc_ekslarasi`='".$_GET["c"]."';";
	...
	window.location.href='master_acc_ekslarasi_rab';
	</script>";
}
...
$plugins = '<!-- datepicker -->
    ...
    $title	= 'Form ACC Ekslarasi RAB Detail';
