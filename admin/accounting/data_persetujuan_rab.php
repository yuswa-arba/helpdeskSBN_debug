...
if($loggedin["group"] == 'admin'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open data Persetujuan RAB");
    global $conn;
    ...
    $content ='
				...
				<div class="box-header">
                                    <h2 class="box-title">Data Persetujuan RAB</h2>
                                </div>
...
if(isset($_POST["save_search"])){
 ...
 $sql_rab	= "SELECT * FROM `gx_rab_persetujuan` WHERE `level` = '0' AND `status` = '1' ORDER BY `id_rab_persetujuan` DESC LIMIT 0,10;";
}
 ...
 if($_GET["f"] == "ekslarasi_rab"){
  while ($row_rab = mysql_fetch_array($query_rab)) {
	 ...
	 $content .='<tr>
		     ...
		     <td>'.$row_rab["kode_rab_persetujuan"].'</td>
		     <td>'.$row_rab["tanggal"].'</td>
		     <td>'.$row_rab["user_add"].'</td>
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_rab["kode_rab_persetujuan"]).'\', \''.mysql_real_escape_string($row_rab["keterangan"]).'\', \''.mysql_real_escape_string($row_rab["periode"]).'\', \''.mysql_real_escape_string($row_rab["probability_index"]).'\', \''.mysql_real_escape_string($row_rab["target_user_baru"]).'\', \''.mysql_real_escape_string($row_rab["user_add"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(kode_rabpersetujuan, ketterangan, perriode, probabilityindex, target_userbaru, pembuatrab){
		 window.opener.document.'.$return_form.'.kode_rab_persetujuan.value=kode_rabpersetujuan;
