...
enableLog($id_customer="", $nama_customer= $loggedin["username"], $id_admin=$loggedin["id_employee"],$activity="Open list Call History");
global $conn_voip;
...
if(isset($_POST["search"]))
{
    ...
    $callerid			= isset($_POST["callerid"]) ? trim(strip_tags($_POST["callerid"])) : "";
    ...
    if($callerid != ''){
	    $sql_callerid = "AND `src` LIKE '%$callerid%'";
    }else{
	    $sql_callerid = "";
    }
    ...
    $sql_callhistory = "SELECT t1.starttime, t1.src, t1.id_tariffplan, t1.calledstation, t1.destination, t1.sessiontime, t1.terminatecauseid, t1.sipiax, t1.sessionbill
		    ...
		    $sql_today
		    $call_type2
                    AND starttime ORDER BY t1.starttime DESC LIMIT $start, $perhalaman;";
		    
    $sql_timetotal = "SELECT sum(t1.sessiontime) AS total_time ,sum(t1.sessionbill) AS total_bill
                    FROM `cc_call` t1
                    WHERE `src` != 0 AND `card_id` != ''
		    $sql_callerid
		    $sql_phonenumber
		    $sql_fromday
		    $sql_today
	...
	$sql_total_data	= mysql_num_rows(mysql_query("SELECT t1.starttime, t1.src, t1.id_tariffplan, t1.calledstation, t1.destination, t1.sessiontime, t1.terminatecauseid, t1.sipiax, t1.sessionbill
                    FROM `cc_call` t1
                    WHERE `src` != 0
		    $sql_callerid
		    $sql_phonenumber
		    ...
		    $sql_today
		    $call_type2
                    AND starttime ORDER BY t1.starttime DESC;", $conn_voip));
	
    
    
//echo $sql_callhistory;
enableLog($id_customer="", $nama_customer= $loggedin["username"], $id_admin=$loggedin["id_employee"],$activity="Search Call History = $sql_callhistory ");
...
$content.='                                        </select>
			...
			<tr>
				<td align="left" class="bgcolor_004">
					<font class="fontstyle_003">&nbsp;&nbsp;PHONE NUMBER</font>
				</td>
			...
			<tr>
				<td align="left" class="bgcolor_004">
					<font class="fontstyle_003">&nbsp;&nbsp;Caller ID</font>
				</td>
				<td align="left" class="bgcolor_005">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tbody><tr><td class="fontstyle_searchoptions"><input type="text" name="callerid" value="" class="form_input_text"></td>				
				</tr></tbody></table></td>
			</tr>
			<!-- Select Calltype: -->
			<tr>
	...
	<br />';
		...
		$content .='<center>
		    <h4>Number Of Calls : '.$sql_total_data.'</h4>
		    </center>';
		...
		if(isset($_POST["search"])){
		   ...
		   <tbody><tr><td bgcolor="#000000">			
			   ...
			   <td align="center" class="callhistory_td2">DURATION</td>
				   ...
				   <td bgcolor="#F2F8FF" align="right" nowrap="nowrap" class="fontstyle_001">'.$sql_total_data.'</td>
				   <td bgcolor="#F2F8FF" align="right" nowrap="nowrap" class="fontstyle_001">00:17 </td>
					   <td bgcolor="#F2F8FF" align="right" nowrap="nowrap" class="fontstyle_001">'.number_format($row_toootal['total_bill'], 2 ,',', '.').' IDR</td>
			   ...
			   <tr class="callhistory_td2">
				   ...
				   <td align="center" nowrap="nowrap" class="callhistory_td4">'.$sql_total_data.'</td>
				   <td align="center" nowrap="nowrap" class="callhistory_td4">00:17</td>
				   <td align="center" nowrap="nowrap" class="callhistory_td4">'.number_format($row_toootal['total_bill'], 2 ,',', '.').' IDR</td>
			   </tr>
