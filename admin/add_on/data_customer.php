  ...
  while ($row_customer = mysql_fetch_array($query_customer)) {
   ...
   $row_paket 	= mysql_fetch_array($query_paket);
	 ...
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["cKode"]).'\', \''.mysql_real_escape_string($row_customer["cNama"]).'\', \''.mysql_real_escape_string($row_customer["cKdPaket"]).'\', \''.mysql_real_escape_string($row_customer["cPaket"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
if(isset($_GET["f"])){
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(customernumber, cnamacust, ckodepaketlama, cnamapaketlama){
