...
//UPDATE
    ...
    $redirect_action_lock = "master_acc_kenaikan_gaji.php";
...
//String Data View
    //Judul Form
    $judul_form = "Detail ACC Kenaikan Gaji";
    $judul_table = "detail ACC Kenaikan Gaji";
    
    //id web
    $title_header = 'Detail ACC Kenaikan Gaji';
    $submenu_header = 'detail_acc_kenaikan_gaji';
    
    
if($loggedin["group"] == 'admin'){
    enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Detail ACC Kenaikan Gaji");
    global $conn;
    $id_d = isset($_GET['id']) ? mysql_real_escape_string(trim($_GET['id'])) : '';
    $c_d = isset($_GET['c']) ? mysql_real_escape_string(trim($_GET['c'])) : '';
    $sql_d = "SELECT * FROM `gx_master_acc_kenaikan_gaji` WHERE `id_acc_kenaikan_gaji`='$id_d' AND `level`='0' ORDER BY `id` DESC LIMIT 0,1";
	...
	$no_acc_kenaikan_gaji		= $data_d['no_acc_kenaikan_gaji'];
