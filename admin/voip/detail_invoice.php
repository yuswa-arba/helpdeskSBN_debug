...
//data invoice
$sql_invoice = "SELECT `gx_voip_invoice`.*, `gx_cabang`.* FROM `gx_voip_invoice`, `gxLogin`, `gx_cabang` 
                WHERE `gx_voip_invoice`.`id_voip` = `gxLogin`.`id_voip`
                AND `gxLogin`.`id_cabang` = `gx_cabang`.`id_cabang`
                AND `gx_voip_invoice`.`id_invoice` = '".$id_invoice."'
...
$sql_cust = mysql_query("SELECT `tbCustomer`.* FROM `tbCustomer`, `gxLogin`
			     ...
			     AND `gxLogin`.`id_voip` = '".$row_invoice["id_voip"]."';", $conn);
$row_cust = mysql_fetch_array($sql_cust);

//desc invoice
$sql_desc_invoice = "SELECT `gx_voip_invoice`.*, `gx_paket`.`nama_paket`
                FROM `gx_voip_invoice`, `gx_paket`
                WHERE `gx_voip_invoice`.`id_paket` = `gx_paket`.`id_paket`
                AND `gx_voip_invoice`.`id_invoice` = '".$id_invoice."'
                AND `gx_voip_invoice`.`invoice_status` = 'unpaid' 
                AND `gx_voip_invoice`.`level` = '0' ;";
