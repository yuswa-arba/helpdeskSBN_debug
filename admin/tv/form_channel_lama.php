...
if(!($con = ssh2_connect("192.168.223.11", 22))){
    ...
    if(!ssh2_auth_password($con, "root", "123456")) {
        ...
        // allright, we're in!
        echo "okay: logged in...\n";
        ssh2_scp_send($con, "/var/www/ott/upload/$NewImageName", "/usr/local/OTTServer/apache-tomcat-7.0.50/webapps/ottserver/upload/$NewImageName", 0644);
...
/*
 *Create file /usr/local/WowzaStreamingEngine/conf/StartupStreams.xml
...
//restart wowza stream engine
//shell_exec('/usr/local/WowzaStreamingEngine/start.sh 2>&1 & echo $!');
