...
$no = 1;
  ...
  while ($row_customer = mysql_fetch_array($query_customer)) {
	 ...
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_customer["cUserID"]).'\', \'000000\', \''.mysql_real_escape_string($row_customer["dTglLahir"]).'\', \''.mysql_real_escape_string($row_customer["ctelp"]).'\', \''.mysql_real_escape_string($row_customer["cEmail"]).'\', \''.mysql_real_escape_string($row_customer["cAlamat1"]).'\', \''.mysql_real_escape_string($row_customer["cKota"]).'\')">Select</a>
		     </td>
		   </tr>';
	 $no++;
     }
...
';
 ...
 <script type="text/javascript">
	 ...
	 function validepopupform2(cusername, cpass, ctgllahir, cphone, cemail, caddress, clokasi){
		 window.opener.document.'.$return_form.'.username.value=cusername;
		 window.opener.document.'.$return_form.'.birthday.value=ctgllahir;
		 window.opener.document.'.$return_form.'.phone.value=cphone;
		 window.opener.document.'.$return_form.'.email.value=cemail;
		 window.opener.document.'.$return_form.'.address.value=caddress;
		 window.opener.document.'.$return_form.'.location.value=clokasi;
		 self.close();
	 }
 </script>
