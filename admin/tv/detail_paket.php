...
if(isset($_GET["id"]))
{
    ...
    $sql_data	= mysql_query($query_data, $conn_ott);
...
//select data
    ...
    $sql_data		= mysql_query("SELECT * FROM `ott_movie`;", $conn_ott);
    $nom = 1;
    ...
    while($row_data = mysql_fetch_array($sql_data))
    {
		...
		$sql_service	= mysql_query($query_service, $conn_ott);
		$numrow_service	= mysql_num_rows($sql_service);
...
//select data
    ...
    $sql_datac		= mysql_query("SELECT * FROM `ott_livechannel`;", $conn_ott);
    $noc = 1;

    while($row_datac = mysql_fetch_array($sql_datac))
    {
		$id_data		= isset($_GET['id']) ? (int)$_GET['id'] : '';
		$query_service2	= "SELECT * FROM `ott_productservice` WHERE `type`='2' AND `proID`='".$id_data."' AND `movieId`='".$row_datac["channelId"]."' LIMIT 0,1;";
		
		$sql_service2	= mysql_query($query_service2, $conn_ott);
...
';
	...
	header("location: ".URL_OTT."logout.php");
    }
