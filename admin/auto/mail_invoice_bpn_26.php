...
require_once("/var/www/sbn/beta/admin/auto/mail_generate_bpn.php");
...
while($row_invoice = mysql_fetch_array($sql_invoice))
{
	   ...
	   AND $row_customer["kode_inactive"] == "")
	{
		...
		try {
			...
			$mail->SetFrom( $from, 'Helpdesk SBN BPN');
			...
			//admin invoice SBN
			$mail->AddBCC('adminholding@globalxtreme.net ');
			$mail->AddCC('admin.balikpapan@globalxtreme.net');
