...
global $conn;
	...
	$sql_cabang = mysql_query("SELECT * FROM `gx_cabang` WHERE `id_cabang` = '18' AND `level` = '0';", $conn);
		...
		{		
			...
			foreach($lines as $key)
			{
				...
				if (stripos($first_string[0], 'EPON') !== false)
				{
					...
					if($row_olt_customer["userid"] != "")
					{
						echo "sms";
						$sql_customer = mysql_query("SELECT * FROM `tbCustomer`
											WHERE `cUserID` = '".$row_olt_customer["userid"]."'
											LIMIT 0,1;", $conn);
						
						$row_customer	= mysql_fetch_array($sql_customer);
						
						$sql_sms_internal = mysql_query("SELECT * FROM `gx_setting_sms_notif`
											WHERE `id_setting_sms_notif` = '1'
											LIMIT 0,1;", $conn);
						
						$row_sms_internal	= mysql_fetch_array($sql_sms_internal);
						
						//sms jika status wire-down
						if($LastDeregReason == "wire-down" OR $LastDeregReason == "wire down")
						{
							$body_sms = '[GlobalXtreme Helpdesk] userid: '.$row_olt_customer["userid"].', connection status is wire-down.';
							...
							//
							//if($row_customer["sms_1"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_1"], $body_sms);
							//if($row_customer["sms_2"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_2"], $body_sms);
							//if($row_customer["sms_3"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_3"], $body_sms);
							//if($row_customer["sms_4"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_4"], $body_sms);
							echo $body_sms."<br>";
						}
						elseif($LastDeregReason == "power-down" OR $LastDeregReason == "power down")
						{
							$body_sms = '[GlobalXtreme Helpdesk] userid: '.$row_olt_customer["userid"].', connection status is power-down.';
							//if($row_customer["cNoHp1"] !="") send_mail("sms-gateway@dps.globalxtreme.net", $row_customer["cNama"], $row_customer["cNoHp1"], $body_sms);
							//if($row_customer["cNoHp2"] !="") send_mail("sms-gateway@dps.globalxtreme.net", $row_customer["cNama"], $row_customer["cNoHp2"], $body_sms);
							//
							//if($row_customer["sms_1"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_1"], $body_sms);
						...
						{
							$body_sms = '[GlobalXtreme Helpdesk] userid: '.$row_olt_customer["userid"].', connection status is '.$LastDeregReason.'.';
							//if($row_customer["cNoHp1"] !="") send_mail("sms-gateway@dps.globalxtreme.net", $row_customer["cNama"], $row_customer["cNoHp1"], $body_sms);
							//if($row_customer["cNoHp2"] !="") send_mail("sms-gateway@dps.globalxtreme.net", $row_customer["cNama"], $row_customer["cNoHp2"], $body_sms);
							//
							//if($row_customer["sms_1"] !="") send_mail("sms-gateway@dps.globalxtreme.net", '', $row_customer["sms_1"], $body_sms);
