...
function generate_pdf($kode_invoice)
{
    $kode_invoice = ($kode_invoice == "") ? "" : trim($kode_invoice);
    global $conn;
    ...
    if ($conn) {
        ...
        $query_invoice = "SELECT `gx_invoice`.* , `tbCustomer`.*, `gx_paket2`.`nama_paket`
			...
			AND `gx_invoice`.`kode_invoice` ='" . $kode_invoice . "' LIMIT 0,1;";
        ...
        <tbody>
            <tr>
                <td style="text-align:center;">
                    ...
                    Printed by : ' . (($row_invoice["printed_by"] != "") ? $row_invoice["printed_by"] : 'Helpdesk') . ' ( ' . $_SERVER['REMOTE_ADDR'] . ' )<br>
...
//echo $html;
        ...
        require_once('/var/www/sbn/beta/js/tcpdf/tcpdf.php');
//require_once ('/var/www/sbn/beta/../js/fpdf/fpdf.php');
        require_once('/var/www/sbn/beta/js/fpdi/fpdi.php');

        $pdf = new FPDI(); //FPDI extends TCPDF
//$pdf->AddPage();
        $pages = $pdf->setSourceFile('/var/www/sbn/beta/admin/auto/inv.pdf');
...
//echo $page_satu;
        ...
        $pdf->Output('/var/www/sbn/beta/admin/auto/invoice/sbn_' . $row_invoice["kode_invoice"] . '.pdf', 'F');
