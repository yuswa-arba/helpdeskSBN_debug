	...
	while ($row_cabang = mysql_fetch_array($sql_cabang))
	{
		...
		{		
			...
			foreach($lines as $key)
			{
				...
				{
					$IntfName	= substr($key, 0, 11);
					$mac_addr	= substr($key, 11, 15);
					$status		= substr($key, 26, 16);
					$LastRegTime		= trim(substr($key, 42, 20));
					$LastDeregTime		= trim(substr($key, 62, 20));
					$LastDeregReason	= substr($key, 82, 18);
