...
if(isset($_POST["save_search"])){
 ...
 $sql_data	= "SELECT * FROM `gx_pegawai` WHERE `id_bagian` = 'Marketing' AND `level` = '0' ORDER BY `id_employee` ASC;";
}
...
$no = 1;
  ...
  while ($row_data = mysql_fetch_array($query_data)) {
	 $content .='<tr>
		     ...
		     <td>
		       <a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_data["kode_pegawai"]).'\',';
...
<script type="text/javascript">
	...
	function validepopupform2(id, nama){
                window.opener.document.'.$return_form.'.kode_pegawai.value=id;
				window.opener.document.'.$return_form.'.nama_pegawai.value=nama;
                self.close();
        }
</script>
