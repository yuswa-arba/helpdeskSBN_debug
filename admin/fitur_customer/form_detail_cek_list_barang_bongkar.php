...
$d = isset($_GET['d']) ? $_GET['d'] : '';
	...
	//SELECT `id`, `kode_list_barang_customer`, `kode_cabang`, `cabang`, `kode_customer`, `nama_customer`, `uid`, `status`, `date_add`, `date_upd`, `user_add`, `user_upd`, `level` FROM `gx_list_barang_customer` WHERE 1
	//SELECT `id`, `kode_list_barang_customer_detail`, `kode_list_barang_customer`, `tanggal`, `kode_barang`, `nama_barang`, `qty`, `serial_number`, `status`, `date_add`, `date_upd`, `user_add`, `user_upd`, `level` FROM `gx_list_barang_customer_detail` WHERE 1
	$sql_read_data = "SELECT * FROM `gx_list_barang_customer`, `gx_list_barang_customer_detail` WHERE `gx_list_barang_customer`.`kode_list_barang_customer`=`gx_list_barang_customer_detail`.`kode_list_barang_customer` AND `gx_list_barang_customer_detail`.`kode_barang`='".$d."' ORDER BY `gx_list_barang_customer`.`id` DESC LIMIT 0,1";
	$query_read_data = mysql_query($sql_read_data, $conn);
	$array_read_data = mysql_fetch_array($query_read_data);
	
	$sql_detail = "INSERT INTO `gx_cek_list_barang_bongkar_detail`(`id`, `kode_cek_list_barang_bongkar`, `no_penerimaan_barang`, `kode_alat`, `nama_barang`, `quantity`, `serial_number`, `status`, `date_add`, `date_upd`, `user_add`, `user_upd`, `level`)
	VALUES (NULL,'-','".$c."','".$d."','".$array_read_data['nama_barang']."','".$array_read_data['qty']."','".$array_read_data['serial_number']."','0',NOW(),NOW(),'".$loggedin['username']."','".$loggedin['username']."','0')";
	
	$query_detail = mysql_query($sql_detail, $conn);
	if($query_detail){
	    	echo"<script>alert('berhasil dipindah ke gudang!');
		window.location.href='".$redirect_update_data."?c=".$c."';
		</script>";
	}else{
	    	echo"<script>alert('error system!');
		window.location.href='".$redirect_update_data."?c=".$c."';
		</script>";
	}
}
    ...
    $content = '<section class="content-header">
				...
				<!-- form start -->
					...
					<div class="row">
						...
						<table class="table table-hover">
						    ...
						    $sql_2 = "SELECT * FROM `gx_jawaban_spk_bongkar`, `gx_jawaban_spk_bongkar_list_alat` WHERE `gx_jawaban_spk_bongkar_list_alat`.`kode_jawaban_spk_bongkar`=`gx_jawaban_spk_bongkar`.`kode_jawaban_spk_bongkar` ";
						    $query_2 = mysql_query($sql_2, $conn);
						
							if(isset($c)){
							    $no = 1;
							    while($row_2 = mysql_fetch_array($query_2)){	
								$content .= '<tr><td>'.$no.'</td><td>'.$row_2['nama_barang'].'</td><td>'.$row_2['qty'].'</td><td>'.$row_2['serial_number'].'</td></tr>';
							    $no++;
							    } 
							}
					...
					<div class="row">
						...
						<table class="table table-hover">
							...
							$sql_1 = "SELECT distinct `kode_customer` FROM `tbCustomer`,`gx_cek_list_barang_bongkar` WHERE `tbCustomer`.`cKode`=`gx_cek_list_barang_bongkar`.`kode_customer` AND `gx_cek_list_barang_bongkar`.`no_penerimaan_barang`='".$c."' AND `gx_cek_list_barang_bongkar`.`level`='0'";
				...
				$content .= '
				 ...
				 <!--
					...
					<div class="row">
						...
						<table class="table table-hover">
						    <thead>
							<tr><th>No</th><th>Nama Barang</th><th>Quantity</th><th>Serial Number</th><th>Masuk ke Gudang</th></tr>
						    </thead>
						    <tbody>';
						    $c = isset($_GET['c']) ? $_GET['c'] : '';
						    $l = isset($_GET['l']) ? $_GET['l'] : '';
							...
							if(isset($c)){
							    $no = 1;
							    while($row_2 = mysql_fetch_array($query_2)){	
								$content .= '<tr><td>'.$no.'</td><td>'.$row_2['nama_barang'].'</td><td>'.$row_2['qty'].'</td><td>'.$row_2['serial_number'].'</td><td></td></tr>';
							    $no++;
							    } 
							}
				...
				if($c != ''){
					...
					<div class="row">
						...
						<table class="table table-hover">
						    <thead>
							<tr><th>Kode Barang</th><th>Nama Barang</th><th>Qty</th><th>Serial Number</th><th>Masuk ke Gudang</th></tr>
						    </thead>
						    <tbody>';
							...
							while($row_1 = mysql_fetch_array($query_1)){
								...
								if(mysql_num_rows($data_cek) > 0){
								   $link_pindah = 'data telah dipindah ke gudang';								    
								}else{
								   $link_pindah = '<a href="?action=togudang&c='.$c.'&d='.$row_2['kode_list_barang_customer_detail'].'">pindah ke gudang</a>';
								  }
								
								$content .= '<tr><td>'.$row_2['kode_barang'].'</td><td>'.$row_2['nama_barang'].'</td><td>'.$row_2['qty'].'</td><td>'.$row_2['serial_number'].'</td><td>'.$link_pindah.'</td></tr>';
							    }
							}    
