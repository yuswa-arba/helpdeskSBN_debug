...
//INSERT 
    ...
    $url_redirect_insert 		= "master_cek_list_gudang";
    $url_redirect_insert_detail	 	= "form_detail_cek_list_gudang";
...
//UPDATE
    ...
    $redirect_update_data = "form_cek_list_gudang.php";
...
//String Data View
    ...
    $url_form_edit = "form_cek_list_gudang";
    
    $form_name = "form_cek_list_gudang";
    $form_id = "form_cek_list_gudang";
...
if(isset($_POST["save"]))
{
    ...
    if($kode_penerimaan_barang != ""){
    
    
    
    $sql_insert = "INSERT INTO `gx_cek_list_gudang`(`id`, `kode_penerimaan_barang`, `tanggal`, `kode_cabang`, `cabang`, `spk_maintance`, `kode_customer`, `nama_customer`, `latitude`, `longtidute`, `tiang_terdekat`, `nama_created`, `fiber_optic`, `wireless`, `type_connection`, `status`, `date_add`, `date_upd`, `user_add`, `user_upd`, `level`)
    VALUES (NULL,'".$kode_penerimaan_barang."','".$tanggal."','".$kode_cabang."','".$cabang."','".$spk_maintance."','".$kode_customer."','".$nama_customer."','".$latitude."','".$longtidute."','".$tiang_terdekat."','".$nama_created."','','','".$type_connection."','0',NOW(),NOW(),'".$loggedin['username']."','".$loggedin['username']."','0')";
...
}elseif(isset($_POST["update"]))
{
	...
	$data_array_select 	= mysql_fetch_array(mysql_query("SELECT `id` FROM `gx_cek_list_gudang` WHERE `kode_penerimaan_barang`='".$c."' ORDER BY `id` DESC LIMIT 0,1", $conn));
	$id			= $data_array_select['id'];
	
	
	$sql_update = "UPDATE `gx_cek_list_gudang` SET `date_upd`=NOW(),`user_upd`='".$loggedin['username']."',`level`='1' WHERE `kode_penerimaan_barang`='".$c."'";
    ...
    $content = '<section class="content-header">
                ...
                <section class="content">
				    ...
				    $content.=' <div class="col-xs-2">
					    ...
					    <div class="col-xs-4">
						<input type="text" readonly="" class="form-control" required="" name="kode_penerimaan_barang" value="'.(isset($_GET['c']) ? $row["kode_penerimaan_barang"] : 'PBR-'.rand(0000000,9999999) ).'" >
					    </div>
					    <div class="col-xs-2">
						<label>Tanggal</label>
					    </div>
					...
					<div class="row">
					    ...
					    <div class="col-xs-4">
						<input readonly="" type="text" class="form-control" required="" name="cabang" value="'.(isset($_GET['c']) ? $row["cabang"] : '').'" onclick="return valideopenerform(\'data_cabang.php?r=form_cek_list_gudang&f=data_cek_list_gudang\',\'cek_list_barang\');">
					    ...
					    <div class="col-xs-4">
						<input readonly="" type="text" class="form-control" required="" name="spk_maintance" value="'.(isset($_GET['c']) ? $row["spk_maintance"] : '').'" onclick="return valideopenerform(\'data_spk_maintance.php?r=form_cek_list_gudang&f=data_cek_list_gudang\',\'cek_list_gudang\');">
					    </div>
