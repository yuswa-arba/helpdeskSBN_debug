...
//SELECT
    ...
    $select_all_data_valid = "SELECT * FROM `gx_invoice_balik_nama` WHERE `level` =  '0'";
...
//UPDATE
    ...
    //$sql_update_lock 
    $table_update_lock = "gx_invoice_balik_nama";
    $redirect_action_lock = "master_invoice_balik_nama";
...
//String Data View
    //Judul Form
    $judul_form = "Master Invoice Balik Nama";
    $link_form_add_data = '<a href="form_invoice_balik_nama.php" class="btn bg-maroon btn-flat margin">Create New</a>';
    $judul_table = "List Data Invoice Balik Nama";
    $data_field = array("Nama Pelanggan", "Periode Tagihan", "Kode Customer", "NPWP");
    $data_sql_field = array("nama_pelanggan", "periode_tagihan", "kode_customer", "npwp");
    $index_field_sql = "id";
...
if($loggedin["group"] == 'admin'){
    enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Master Invoice Balik Nama");
    global $conn;
...
//select data
	...
	$content .= '<tr>
			...
			$content .= '
			<td width="300px">'.(($row_data["status"] == "1") ? '
				...
				<a href="'.$url_form_detail.'?c='.$row_data[$unix_field_sql].'"><span class="label label-info">Detail</span></a>
				<a href="pdf_invoice_balik_nama?c='.$row_data[$unix_field_sql].'" target="_blank"><span class="label label-info">View PDF</span></a>'
				).'
			</td>
		    </tr>';
	$no++;
