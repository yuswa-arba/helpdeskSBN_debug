        ...
        $no = 1;
            ...
            while ($row_employee = mysql_fetch_array($query_employee)) {
                ...
                $content .='<tr>
                            ...
                            <!--<td>'.$ttl_spk.'</td>-->
                            <td>';
                              if($jml_spk < 8){ $content .= '<a href="" onclick="validepopupform2(\''.mysql_real_escape_string($row_employee["id_employee"]).'\',\''.mysql_real_escape_string($row_employee["cNama"]).'\')">Select</a>'; } else { $content .= '( SPK penuh )'; }
                            $content .= '</td>
                            </tr>';
                $no++;
            }
