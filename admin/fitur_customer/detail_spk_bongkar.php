...
//SELECT 
    ...
    $select_all_data_valid = "SELECT * FROM `gx_spk_bongkar` WHERE `level` =  '0'";
...
//UPDATE
    ...
    $redirect_action_lock = "master_spk_bongkar.php";
...
//String Data View
    ...
    $judul_table = "detail spk bongkar";
    
    //id web
    $title_header = 'Detail SPK Bongkar';
    $submenu_header = 'detail_spk_bongkar';
...
if($loggedin["group"] == 'admin'){
    ...
    $sql_d = "SELECT * FROM `gx_spk_bongkar` WHERE `kode_spk_bongkar`='$c_d' ORDER BY `id` DESC LIMIT 0,1";
    $query_d = mysql_query($sql_d, $conn);
    $data_d = mysql_fetch_array($query_d);    
    

    $kode_spk_bongkar					= isset($data_d['kode_spk_bongkar']) ? mysql_real_escape_string(trim($data_d['kode_spk_bongkar'])) : '';
