...
//SELECT 
    ...
    $select_all_data_valid = "SELECT * FROM `gx_link_budget_maintance` WHERE `level` =  '0'";
...
//UPDATE
    //$sql_update_lock 
    $table_update_lock = "gx_link_budget_maintance";
    $redirect_action_lock = "master_link_budget_maintance.php";
...
//String Data View
    //Judul Form
    $judul_form = "Detail Link Budget Maintance";
    $judul_table = "detail link budget maintance";
    
    //id web
    $title_header = 'Detail Link Budget Maintance';
    $submenu_header = 'detail_link_budget_maintance';
    
    
if($loggedin["group"] == 'admin'){
    enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Detail SPK Bongkar");
    global $conn;
    $id_d = isset($_GET['id']) ? mysql_real_escape_string(trim($_GET['id'])) : '';
    $c_d = isset($_GET['c']) ? mysql_real_escape_string(trim($_GET['c'])) : '';
    $sql_d = "SELECT * FROM `gx_link_budget_maintance` WHERE `kode_link_budget_maintance`='$c_d' ORDER BY `id` DESC LIMIT 0,1";
    $query_d = mysql_query($sql_d, $conn);
    $data_d = mysql_fetch_array($query_d);    
    

    $kode_link_budget_maintance					= isset($data_d['kode_link_budget_maintance']) ? mysql_real_escape_string(trim($data_d['kode_link_budget_maintance'])) : '';
