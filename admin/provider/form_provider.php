...
while($row_parent = mysql_fetch_array($sql_parent))
{
	$content .='<option value="'.$row_parent["id_provider"].'" '.((isset($_GET['id']) AND $row_provider["id_parent"] == $row_parent["id_provider"]) ? 'selected=""' : "").'>'.$row_parent["nama"].'</option>';
									...
									<div class="row">
										...
										<div class="col-xs-8">
										<input type="text" required="" maxlength="20" '.(isset($_GET['id']) ? 'readonly=""' : "").' class="form-control" id="username" name="username" value="'.(isset($_GET['id']) ? $row_user["username"] : "").'">
									...
									<div class="row">
										...
										<div class="col-xs-8">
										<input type="text" '.(isset($_GET['id']) ? "" : 'required=""').' maxlength="20" class="form-control" name="password" value="">
										</div>
														</div>
									</div>
