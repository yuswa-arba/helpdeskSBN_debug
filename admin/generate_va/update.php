	...
	if ($_FILES["file"]["error"] > 0)
		...
		$data_va = fread($fp,filesize($_FILES['file']['tmp_name']));
											...
											*/
											...
											$sql_update_1 = "UPDATE `va` SET `date_upd`=NOW(),`level`='1',`user_upd`='".$loggedin['username']."' WHERE  `id_table`='".$c."'";
											$sql_update_2 = "UPDATE `va_detail` SET `date_upd`=NOW(),`level`='1',`user_upd`='".$loggedin['username']."' WHERE  `id_foreign_key`='".$c."'";
											
											mysql_query($sql_update_1, $conn) or die ("<script language='JavaScript'>
													alert('Maaf Data tidak bisa diupdate ke dalam Database, Ada kesalahan pada table 1!');
													window.history.go(-1);
												    </script>");
											mysql_query($sql_update_2, $conn) or die ("<script language='JavaScript'>
													alert('Maaf Data tidak bisa diupdate ke dalam Database, Ada kesalahan pada table 2!');
													window.history.go(-1);
												    </script>");
											...
											$num_table_1 = $c != '' ? $c : $data_num_tbl_1['id_table']+1;
											...
											if($c != ''){
												$data_num_tbl_2 = mysql_fetch_assoc(mysql_query("SELECT `id_table` FROM `va_detail` WHERE `id_foreign_key`='".$c."' ORDER BY `id_table` DESC LIMIT 1", $conn));	
											}else{
												$data_num_tbl_2 = mysql_fetch_assoc(mysql_query("SELECT `id_table` FROM `va_detail` ORDER BY `id_table` DESC LIMIT 1", $conn));	
											}
											
											
											$num_table_2 = $c != '' ? $data_num_tbl_2['id_table'] : $data_num_tbl_2['id_table']+1;
											$num = $num_table_2;
