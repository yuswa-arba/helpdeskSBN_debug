				    ...
				    <br><br><br>
							...
							<div class="box">
                                ...
                                <div class="box-body no-padding">
									...
									while($row_data = mysql_fetch_array($sql_data))
									{
										...
										<tr>
											<td>'.$no.'.</td>
											<td><a href="'.URL.'data/inpdf?id='.$row_data["id_invoice"].'" target="_blank">'.$row_data["kode_invoice"].'</a></td>
                            ...
                            <div class="box">
                                ...
                                <div class="box-body no-padding">
									...
									$sql_data_complaint		= mysql_query("SELECT * FROM `gx_helpdesk_complaint` WHERE `cust_number` = '".$loggedin["customer_number"]."' AND `level` =  '0' ORDER BY `id_complaint` DESC LIMIT 0,5;", $conn);
									...
									while($row_data_complaint = mysql_fetch_array($sql_data_complaint))
									{
										...
										<tr>
											<td>'.$no.'.</td>
											<td><a href="">'.(($row_data_complaint["problem"] == "") ? $row_data_complaint["problem_select"] : $row_data_complaint["problem"]).'</a></td>
			...
			<section class="col-lg-4 connectedSortable">
			    <div class="box box-solid box-warning">
				    ...
				    Sewa STB:  <b>Rp 40.000,00</b><br><br><br><br><br>
                            ...
                            <div class="box">
                                <div class="box-header">
                                    <h3 class="box-title">Last Channel Seen</h3>
				    <div class="box-tools">
...
$sql_tv_channel = mysqli_query($conn_tv, "SELECT `boss`.`t_consumerecord`.* FROM `boss`.`t_consumerecord` WHERE `CLIENTID` = '".$row_data_tv["CLIENTID"]."' ORDER BY `EXPENSEDT` DESC LIMIT 0,5;");
$no = 1;
while ($row_tv_channel = mysqli_fetch_array($sql_tv_channel)){
	
$content .='                            <tr>
											<td>'.$no.'</td>
                                            <td>'.$row_tv_channel["PRODUCTNAME"].'</td>
                                            <td>'.date("d-m-Y", strtotime($row_tv_channel["EXPENSEDT"])).'</b>'.'</td>
                                            <td></td>
                                        </tr>';
