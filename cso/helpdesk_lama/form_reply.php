...
Please let my supervisor know: mangnik@globalxtreme.net<br>';
	...
	$subject_reply = '['.$id_complaint.'] : '.$subject;
         ...
         $from		= "help.gx@globalxtreme.net";
	 $email_bcc	= "dwi@globalxtreme.net";
...
try {
 ...
 //echo date("Y-m-d H:i:s");
    ...
    $sql_email = mysql_query("SELECT `ID`, `EmailFrom`, `EmailFromP`, `EmailTo`, `DateE`, `DateDb`, `DateRead`, `DateRe`, `Subject`, `Message`, `Message_html`,`id_kategori`, `id_complaint`
			 FROM `gx_email`
			 WHERE `ID` = '".$id_mailbox."'
			 LIMIT 0,1;",$conn);
