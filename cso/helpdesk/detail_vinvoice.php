...
if($loggedin["group"] == 'cso'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open detail Customer");
	...
	$id_invoice		= isset($_GET['inv']) ? mysql_real_escape_string(strip_tags(trim($_GET['inv']))) : "";
	
    $query_customer = "SELECT * FROM `tbCustomer` WHERE `cKode`='".$id_customer."' AND `level` = '0' LIMIT 0,1;";
...
if(isset($_GET["inv"]))
{
    $id_invoice		= isset($_GET['inv']) ? (int)$_GET['inv'] : '';
...
while($row_invoice_item = mysql_fetch_array($sql_invoice_item))
{
    ...
    <td>'.Rupiah((int)$row_invoice_item["price"]).'</td>
    </tr>';
    $no++;
