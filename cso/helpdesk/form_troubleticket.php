 ...
 //echo date("Y-m-d H:i:s");   
	...
	<div >
		...
		<div class="table-container table-form">
		   <!-- <a href="'.URL_CSO.'helpdesk/data_cust.php?r=form_troubleticket" class="btn btn-sm bg-navy btn-flat margin pull-right"
						onclick="return valideopenerform(\''.URL_CSO.'helpdesk/data_cust.php?r=form_troubleticket\',\'troubleticket\');">Search Customer</a>
		    <br>-->
			...
			<td>
			  <input class="form-control" readonly="" name="connection_type" placeholder="Connection Type" type="text" value="'.((isset($_GET['id_complaint']) || isset($_GET['id_troubleticket'])) ? $data_troubleticket['connection_type'] :"") .'">
			</td>
		      </tr>-->
		    ...
		    <tr>
			<td colspan="2">
			   <!--<a href="'.URL_CSO.'helpdesk/data_complaint.php?r=form_troubleticket" class="btn btn-sm bg-navy btn-flat margin pull-right"
						onclick="return valideopenerform(\''.URL_CSO.'helpdesk/data_complaint.php?r=form_troubleticket\',\'troubleticketc\');">Search Complaint</a>-->
		    <br>
			</td>
		      </tr>
			...
			<tr>
				...
				<!--<select name="media" style="width:200px;">
				    <option value="telephone" '.((isset($_GET["id_complaint"]) && $data_troubleticket["media"]== "telephone" ) ? 'selected="selected"' :"") .'>Telephone</option>
				    <option value="email" '.((isset($_GET["id_complaint"]) && $data_troubleticket["media"]== "email" ) ? 'selected="selected"' :"") .'>Email</option>
				    <option value="website" '.((isset($_GET["id_complaint"]) && $data_troubleticket["media"]== "website" ) ? 'selected="selected"' :"") .'>Website</option>
				    <option value="sms" '.((isset($_GET["id_complaint"]) && $data_troubleticket["media"]== "sms" ) ? 'selected="selected"' :"") .'>SMS</option>
				    <option value="walkin" '.((isset($_GET["id_complaint"]) && $data_troubleticket["media"]== "walkin" ) ? 'selected="selected"' :"") .'>Walk In</option>
				</select>-->
			    </td>
		      </tr>
			...
			<td>';
			$wich	="";
			$wich	.= isset($_GET['id_troubleticket']) ? '<input class="required" type="radio" name="which_side" value="customer" style="float:left;" '.((isset($_GET["id_troubleticket"]) && $data_troubleticket["which_side"]== "customer" ) ? "checked" :"") .'> Customer
			...
			$wich	.= isset($_GET['id_complaint']) ? '<input class="form-control" readonly="" name="which_side"  type="text" value="'.(isset($_GET["id_complaint"]) ? $data_troubleticket["which_side"] :"") .'" >' : "";
			$content .=''.$wich.'
			</td>
		      </tr>
		      <tr>
			...
			<td>
			  <textarea hidden="" class="required" name="problem" rows="3" cols="70" style="resize: none;">'.((isset($_GET["id_complaint"]) || $_GET["id_troubleticket"]) ? $data_troubleticket['problem'] :"") .'</textarea>
              <select required="" class="form-control"  name="problem_select" style="width:200px;">';
			...
			$content .='<option value="'.$row_problem["problem"].'" '.((isset($_GET["id_complaint"]) && $data_troubleticket["problem_select"]== $row_problem["problem"] ) ? 'selected="selected"' :"") .'>'.$row_problem["problem"].'</option>';
		    }else{
			...
			<td>
		      ...
		      <tr>
			    ...
			    <td>
				<select name="priority" class="form-control"  style="width:200px;">
	       ...
	       <div class="form-group">																
            <div class="row">						
                ...
                <div class="col-xs-6">					
                    '.((isset($_GET['id_complaint']) OR isset($_GET['id_troubleticket']))  ? $data_troubleticket["created_by"]." on ".date("d-m-Y H:i:s", strtotime($data_troubleticket["date_add"])) : $loggedin["username"]).'				
                </div>					
            </div>						
            </div>						
            ...
            <div class="row">						
                ...
                <div class="col-xs-6">					
                    '.((isset($_GET['id_complaint']) OR isset($_GET['id_troubleticket'])) ? $data_troubleticket["updated_by"]." on ".date("d-m-Y H:i:s", strtotime($data_troubleticket["date_upd"])) : "").'				
                </div>					
            </div>						
            </div>
