...
if(isset($_POST["search"])){
	...
	$sql_complaint = mysql_query("SELECT `gx_outgoing_report`.*  FROM `gx_outgoing_report`
				     WHERE `gx_outgoing_report`.`level` = '0'
					$sql_date
					$sql_uid
					$sql_media
					$sql_status
					$sql_side
				       ORDER BY `gx_outgoing_report`.`date_add`
				       DESC LIMIT $start, $perhalaman;", $conn);
	
	$sql_total_compliant = mysql_num_rows(mysql_query("SELECT `gx_outgoing_report`.*  FROM `gx_outgoing_report`
				     WHERE `gx_outgoing_report`.`level` = '0'
					$sql_date
					$sql_uid
					$sql_media
					$sql_status
					$sql_side
				       ORDER BY `gx_outgoing_report`.`date_add`;", $conn));
	...
	$sql_complaint = mysql_query("SELECT `gx_outgoing_report`.*  FROM `gx_outgoing_report`
				     WHERE `gx_outgoing_report`.`level` = '0'
					 AND `gx_outgoing_report`.`date_add` LIKE '%".date("Y-m-d")."%' AND `gx_outgoing_report`.`id_cso` = '".$id_cso."'
				       ORDER BY `gx_outgoing_report`.`date_add`
				       DESC LIMIT $start, $perhalaman;", $conn);
	
	$sql_total_compliant = mysql_num_rows(mysql_query("SELECT `gx_outgoing_report`.*, `tbCustomer`.`cKode`, `tbCustomer`.`id_cabang`  FROM `gx_outgoing_report`, `tbCustomer`
				     WHERE `gx_outgoing_report`.`level` = '0'
					 AND `gx_outgoing_report`.`cust_number` = `tbCustomer`.`cKode`
...
$content = '<!-- Content Header (Page header) -->
	...
	while($r_complaint = mysql_fetch_array($sql_complaint))
	{
		...
		$trouble_ticket .= '<a href="detail_troubleticket.php?id_troubleticket='.$r_complaint["id_outgoing_report"].'" onclick="return valideopenerform(\'detail_troubleticket.php?id_troubleticket='.$r_complaint["id_outgoing_report"].'\',\'complaint\');">Detail TroubleTicket</a>';
	    
	    }else{
		$trouble_ticket .= '<a href="form_troubleticket.php?id_complaint='.$r_complaint["id_outgoing_report"].'">Create TroubleTicket</a>';
	    }
		...
		<td><a href="detail_outgoing_report.php?id_outgoing='.$r_complaint["id_outgoing_report"].'" onclick="return valideopenerform(\'detail_outgoing_report.php?id_outgoing='.$r_complaint["id_outgoing_report"].'\',\'complaint\');">'.
		(($r_complaint["problem_select"]  != "" ) ? $r_complaint["problem_select"] : "Details").'</a></td>
		<td>'.$status.'</td>
		<td>
		<a href="form_outgoing_report.php?id_outgoing='.$r_complaint["id_outgoing_report"].'">edit</a></td>
...
<!-- datepicker -->
    ...
    $title	= 'Helpdesk Outgoing Report';
