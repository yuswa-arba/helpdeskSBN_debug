...
if( checkRole($loggedin["id_group"], 'email') == "0")  { die( 'Access Denied!' );}
    ...
    if(isset($_POST["update_kategori"]))
    {
	  ...
	  header('location: '.URL_CSO.'helpdesk/mailbox.php');
    }
...
$q = isset($_GET["type"]) ? mysql_real_escape_string($_GET["type"]) : "";
	...
	$sql_email 	= mysql_query("SELECT * FROM `gx_email` WHERE `id_kategori` LIKE '%".$kategori."%' ORDER BY `DateE` DESC LIMIT 0,200;",$conn);
	
    }else{
	$sql_email	= mysql_query("SELECT * FROM `gx_email` ORDER BY `DateE` DESC LIMIT 0,200;",$conn);
    }
