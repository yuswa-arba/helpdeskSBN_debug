...
$sql_email_staff	   	= mysql_fetch_array(mysql_query("select * FROM `gx_pegawai` WHERE `id_employee` = '".$loggedin["id_employee"]."' AND `level` = '0' LIMIT 0,1", $conn));
...
try {
 ...
 //echo date("Y-m-d H:i:s");
    ...
    $content ='<section class="content-header">
		...
		<td>
		    <div class="">
			<input type="radio" name="email_from" value="'.$sql_email_staff["email"].'" readonly="" class="form-control"> Email Staff ( '.(($sql_email_staff["email"] == "") ? "Kosong" : $sql_email_staff["email"]).' )<br>
			<input type="radio" name="email_from" value="'.$sql_email_setting["reply_from"].'" checked="" class="form-control"> Default ( '.$sql_email_setting["reply_from"].' )<br><br>
		    </div>
		</td>
	    </tr>
	    <tr style="vertical-align: middle;">
