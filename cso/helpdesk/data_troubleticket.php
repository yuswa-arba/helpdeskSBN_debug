...
if($loggedin["group"] == 'cso'){
 enableLog("", $loggedin["username"], $loggedin["id_employee"], "Open Data Trouble ticket");
	...
	$sql_email
	$sql_phone
	ORDER BY `gx_helpdesk_complaint`.`date_add` DESC  LIMIT 0,10;";
...
while ($row_troubleticket = mysql_fetch_array($query_troubleticket)) {
	...
	$sql_spk = mysql_query("SELECT `spk_number` FROM `gx_helpdesk_spk` WHERE `id_trouble_ticket` = '".$row_troubleticket["ticket_number"]."' AND `level` ='0' LIMIT 0,1;", $conn);
	$total_spk = mysql_num_rows($sql_spk);
	...
	$content .='<tr>
		    ...
		    <td>'.date("d-m-Y", strtotime($row_troubleticket["date_add"])).'</td>
		    ...
		    <td></td>-->
                    <td>
                      <a href="" onclick="validepopupform3(\''.$row_troubleticket["user_id"].'\',';
					  $content .='\''.$row_troubleticket["cust_number"].'\',';
					  $content .='\''.$row_troubleticket["name"].'\',';
					  $content .='\''.$row_troubleticket["address"].'\',';
					  $content .='\''.$row_troubleticket["phone"].'\',';
					  $content .='\''.$row_troubleticket["email"].'\',';
					  $content .='\''.mysql_real_escape_string($row_troubleticket["note_"]).'\',';
					  $content .='\''.$row_troubleticket["problem_select"].'\',';
					  $content .='\''.$row_troubleticket["ticket_number"].'\',';
					  $content .='\''.$row_troubleticket["id_complaint"].'\')">Select</a>
                    </td>
                  </tr>';
	$no++;
	}
...
<script type=\'text/javascript\'>
	function validepopupform3(uid, custNumber, nama, alamat,ctelp, cemail, cProblem, cProblemSelect, cTrouble, id_complaint){
		...
		window.opener.document.'.$return_form.'.note.value=cProblem;
		window.opener.document.'.$return_form.'.problem_select.value=cProblemSelect;
