...
if(isset($_POST["search"]))
{
	...
	$sql_complaint = mysql_query("SELECT * FROM `gx_helpdesk_spk`, `gx_helpdesk_spk`
				     WHERE `gx_helpdesk_spk`.`level` = '0'
					 AND `gx_helpdesk_spk`.`type_spk` = '1'
					$sql_date
					$sql_uid
					$sql_media
					$sql_status
					$sql_side
				       ORDER BY `gx_helpdesk_spk`.`date_add`
				       DESC LIMIT $start, $perhalaman;", $conn);
	
	$sql_total_compliant = mysql_num_rows(mysql_query("SELECT * FROM `gx_helpdesk_spk`
				     WHERE `gx_helpdesk_spk`.`level` = '0'
					  AND `gx_helpdesk_spk`.`type_spk` = '1'
					$sql_date
					$sql_uid
					...
					$sql_status
					$sql_side
				       ORDER BY `gx_helpdesk_spk`.`date_add`;", $conn));
    
    $hal= "?u=$uid&d=$date&m=$media&s=$status&w=$which_side&";
    $no = $start + 1;

}
elseif(isset($_GET["u"]) || isset($_GET["d"]))
{
	...
	$sql_complaint = mysql_query("SELECT * FROM `gx_helpdesk_spk`
				     WHERE `gx_helpdesk_spk`.`level` = '0'
					  AND `gx_helpdesk_spk`.`type_spk` = '1'
					$sql_date
					$sql_uid
...
$content = '<!-- Content Header (Page header) -->
	...
	<hr>
								<div class="box-header">
                                    <h3 class="box-title">List SPK Maintenance</h3>
									<a href="form_spk.php" class="btn bg-olive btn-flat margin pull-right">Create SPK</a>
                                </div><!-- /.box-header -->
	...
	while($r_complaint = mysql_fetch_array($sql_complaint))
	{
	    ...
	    $sql_hcomplaint	= mysql_query("SELECT * FROM `gx_helpdesk_complaint`  WHERE `id_complaint` = '".$r_complaint["id_complaint"]."'", $conn);
		$row_hcomplaint        = mysql_fetch_array($sql_hcomplaint);
	    $content .='<tr>
		<td>'.$no.'.</td>
		<td>'.$r_complaint["created_by"].'</td>
		<td>'.$r_complaint["cust_number"].'</td>
		<td>'.$row_hcomplaint["user_id"].'</td>
		...
		<td>'.$status.'</td>
		<td><a href="form_spk.php?id_spk='.$r_complaint["id_spk"].'"><span class="label label-info">Edit</span></a>  <a href="spk_maintenance_pdf.php?id='.$r_complaint["id_spk"].'" target="_blank"><span class="label label-info">View PDF</span></a></td>
