...
if(isset($_POST["search"])){
    $date1		= isset($_POST["date1"]) ? trim(strip_tags($_POST["date1"])) : "";
	$date2		= isset($_POST["date2"]) ? trim(strip_tags($_POST["date2"])) : "";
	...
	$sql_date		= (($date1 != "") AND ($date2 != "")) ? "AND (`date_add` BETWEEN '".date("Y-m-d 00:00:00", strtotime($date1))."' AND '".date("Y-m-d 23:59:59", strtotime($date2))."')" : "";
	...
	$sql_complaint = mysql_query("SELECT `gx_helpdesk_complaint`.*  FROM `gx_helpdesk_complaint`
	...
	$sql_total_compliant = mysql_num_rows(mysql_query("SELECT `gx_helpdesk_complaint`.*  FROM `gx_helpdesk_complaint`
				     WHERE `gx_helpdesk_complaint`.`level` = '0'
					$sql_date
					$sql_uid
					$sql_media
    ...
    $hal= "?u=$uid&d1=$date1&d2=$date2&m=$media&s=$status&w=$which_side&";
    $no = $start + 1;
}else{
	$hal = "?";
	...
	$sql_complaint = mysql_query("SELECT `gx_helpdesk_complaint`.*  FROM `gx_helpdesk_complaint`
				     WHERE `gx_helpdesk_complaint`.`level` = '0'
					 AND `gx_helpdesk_complaint`.`date_add` LIKE '%".date("Y-m-d")."%' AND `gx_helpdesk_complaint`.`id_cso` = '".$id_cso."'
				       ORDER BY `gx_helpdesk_complaint`.`date_add`
				       DESC LIMIT $start, $perhalaman;", $conn);
	
	$sql_total_compliant = mysql_num_rows(mysql_query("SELECT `gx_helpdesk_complaint`.*, `tbCustomer`.`cKode`, `tbCustomer`.`id_cabang`  FROM `gx_helpdesk_complaint`, `tbCustomer`
				     WHERE `gx_helpdesk_complaint`.`level` = '0'
					 AND `gx_helpdesk_complaint`.`cust_number` = `tbCustomer`.`cKode`
					 AND `tbCustomer`.`id_cabang` = '".$loggedin["cabang"]."'
					 AND `gx_helpdesk_complaint`.`date_add` LIKE '%".date("Y-m-d")."%' AND `gx_helpdesk_complaint`.`id_cso` = '".$id_cso."'
				       ORDER BY `gx_helpdesk_complaint`.`date_add`;", $conn));
	
}
if(isset($_GET["u"]) || isset($_GET["d1"])){
    $date1		= isset($_GET["d1"]) ? trim(strip_tags($_GET["d1"])) : "";	
    $date2		= isset($_GET["d2"]) ? trim(strip_tags($_GET["d2"])) : "";
...
$content = '<!-- Content Header (Page header) -->
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="box">
								...
								<form action="complaint.php" method="post" name="form_search">
										...
										<div class="row">
											...
											<div class="col-xs-3">
												<input type="text" class="form-control hasDatepicker" id="datepicker" readonly name="date1" id="date1" value="'.date("Y-m-d").'">
											</div>
											...
											<div class="col-xs-3">
												<input type="text" class="form-control hasDatepicker" id="datepicker" readonly name="date2" id="date2" value="'.date("Y-m-d").'">
											</div>
										</div>
										</div>
	...
	while($r_complaint = mysql_fetch_array($sql_complaint))
	{
		...
		}elseif($r_complaint["status"] == "closed")
		{
			...
			<span class="label label-success">'.$r_complaint["updated_by"].'</span>
			<span class="label label-success">'.date("d-m-Y H:i", strtotime($r_complaint["date_upd"])).'</span>';
		...
		<td>'.date("d-m-Y h:i", strtotime($r_complaint["date_add"])).'</td>
