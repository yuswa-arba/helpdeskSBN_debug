...
if(isset($_POST["search"])){
	...
	if($kategori == "1")
	{
		...
		While($row_olt = mysql_fetch_array($sql_server_ras))
		{
			...
			$fh = fopen('/var/www/software/beta/cso/helpdesk/data/olt/list_inactive'.$noa.'.exp', 'w');
			fwrite($fh, $script);
			fclose($fh);
			chmod("/var/www/software/beta/cso/helpdesk/data/olt/list_inactive$noa.exp",0755);
			
			$old_path = getcwd();
			chdir('/var/www/software/beta/cso/helpdesk/data/olt/');
	...
	fwrite($fh, $script);
	fclose($fh);
	chmod("/var/www/software/beta/cso/helpdesk/data/olt/list_inactive.exp",0755);
