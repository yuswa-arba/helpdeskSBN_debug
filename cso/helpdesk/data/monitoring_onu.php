...
if(isset($_POST["search"]))
{
	...
	if($userid != "")
	{
		...
		$result_olt_customer = mysql_query("SELECT * FROM `v_olt_customer` where userid = '".$userid."' LIMIT 0,1;", $conn);
		...
		if($count_olt_customer == 1)
		{
			...
			fwrite($fh, $script);
			fclose($fh);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script.exp",0755);
			
			$flan1 = fopen('olt/script_lan1.exp', 'w');
			fwrite($flan1, $script_lan1);
			fclose($flan1);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script_lan1.exp",0755);
			$flan2 = fopen('olt/script_lan2.exp', 'w');
			fwrite($flan2, $script_lan2);
			fclose($flan2);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script_lan2.exp",0755);
			$flan3 = fopen('olt/script_lan3.exp', 'w');
			fwrite($flan3, $script_lan3);
			fclose($flan3);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script_lan3.exp",0755);
			$flan4 = fopen('olt/script_lan4.exp', 'w');
			fwrite($flan4, $script_lan4);
			fclose($flan4);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script_lan4.exp",0755);
			
			$fpower = fopen('olt/script_power.exp', 'w');
			fwrite($fpower, $script_power);
			fclose($fpower);
			chmod("/var/www/sbn/beta/cso/helpdesk/data/olt/script_power.exp",0755);
			
			
			$old_path = getcwd();
			chdir('/var/www/sbn/beta/cso/helpdesk/data/olt/');
			...
			elseif (strpos($output, 'has bound 1 inactive ONUs') !== false)
			{
				...
				}elseif (strpos($output, 'power off') !== false) {
					$output = 'inactive';
					$output_inactive = 'power off';
				}elseif (strpos($output, 'wire down') !== false) {
					$output = 'inactive';
			...
			if (strpos($lines[$line_data], 'received power(DBm)') !== true) {
				$output_power = '-';
			}
