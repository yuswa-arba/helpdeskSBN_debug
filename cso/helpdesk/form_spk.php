...
if($save == "Save"){
	...
	$note		= isset($_POST['note']) ? mysql_real_escape_string(strip_tags(trim($_POST['note']))) : "";
	...
	$query = "INSERT INTO `gx_helpdesk_spk` (`id_spk`, `spk_number`, `id_complaint`, `id_trouble_ticket`, `cust_number`, `name`, `address`, `note`,
					`type_spk`, `problem_select`, `id_teknisi`, `id_cso`, `date_add`, `date_upd`, `created_by`, `updated_by`, `level`)
					VALUES ('', '$spk_number', $id_complaint, '$ticket_number', '$cust_number', '$namee', '$address', '$note',
					'1', '$problem_select', '$teknisi_id', '".$loggedin["id_employee"]."', NOW(), NOW(), '$created_by', '$created_by', '0');";
	...
	echo "<script language='JavaScript'>
			...
			location.href = 'spk_maintenance.php';
            </script>";
}elseif($edit == "Save"){
	$id_complaint	= isset($_POST['id_complaint']) ? mysql_real_escape_string(strip_tags(trim($_POST['id_complaint']))) : "";
	...
	$query = "UPDATE `gx_helpdesk_spk` SET `id_trouble_ticket` = '$ticket_number',`id_complaint` = '$id_complaint',
						...
						`address` = '$address', `id_cso` = '".$loggedin["id_employee"]."',
						`note` = '$note', `problem_select` = '$problem_select', `id_teknisi` = '$teknisi_id', 
...
$data_spk	= mysql_fetch_array(mysql_query("SELECT `gx_helpdesk_spk`.*, `gx_helpdesk_complaint`.`phone`, `gx_helpdesk_complaint`.`email`, `gx_pegawai`.`nama`,
						...
						AND `gx_helpdesk_spk`.`id_teknisi` = `gx_pegawai`.`id_employee`
...
$sql_last_data  = mysql_num_rows(mysql_query("SELECT * FROM `gx_helpdesk_spk` WHERE `date_add` LIKE '".date("Y-m-d")."%' ORDER BY `date_add` DESC", $conn)) + 1;
    ...
    $content ='<section class="content-header">
	    ...
	    <div class="table-container table-form">	    
			...
			$content .='<option value="'.$row_problem["problem"].'" '.((isset($_GET["id_complaint"]) && $data_spk["problem_select"]== $row_problem["problem"] ) ? 'selected="selected"' :"") .'>'.$row_problem["problem"].'</option>';
		    }else{
			$content .='<option value="" disabled>-----------------------------------</option>';
			  ...
			  <tr style="vertical-align: middle;">
			    ...
			    <td>
				<textarea name="note" id="note" rows="4" cols="70" style="resize: none;">'.(isset($_GET["id_spk"]) ? $data_spk['note'] :"").'</textarea>
				</td>
		      </tr>
		      <tr>
