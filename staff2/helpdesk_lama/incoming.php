...
}elseif($type == "spktech")
{
    $content .= '<hr>
			...
			<th width="2%">#</th>
                ...
                <tbody>';
    $sql_spktech = mysql_query("SELECT  `gx_spk`. * ,  `gx_employee`.`first_name` , `users`.`username`,  `gx_employee`.`last_name` ,  `gx_helpdesk_complaint`.`name` , `gx_helpdesk_complaint`.`status` ,  `gx_helpdesk_complaint`.`note_nclient` ,  `gx_helpdesk_complaint`.`address` ,  `gx_helpdesk_complaint`.`email` ,  `gx_helpdesk_complaint`.`phone` 
                                FROM  `gx_spk` ,  `gx_helpdesk_complaint` ,  `gx_employee` , `users`
                                WHERE  `spk_number` LIKE  '%MCT%'
                                AND  `gx_spk`.`id_complaint` =  `gx_helpdesk_complaint`.`id_complaint` 
                                AND  `gx_spk`.`id_teknisi` =  `gx_employee`.`id_employee`
                                AND  `gx_spk`.`id_teknisi` =  `users`.`id_user`
                                AND `gx_spk`.`level` = '0' ORDER BY `gx_spk`.`date_add` DESC;", $conn);
    $no= 1;
...
<table id="example1" class="table table-bordered table-striped">
    ...
    $sql_spkmkt = mysql_query("SELECT  `gx_spk` . * ,  `gx_employee`.`first_name` , `users`.`username`,  `gx_employee`.`last_name` ,  `gx_helpdesk_complaint`.`name` ,  `gx_helpdesk_complaint`.`note_nclient` ,  `gx_helpdesk_complaint`.`address` ,  `gx_helpdesk_complaint`.`email` ,  `gx_helpdesk_complaint`.`phone` 
			    FROM  `gx_spk` ,  `gx_helpdesk_complaint` ,  `gx_employee` , `users`
			    ...
			    ORDER BY `gx_spk`.`date_add` DESC;", $conn);
    $no= 1;
...
}elseif($type == "complaint")
{
	...
	$sql_complaint = mysql_query("SELECT * FROM `gx_helpdesk_complaint`
				     WHERE `gx_helpdesk_complaint`.`level` = '0' 
				       ORDER BY `gx_helpdesk_complaint`.`log_time`
				       DESC LIMIT 100;", $conn);
