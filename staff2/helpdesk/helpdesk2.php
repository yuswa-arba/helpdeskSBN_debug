...
$content .= '<!-- Content Header (Page header) -->
                ...
                <a href="helpdesk.php?type=complaint">Complaint</a>
                <a href="helpdesk.php?type=spktech">SPK Technician</a>
		<a href="helpdesk.php?type=spkmkt">SPK Marketing</a>
		<a href="helpdesk.php?type=prospek">Prospek</a>
		<a href="helpdesk.php?type=nonprospek">Non Prospek</a>
...
$content .= '
	   ...
	   <hr>
                    <a href="helpdesk.php?type=complaint" class="btn btn-blue">list Complaint</a>
                    <a href="helpdesk.php?type=spktech" class="btn btn-orange">List SPK Teknisi</a>
                    <a href="helpdesk.php?type=spkmkt" class="btn btn-red">List SPK Marketing</a>
                    <a href="helpdesk.php?type=prospek" class="btn btn-green">List Prospek</a>
                    <a href="helpdesk.php?type=nonprospek" class="btn btn-black">List Non Prospek</a>
                    <hr>
        <div class="clear">
...
$sql_nonprospek 	= mysql_query("SELECT `gx_complaint`.*,`gx_employee`.`first_name`,`gx_employee`.`first_name`  FROM  `gx_complaint` ,  `gx_employee` 
...
<table id="example1" class="table table-bordered table-striped">
	...
	$no= 1;
	$sql_prospek = mysql_query("SELECT `gx_complaint`.*,`gx_employee`.`first_name`,`gx_employee`.`first_name`  FROM  `gx_complaint` ,  `gx_employee` 
				WHERE  `gx_complaint`.`id_cso` =  `gx_employee`.`id_employee` 
				AND  `gx_complaint`.`level` =  '0'
				AND  `gx_complaint`.`type_client` =  'nonclient'
...
<hr>
	...
	$sql_spktech = mysql_query("SELECT  `gx_spk`. * ,  `gx_employee`.`first_name` , `users`.`username`,  `gx_employee`.`last_name` ,  `gx_complaint`.`name` , `gx_complaint`.`status` ,  `gx_complaint`.`note_nclient` ,  `gx_complaint`.`address` ,  `gx_complaint`.`email` ,  `gx_complaint`.`phone` 
                                ...
                                AND `gx_spk`.`level` = '0' ORDER BY `gx_spk`.`date_add` DESC");	
	$no= 1;
...
elseif($type == "complaint"){
    ...
    $sql_data_complaint	= mysql_query("SELECT * FROM `gx_complaint`, `gx_employee` WHERE
				      ...
				      ") or (mysql_error());
    $jum_data_complaint = mysql_num_rows(mysql_query("SELECT * FROM `gx_complaint`, `gx_employee` WHERE
				      `gx_complaint`.`id_cso` = `gx_employee`.`id_employee` AND
				      `gx_complaint`.`type_client` = 'client' AND
				      `gx_complaint`.`level` = '0'"));
...
<table id="example1" class="table table-bordered table-striped">
	...
	$sql_complaint = mysql_query("SELECT * FROM `gx_complaint`, `gx_employee` WHERE
				      `gx_complaint`.`id_cso` = `gx_employee`.`id_employee` 
				       AND `gx_complaint`.`level` = '0'  AND `gx_complaint`.`id_cso` = '$id_cso'
				       AND `gx_complaint`.`log_time` = CURDATE()
				       ORDER BY `gx_complaint`.`log_time`
				       DESC");
	while($r_complaint = mysql_fetch_array($sql_data_complaint)){
...
$content .='</tbody>
<tfoot>
<tr><td colspan="10">'.(halaman_software($jum_data_complaint, $perhalaman, 1, "$hal")).'</td></tr>
</tfoot>
</table><br>
...
elseif($type == ""){
header("location:helpdesk2.php?type=complaint");
}
