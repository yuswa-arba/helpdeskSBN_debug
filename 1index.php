...
if (isset($_POST['username']) && isset($_POST['password'])) { // Check if submit button has been pressed.#fdf1f1
	...
	{ // Check if they are correct
	    ...
	    $results = mysql_query("INSERT into `sessions` (`sess_id`, `uid`, `logged`, `ip`, `waktu`)
			       VALUES ('".$_COOKIE['PHPSESSID']."', '".$vcu['id_user']."', '0',
		...
		{ // If it submitted it then success.
			...
			$sql = mysql_query("UPDATE `users` SET `user_active` = 'yes',
					   `user_ip_lastvisit` = '".$_SERVER['REMOTE_ADDR']."'
					    WHERE `id_user` = '".$vcu['id_user']."' ");
