...
if($loggedin["group"] == 'customer'){
	...
	if (file_exists("upload/" . $_FILES["file_foto"]["name"])) {
     ...
     /*
	    ...
	    $uploadedfile = $_FILES['file_foto']['tmp_name'];
	    ...
	    $filename = "upload/". $_FILES['file_foto']['name'];
	...
	*/    
	  //resize new
	   /*
	    $filePath = "upload/" . $_FILES["file_foto"]["name"];
	    $filePath_folder = = "upload/" ."thumb_". $_FILES["file_foto"]["name"];
	    $orig_image = imagecreatefromjpeg($filePath);
	    $image_info = getimagesize($filePath); 
	    $width_orig  = $image_info[0]; // current width as found in image file
	    $height_orig = $image_info[1]; // current height as found in image file
	    $width = 10; // new image width
	    $height = 10; // new image height
